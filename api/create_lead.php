		...
		if(!empty($ENROLLMENT->COURSE_OFFERING)){
			foreach($ENROLLMENT->COURSE_OFFERING as $COURSE_OFFERING) {
				...
				if($PK_COURSE_OFFERING == '') {
					$data['SUCCESS'] = 0;
					$data['MESSAGE'] .= 'Missing ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->ID Value';
				} else {
					...
					if($res_st->RecordCount() == 0){
						...
						$data['MESSAGE'] .= 'Invalid ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->ID Value - '.$PK_COURSE_OFFERING;
					}
				}
				...
				if($FINAL_GRADE_ID != '') {
					...
					if($res_st->RecordCount() == 0){
						...
						$data['MESSAGE'] .= 'Invalid ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->FINAL_GRADE_ID Value - '.$FINAL_GRADE_ID;
					}
				}
				...
				if($PK_COURSE_OFFERING_STUDENT_STATUS != '') {
					...
					if($res_st->RecordCount() == 0){
						...
						$data['MESSAGE'] .= 'Invalid ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->COURSE_OFFERING_STUDENT_STATUS Value - '.$FINAL_GRADE_ID;
					}
				}
				...
				if(!empty($COURSE_OFFERING->ATTENDANCE)){
					foreach($COURSE_OFFERING->ATTENDANCE as $ATTENDANCE) {
						...
						if($ATTENDANCE->DATE == '') {
							...
							$data['MESSAGE'] .= ' Missing ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->ATTENDANCE->DATE Value';
						}
						...
						if($ATTENDANCE->START_TIME == '') {
							...
							$data['MESSAGE'] .= ' Missing ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->ATTENDANCE->START_TIME Value';
						}
						...
						if($PK_ATTENDANCE_CODE != '') {
							...
							if($res_st->RecordCount() == 0){
								...
								$data['MESSAGE'] .= 'Invalid ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->ATTENDANCE->ATTENDANCE_CODE_ID Value - '.$PK_ATTENDANCE_CODE;
							}
						}
				...
				if(!empty($COURSE_OFFERING->NON_SCHEDULED_ATTENDANCE)){
					foreach($COURSE_OFFERING->NON_SCHEDULED_ATTENDANCE as $NON_SCHEDULED_ATTENDANCE) {
						...
						if($NON_SCHEDULED_ATTENDANCE->DATE == '') {
							...
							$data['MESSAGE'] .= ' Missing ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->NON_SCHEDULED_ATTENDANCE->DATE Value';
						}
						...
						if($NON_SCHEDULED_ATTENDANCE->START_TIME == '') {
							...
							$data['MESSAGE'] .= ' Missing ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->NON_SCHEDULED_ATTENDANCE->START_TIME Value';
						}
						...
						if($NON_SCHEDULED_ATTENDANCE->END_TIME == '') {
							...
							$data['MESSAGE'] .= ' Missing ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->NON_SCHEDULED_ATTENDANCE->END_TIME Value';
						}
						...
						if($PK_ATTENDANCE_CODE != '') {
							...
							if($res_st->RecordCount() == 0){
								...
								$data['MESSAGE'] .= 'Invalid ENROLLMENT->COURSE_OFFERING->COURSE_OFFERING->NON_SCHEDULED_ATTENDANCE->ATTENDANCE_CODE_ID Value - '.$PK_ATTENDANCE_CODE;
							}
						}
		...
		foreach($DATA->ENROLLMENT as $ENROLLMENT){
			...
			if(!empty($ENROLLMENT->COURSE_OFFERING)){
				...
				foreach($ENROLLMENT->COURSE_OFFERING as $COURSE_OFFERING) {
					...
					$res_grade = $db->Execute("SELECT * FROM S_COURSE_OFFERING_GRADE WHERE PK_COURSE_OFFERING = '$STUDENT_COURSE[PK_COURSE_OFFERING]' AND PK_ACCOUNT = '$PK_ACCOUNT'"); 
					while (!$res_grade->EOF) {
					...
					if(!empty($COURSE_OFFERING->ATTENDANCE)){
						foreach($COURSE_OFFERING->ATTENDANCE as $ATTENDANCE) {
							...
							if($res_sch->RecordCount() > 0){
								...
								attendance_entry($PK_COURSE_OFFERING_SCHEDULE_DETAIL,0,'',$PK_STUDENT_MASTER,$PK_STUDENT_ENROLLMENT,$PK_STUDENT_SCHEDULE,$ATTENDED_HOUR, $PK_ATTENDANCE_CODE,$PK_ACCOUNT,0);
					...
					if(!empty($COURSE_OFFERING->NON_SCHEDULED_ATTENDANCE)){
						foreach($COURSE_OFFERING->NON_SCHEDULED_ATTENDANCE as $NON_SCHEDULED_ATTENDANCE) {
							...
							$PK_STUDENT_SCHEDULE = create_non_schedule('',$STUDENT_COURSE['PK_COURSE_OFFERING'],$DATE,$START_TIME,$END_TIME,'',$PK_STUDENT_MASTER,$PK_STUDENT_ENROLLMENT, 1,$PK_ACCOUNT,0);
