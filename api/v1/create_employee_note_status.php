...
//$DATA = '{"EMPLOYEE_NOTE_STATUS":"From","DEPARTMENT":"all department"}';
...
if($flag == 1){
	...
	if($DATA->EMPLOYEE_NOTE_STATUS == '') {
		$data['SUCCESS'] = 0;
		if($data['MESSAGE'] != '')
			$data['MESSAGE'] .= ', ';
			
		$data['MESSAGE'] .= 'Employee Note Status Missing';
	} 
	...
	if($data['SUCCESS'] == 1) {
		...
		$NOTE_STATUS['NOTE_STATUS'] 	= trim($DATA->EMPLOYEE_NOTE_STATUS);
		...
		$data['MESSAGE'] = 'Employee Note Status Created';
	}
}
