...
//$DATA = '{"PK_STUDENT_ENROLLMENT":"123","TEST_LABEL":"Test 1", "TEST_RESULT":"aaa", "PASSED":"Yes", "TEST_DATE":"2021-03-02"}';
...
if($flag == 1){
	...
	if($DATA->PK_STUDENT_ENROLLMENT == '' ) {
		$data['SUCCESS'] = 0;
		$data['MESSAGE'] .= 'Missing PK_STUDENT_ENROLLMENT Value';
	} else if($DATA->PK_STUDENT_ENROLLMENT != '') {
		$PK_STUDENT_ENROLLMENT = $DATA->PK_STUDENT_ENROLLMENT;
		
		$res_st = $db->Execute("SELECT PK_STUDENT_ENROLLMENT, PK_STUDENT_MASTER FROM S_STUDENT_ENROLLMENT WHERE PK_ACCOUNT = '$PK_ACCOUNT' AND PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT' AND PK_STUDENT_ENROLLMENT > 0 AND PK_STUDENT_MASTER > 0");
		if($res_st->RecordCount() == 0) {
			$data['SUCCESS'] = 0;
			$data['MESSAGE'] .= 'No Enrollment Found - '.$PK_STUDENT_MASTER;
		} else {
