...
//$DATA = '{"DSIS_OLD_ENROLLMENT_ID":"123","PK_STUDENT_MASTER":"123","DEPARTMENT_ID":"27","NOTE_STATUS_ID":"3","NOTE_TYPE_ID":"2","NOTE_DATE":"2021-04-23","NOTE_TIME":"19:30:00","FOLLOWUP_DATE":"2021-04-28","FOLLOWUP_TIME":"20:30:00","EMPLOYEE_ID":50,"COMPLETED":"No","COMMENTS":"COMMENTS"}';
...
if($flag == 1){
	...
	if($DATA->DSIS_OLD_ENROLLMENT_ID == '' && $DATA->PK_STUDENT_MASTER == '') {
		$data['SUCCESS'] = 0;
		$data['MESSAGE'] .= 'Either DSIS_OLD_ENROLLMENT_ID or PK_STUDENT_MASTER Requires Value';
	} else if($DATA->DSIS_OLD_ENROLLMENT_ID != '') {
		$DSIS_OLD_ENROLLMENT_ID = $DATA->DSIS_OLD_ENROLLMENT_ID;
		
		$res_st = $db->Execute("SELECT PK_STUDENT_MASTER,PK_STUDENT_ENROLLMENT FROM S_STUDENT_ENROLLMENT WHERE PK_ACCOUNT = '$PK_ACCOUNT' AND DSIS_OLD_ENROLLMENT_ID = '$DSIS_OLD_ENROLLMENT_ID' ");
		if($res_st->RecordCount() == 0) {
			$data['SUCCESS'] = 0;
			$data['MESSAGE'] .= 'Invalid DSIS_OLD_ENROLLMENT_ID Value - '.$DSIS_OLD_ENROLLMENT_ID;
		} else {
		...
		$res_st = $db->Execute("SELECT PK_STUDENT_MASTER,PK_STUDENT_ENROLLMENT FROM S_STUDENT_ENROLLMENT WHERE PK_ACCOUNT = '$PK_ACCOUNT' AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND IS_ACTIVE_ENROLLMENT = 1");
		if($res_st->RecordCount() == 0) {
			$data['SUCCESS'] = 0;
			$data['MESSAGE'] .= 'Invalid PK_STUDENT_MASTER Value - '.$PK_STUDENT_MASTER;
		} else {
			$PK_STUDENT_MASTER 		= $res_st->fields['PK_STUDENT_MASTER'];
			$PK_STUDENT_ENROLLMENT 	= $res_st->fields['PK_STUDENT_ENROLLMENT'];
		}
