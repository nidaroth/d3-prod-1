...
if(!empty($_POST)){
	...
	$res = $db->Execute("SELECT PK_USER FROM Z_USER WHERE ID = '$_GET[id]' AND PK_USER_TYPE = 2 "); 
	...
	db_perform('Z_USER', $USER, 'update'," PK_USER = '$PK_USER' AND PK_ACCOUNT = '$_GET[s_id]'  ");
								...
								<div class="p-20">
									...
									<div class="row">
										...
										<div class="col-9 col-sm-9">
											...
											<button type="button" onclick="window.location.href='user?id=<?=$_GET['id']?>&s_id=<?=$_GET['s_id']?>'"  class="btn waves-effect waves-light btn-dark">Cancel</button>
										</div>
									</div>
								</div>
							</form>
