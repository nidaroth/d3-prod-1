...
if(!empty($_POST)){
	...
	if($_GET['id'] == ''){
		...
		db_perform('Z_TEXT_TEMPLATE', $TEXT_TEMPLATE, 'insert');
	} else {
		$TEXT_TEMPLATE['EDITED_BY'] = $_SESSION['PK_USER'];
		$TEXT_TEMPLATE['EDITED_ON'] = date("Y-m-d H:i");
		db_perform('Z_TEXT_TEMPLATE', $TEXT_TEMPLATE, 'update'," PK_TEXT_TEMPLATE = '$_GET[id]' ");
	}
...
if($_GET['id'] == ''){
	...
	$res = $db->Execute("SELECT * FROM Z_TEXT_TEMPLATE WHERE PK_TEXT_TEMPLATE = '$_GET[id]' "); 
