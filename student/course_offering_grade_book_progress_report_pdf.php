...
class MYPDF extends TCPDF {
    public function Header() {
		...
		if($_SESSION['temp_id'] == $this->PK_STUDENT_ENROLLMENT){
			...
			$this->SetTextColor(000, 000, 000);
			$this->SetX(150);
			//$this->Cell(55, 8, "Student Transcript", 0, false, 'L', 0, '', 0, false, 'M', 'L'); //DIAM-2141
		} else 
...
// $_REQUEST['id'] = $_SESSION['PK_STUDENT_ENROLLMENT'];
// $_REQUEST[id] = $_SESSION['PK_STUDENT_ENROLLMENT'];

$_SESSION['temp_id'] = '';
//Ticket # 936
if(has_custom_sap_report($_SESSION['PK_ACCOUNT'])){ 
	require_once("../school/pdf_custom_sap_header.php"); //Ticket # 936
//Ticket # 936	
}else{
...
function co_grade_book_progress_pdf($PK_STUDENT_MASTERS, $one_stud_per_pdf, $report_name = null){
	...
	if($_REQUEST['term_id'] != '')
	{
		$cond3 = " AND S_TERM_MASTER.PK_TERM_MASTER = '$_REQUEST[term_id]' ";
	}
	if($_REQUEST['stud_course_id'] != '')
	{
		$cond4 = " AND S_COURSE_OFFERING.PK_COURSE = '$_REQUEST[stud_course_id]' ";
	}
	if($_REQUEST['type'] == 1) {
		
		$label = "All Enrollments";
		$cond  = " AND S_STUDENT_COURSE.PK_STUDENT_ENROLLMENT IN (".$_REQUEST['eid'].") ";
		$cond1 = " AND PK_STUDENT_ENROLLMENT IN (".$_REQUEST['eid'].") ";
		$cond2 = " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT IN (".$_REQUEST['eid'].") ";

	} else if($_REQUEST['type'] == 2) {
		$res_en11 = $db->Execute("SELECT PK_STUDENT_ENROLLMENT FROM S_STUDENT_ENROLLMENT WHERE PK_STUDENT_MASTER = '$_REQUEST[id]' AND IS_ACTIVE_ENROLLMENT = 1");
		...
		$label = "By Term";
		$cond  = " AND S_COURSE_OFFERING.PK_TERM_MASTER = '$_REQUEST[term]' ";
		$cond1 = " AND PK_TERM_MASTER = '$_REQUEST[term]' ";
		$cond2 = " AND S_STUDENT_ENROLLMENT.PK_TERM_MASTER = '$_REQUEST[term]' ";
	}
	...
	foreach($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
		...
		$en_cond3 = "";
		if($_REQUEST['eid'] != ''){
			$en_cond = " AND PK_STUDENT_ENROLLMENT IN ($_REQUEST[eid]) ";
			$en_cond3 = " AND S_STUDENT_CREDIT_TRANSFER.PK_STUDENT_ENROLLMENT IN ($_REQUEST[eid]) ";
		}
		...
		$txt .= '<div style="border-top: 2px #c0c0c0 solid" ></div>
				<table border="0" cellspacing="0" cellpadding="3" width="100%" >
					<tr>
						<td width="100%" align="center" ><b><i style="font-size:50px">Course Offering Grade Book Progress Report</i></b><br /></td>
					</tr>
				</table>
				<br /><br />';
				...
				if($res_stu_point->RecordCount() > 0) {
					...
					S_COURSE_OFFERING_GRADE.PK_GRADE_BOOK_TYPE = M_GRADE_BOOK_TYPE.PK_GRADE_BOOK_TYPE $cond $cond3 $show_cond 
					...
					if($res_term->RecordCount() > 0) {		
						...
						$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%" >'.$thead_start.'
							...
							while (!$res_term->EOF) { 
								...
								S_COURSE_OFFERING_GRADE.PK_COURSE_OFFERING = S_COURSE_OFFERING.PK_COURSE_OFFERING AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' $in_progress $cond1 ");
								...
								if($res_stu_point->RecordCount() > 0) {
									...
									S_COURSE_OFFERING_GRADE.PK_GRADE_BOOK_TYPE = M_GRADE_BOOK_TYPE.PK_GRADE_BOOK_TYPE $cond $cond4 $show_cond 
												...
												if($res_stu_point->RecordCount() > 0){
													...
													while (!$res_stu_point->EOF) { 
														...
														if($Points_Earned != "")
														{
															...
															$Percentage_Earned = number_format_data_value(($STUD_WEIGHTED_POINTS / $CO_WEIGHTED_POINTS * 100),2).' %';
														}
														...
														if($Sub_Tot_Weighted_Points_Earned != "")
														{
															...
															$Sub_Tot_Weighted_Per_Earned = number_format_data_value((($SUB_TOT_STUD_WEIGHTED_POINTS) / ($SUB_TOT_CO_WEIGHTED_POINTS) * 100),2).' %';
														}
														/** End DIAM-1182 **/
											...
											if($flag == 1){
												...
												$txt .= '<tr>
														...
														$txt .= '<td width="20%" style="'.$border_1.'" ><i>Weighted Current Total:</i></td>
															...
															<td width="15%" style="'.$border_1.'" align="right" >'.number_format_data_value($per1,2).' %</td>
															<td width="10%"></td>
														</tr>';
												/* Ticket # 1219   */
											}
			...
			<td width="100%" >
				...
				$txt .= '<tr>';
					...
					$txt .= '<td width="7%" style="border-bottom:1px solid #000;border-top:1px solid #000;'.$border_left.'" align="right" ><br /><br /><b>Hours Attended</b></td>
						...
						if($_REQUEST['report_type'] == 1) {
							$txt .= '<td width="6%" style="border-bottom:1px solid #000;border-top:1px solid #000;" align="right" ><br /><br /><b>Numeric Grade</b></td>
					...
					/* Ticket #1146 */
					$include_tc = 1;
					if($_REQUEST['exclude_tc'] == 1)
						$include_tc = 0;

					if($include_tc == 1 ) { 
						
						$sQuery_Res = "SELECT S_COURSE.TRANSCRIPT_CODE, 
										...
										WHERE 
											...
											AND S_STUDENT_CREDIT_TRANSFER.PK_GRADE = S_GRADE.PK_GRADE $en_cond3 ";
						
						$res_tc = $db->Execute($sQuery_Res);
					...
					if(has_custom_sap_report($_SESSION['PK_ACCOUNT'])==1 && $_REQUEST['type']==3){
						$cond="";
					}
					...
					M_SESSION.PK_SESSION = S_COURSE_OFFERING.PK_SESSION AND CALCULATE_GPA = 1 $cond $show_cond ");
					while (!$res_course->EOF) {
					...
					$sQuery_Course = "SELECT S_COURSE_OFFERING.PK_COURSE_OFFERING, 
											...
											NUMERIC_GRADE, 
											FINAL_TOTAL_GRADE_NUMBER_GRADE, 
					...
					$res_course = $db->Execute($sQuery_Course);
					...
					if(has_ccmc_access($_SESSION['PK_ACCOUNT'],1))
					{
						if($total_schedule > 0)
						{
							$total_attended_percentage = ($total_attended / $total_schedule) * 100;
						}
					}
					else
					{
				...
				$txt .=	'<td width="7%" align="right" style="'.$border_3.$border_left.'" >'.number_format_value_checker($total_attended,2).'</td> 
						...
						if($_REQUEST['report_type'] == 1) {
							$txt .=	'<td width="6%" align="right" style="'.$border_3.'" >'.number_format_value_checker(($Numerator/$Denominator),2).'</td>
									<td width="6%" align="right" style="'.$border_3.'" >'.number_format_value_checker(($summation_of_gpa/$summation_of_weight),2).'</td>';
						} else {
							$txt .=	'<td width="8%" align="right" style="'.$border_3.'border-right:1px solid #000;" >'.number_format_value_checker(($summation_of_gpa/$summation_of_weight),2).'</td>';
						}
					$txt .=	'</tr>
					<tr>';
					if($_REQUEST['report_type'] != 1) {
					...
					$txt .=	'<td width="29%" ></td>
						...
						<td width="8%" align="right"  ></td>
						<td width="8%" align="right"  ></td>';
						if($_REQUEST['report_type'] == 1 && has_custom_sap_report($_SESSION['PK_ACCOUNT'])==0 ) {
							$txt .=	'<td width="14%" align="right"  ><i>(Cumulative GPA)</i></td>';
						}
					$txt .=	'</tr>';

					if(has_custom_sap_report($_SESSION['PK_ACCOUNT'])==1 && $_REQUEST['report_type'] == 1 && $include_tc!=1)
					{
						$txt.='<tr>
					...
					}else if(has_custom_sap_report($_SESSION['PK_ACCOUNT'])==1 && $_REQUEST['report_type'] == 1 && $include_tc==1){
						$txt.='<tr>
						<td width="60%"></td>
						<td width="40%" align="right"><i>(Cumulative GPA)</i></td>
						</tr>
	...
	// echo $txt; exit;
	$file_name = 'Course_Offering_Grade_Book_Progress_Report_'.uniqid().'.pdf';

	if($one_stud_per_pdf == 0) {
		$file_dir_1 = '../school/temp/';
		if($_REQUEST['download_via_js'] == 'yes'){
...
if($_REQUEST['id'] == '') {
	...
	$_REQUEST['eid'] = $res->fields['PK_STUDENT_ENROLLMENT'];
	
	co_grade_book_progress_pdf($_SESSION['PK_STUDENT_MASTER'], 0);
} else {
	...
	if($_REQUEST['zip'] == 1) {
		...
		if($res === TRUE) {
			$PK_STUDENT_MASTER_ARR = explode(",",$_REQUEST['id']);
			foreach($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
				$file_name_1 = co_grade_book_progress_pdf($PK_STUDENT_MASTER, 1);
		...
		co_grade_book_progress_pdf($_REQUEST['id'], 0);
}
