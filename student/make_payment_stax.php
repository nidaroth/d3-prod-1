...
$res_card = $db->Execute("SELECT * FROM S_STUDENT_CREDIT_CARD_STAX WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_STUDENT_MASTER = '$_SESSION[PK_STUDENT_MASTER]' ");
...
<body class="horizontal-nav boxed skin-megna fixed-layout">
   <? require_once("pre_load.php"); ?>
									...
									<div class="row">
										...
										<div class="col-6">
                                                    ...
                                                    <div class="row form-group" >
                                                        <div class="col-md-6 align-self-center">
                                                            <select id="PK_STUDENT_CREDIT_CARD_STAX" name="PK_STUDENT_CREDIT_CARD_STAX" class="form-control required-entry" onchange="show_cc_field(this.value)" >
                                                                ...
                                                                while (!$res_type->EOF) { ?>
                                                                    <option value="<?=$res_type->fields['PK_STUDENT_CREDIT_CARD_STAX'] ?>" ><?=$res_type->fields['CARD_NO'].' - '.$res_type->fields['CARD_EXP'].' - '.$res_type->fields['CARD_TYPE'].' - '.$res_type->fields['NAME_ON_CARD']?></option>
                                                                <?	$res_type->MoveNext();
                                                                } ?>
                                                            </select>
                                                            <span class="bar"></span>
                                                            <label for="PK_STUDENT_CREDIT_CARD_STAX"><?=CARD_NO?></label>
                                                        </div>
                                                    </div>
											<? } ?>            
                                            ...
                                            <div id="CC_FIELDS" style="<?=$style?>" >
                                                ...
                                                <div class="d-flex">
                                                    <!-- <div style="width:55%; height:35px; display: inline-block;" >
                                                    ...
                                                    <div style="width:10%; height:35px; display: inline-block;">
                                                    ...
                                                    <div id="card-element" class="col-7 col-sm-7 form-group">
											...
											<div class="row">
												...
												<div class="col-9 col-sm-9">
													...
													<button type="button" onclick="window.location.href='payments'"  class="btn waves-effect waves-light btn-dark"><?=CANCEL?></button>
												</div>
											</div>
	...
	{
		var value = $.ajax({
			url: "../school/ajax_get_paymet_method_details",	
			type: "POST",		 
        ...
        if(flag == 1) 
        {
            if(type == 1) 
            {
                jQuery(document).ready(function($) {
                    ...
                    var value = $.ajax({
                        url: "../school/ajax_calc_stax_transaction_charge",	
                        type: "POST",		 
                        data: 'id='+card_id+'&transaction_initiation_type=CIT&amt=<?=$AMOUNT?>',		
                        async: false,
                        cache: false,
                        success: function (data) {	
                            if(data == 'success')
