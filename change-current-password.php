...
if(!empty($_POST)){
	...
	if($result1->RecordCount() > 0){
		...
		$NEW_PASSWORD		= crypt($_POST['PASSWORD'], '$2y$12$' . $salt);
		
		$hash  	   		    = $result1->fields['PASSWORD'];
		$CURRENT_PASSWORD 	= crypt($_POST['CURRENT_PASSWORD'], $hash);
		$NEW_PASSWORD1 		= crypt($_POST['PASSWORD'], $hash);

		$result = $db->Execute("SELECT PK_USER FROM Z_USER where PK_USER = '$_SESSION[PK_USER]' AND PASSWORD = '$CURRENT_PASSWORD' ");
		if($result->RecordCount() == 0){
			$msg = 'Invalid Current Password.';
		} else if($NEW_PASSWORD1 == $CURRENT_PASSWORD){
			$msg = 'Passwords Cannot Be Reused.<br />Please Enter a New and Different Password.';
		} else {
			$USER_1['PASSWORD'] 			= $NEW_PASSWORD;
                ...
                <div class="card-body">
                    <form class="form-horizontal form-material" method="post" id="loginform" name="loginform" action="">
						...
						<? } ?>
						 ...
						 <div class="form-group">
                            <div class="col-xs-12">
                                <input class="form-control required" id="CURRENT_PASSWORD" name="CURRENT_PASSWORD" type="password" placeholder="Current Password">
							</div>
                        </div>
