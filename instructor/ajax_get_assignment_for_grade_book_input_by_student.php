...
<table class="table table-bordered">
	...
	<tbody>
		...
		while (!$result1->EOF) {
			...
			$res_stu_grade = $db->Execute("SELECT PK_STUDENT_GRADE,POINTS,PK_STUDENT_MASTER FROM S_STUDENT_GRADE WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING_GRADE = '$PK_COURSE_OFFERING_GRADE' AND PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT' ");

			if (!empty($res_stu_grade->fields['PK_STUDENT_GRADE'])) {
