...
if($_GET['id'] == ''){
	...
	$res = $db->Execute("select * from Z_INTERNAL_EMAIL $table WHERE 1=1 $cond");
	if($res->RecordCount() == 0 ){
		header("location:my_mails?type=draft");
		exit;
	}
...
<body class="horizontal-nav boxed skin-megna fixed-layout">
									...
									<div class="col-xlg-10 col-lg-9 col-md-8 bg-light border-left">
										<div class="card-body">
											...
											<div class="row">
												...
												<div class="col-md-4 form-group">
													<select id="PK_TERM" name="PK_TERM[]" multiple class="form-control" onchange="get_co()" >
														<? $res_type = $db->Execute("select PK_TERM_MASTER,IF(BEGIN_DATE = '0000-00-00','',DATE_FORMAT(BEGIN_DATE, '%m/%d/%Y' )) AS  BEGIN_DATE_1 from S_TERM_MASTER WHERE ACTIVE = 1 AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]'  order by BEGIN_DATE DESC");
														while (!$res_type->EOF) { ?>
															<option value="<?=$res_type->fields['PK_TERM_MASTER']?>" ><?=$res_type->fields['BEGIN_DATE_1']?></option>
														<?	$res_type->MoveNext();
												...
												<div class="col-md-4 form-group" id="co_div" >
											...
											<div class="form-group" id="RECEPTION_DIV" >
												...
												include("../school/ajax_search_student_for_mail.php"); ?>
											</div>
											...
											<div class="form-group">
												...
												<? } else { ?>
													<input type="text" class="form-control required-entry" placeholder="<?=SUBJECT?>" id="SUBJECT" name="SUBJECT" value="<?=$SUBJECT?>" >
												<? } ?>
											</div>
											<div class="form-group">
	...
	<script type="text/javascript">
		...
		function get_co(val){
			jQuery(document).ready(function($) { 
				var data  = 'PK_TERM_MASTER='+$('#PK_TERM').val();
				var value = $.ajax({
					url: "../school/ajax_get_course_offering_from_term",	
					type: "POST",		 
					data: data,		
					async: false,
					cache: false,
					success: function (data) {	
						//alert(data)
						document.getElementById('co_div').innerHTML = data;
						...
						document.getElementById('PK_COURSE_OFFERING').setAttribute("onchange", "doSearch()");
		...
		function doSearch(){
			jQuery(document).ready(function($) {
				var data  = 'PK_CAMPUS='+$('#PK_CAMPUS').val()+'&PK_STUDENT_STATUS='+$('#PK_STUDENT_STATUS').val()+'&PK_CAMPUS_PROGRAM='+$('#PK_CAMPUS_PROGRAM').val()+'&PK_TERM_MASTER='+$('#PK_TERM_MASTER').val()+'&PK_COURSE_OFFERING='+$('#PK_COURSE_OFFERING').val()+'&INTERNAL_ID=<?=$INTERNAL_ID?>';
				var value = $.ajax({
					url: "../school/ajax_search_student_for_mail",	
					type: "POST",		 
					data: data,		
					async: false,
					cache: false,
