										...
										<table data-toggle="table" data-height="500" data-mobile-responsive="true" class="table-striped" >
											<thead>
												<tr>
													...
													$res_fields = $db->Execute("SELECT FIELDS,FIELD_SIZE from S_CUSTOM_REPORT_DETAIL WHERE PK_CUSTOM_REPORT = '$_GET[id]' AND FIELD_FOR = 'INFO' AND FIELDS != '' AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ORDER BY SORT_ORDER ASC, PK_CUSTOM_REPORT_DETAIL ASC");	
													while (!$res_fields->EOF){
														...
														} else if($field == "IF(S_STUDENT_ENROLLMENT.ENTRY_DATE = '0000-00-00','',DATE_FORMAT(S_STUDENT_ENROLLMENT.ENTRY_DATE,'%m/%d/%Y' )) AS ENTRY_DATE" ){
															$val = ENTRY_DATE;
