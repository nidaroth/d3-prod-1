										...
										<table data-toggle="table" data-height="500" data-mobile-responsive="true" class="table-striped" id="table_1" >
											<thead>
												<tr>
													...
													<? $res_type = $db->Execute("select FIELD_NAME, IF(TAB = 'info',1,2) as TAB_ORDER from S_CUSTOM_FIELDS WHERE SECTION = 1 AND S_CUSTOM_FIELDS.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ORDER BY TAB_ORDER ASC, PK_CUSTOM_FIELDS ASC");
													while (!$res_type->EOF) { ?>
											...
											<tbody>
												...
												$query = "SELECT S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT,S_STUDENT_MASTER.PK_STUDENT_MASTER, S_STUDENT_MASTER.LAST_NAME, S_STUDENT_MASTER.FIRST_NAME, S_STUDENT_MASTER.MIDDLE_NAME, S_STUDENT_MASTER.OTHER_NAME AS STU_OTHER_NAME, S_STUDENT_MASTER.SSN AS SSN, IF(S_STUDENT_MASTER.SSN_VERIFIED = 1,'Yes','No') AS SSN_VERIFIED, IF(S_STUDENT_MASTER.DATE_OF_BIRTH != '0000-00-00',DATE_FORMAT( S_STUDENT_MASTER.DATE_OF_BIRTH, '%m/%d/%Y'),'') AS DATE_OF_BIRTH, Z_GENDER.GENDER AS GENDER, S_STUDENT_MASTER.DRIVERS_LICENSE, Z_STATES_DRIVERS_LICENSE.STATE_NAME AS STATES_DRIVERS_LICENSE_STATE, Z_MARITAL_STATUS_STUD.MARITAL_STATUS AS STU_MARITAL_STATUS, Z_COUNTRY_CITIZEN.NAME AS COUNTRY_CITIZEN, Z_CITIZENSHIP.CITIZENSHIP, PLACE_OF_BIRTH, Z_STATES_OF_RESIDENCY.STATE_NAME AS STATE_OF_RESIDENCY, S_STUDENT_ACADEMICS.STUDENT_ID, S_STUDENT_ACADEMICS.ADM_USER_ID, M_HIGHEST_LEVEL_OF_EDU.HIGHEST_LEVEL_OF_EDU AS HIGHEST_LEVEL_OF_EDU, IF(PREVIOUS_COLLEGE = 1,'Yes','No') AS PREVIOUS_COLLEGE, S_STUDENT_MASTER.BADGE_ID, IF(FERPA_BLOCK = 1,'Yes',IF(FERPA_BLOCK = 2,'No','')) AS FERPA_BLOCK, S_STUDENT_MASTER.IPEDS_ETHNICITY AS IPEDS_ETHNICITY, '' AS RACE,IF(S_TERM_MASTER.BEGIN_DATE = '0000-00-00','',DATE_FORMAT(S_TERM_MASTER.BEGIN_DATE,'%m/%d/%Y' )) AS TERM_DATE,M_CAMPUS_PROGRAM.CODE, M_STUDENT_STATUS.STUDENT_STATUS AS STUDENT_STATUS, IF(S_STUDENT_ENROLLMENT.STATUS_DATE = '0000-00-00','',DATE_FORMAT(S_STUDENT_ENROLLMENT.STATUS_DATE,'%m/%d/%Y' )) AS STATUS_DATE, CONCAT(S_EMPLOYEE_MASTER.FIRST_NAME,' ',S_EMPLOYEE_MASTER.LAST_NAME) AS EMP_NAME, M_LEAD_CONTACT_SOURCE.LEAD_CONTACT_SOURCE, M_LEAD_SOURCE.LEAD_SOURCE, IF(CONTRACT_SIGNED_DATE = '0000-00-00','',DATE_FORMAT(CONTRACT_SIGNED_DATE,'%m/%d/%Y' )) AS CONTRACT_SIGNED_DATE, IF(CONTRACT_END_DATE = '0000-00-00','',DATE_FORMAT(CONTRACT_END_DATE,'%m/%d/%Y' )) AS CONTRACT_END_DATE, IF(ENTRY_DATE = '0000-00-00','',DATE_FORMAT(ENTRY_DATE,'%m/%d/%Y' )) AS ENTRY_DATE, IF(ENTRY_TIME = '00-00-00','',DATE_FORMAT(ENTRY_TIME,'%h:%i %p' )) AS ENTRY_TIME, IF(EXPECTED_GRAD_DATE = '0000-00-00','',DATE_FORMAT(EXPECTED_GRAD_DATE,'%m/%d/%Y' )) AS EXPECTED_GRAD_DATE, IF(ORIGINAL_EXPECTED_GRAD_DATE = '0000-00-00','',DATE_FORMAT(ORIGINAL_EXPECTED_GRAD_DATE,'%m/%d/%Y' )) AS ORIGINAL_EXPECTED_GRAD_DATE, CONCAT(M_ENROLLMENT_STATUS.CODE,' - ',M_ENROLLMENT_STATUS.DESCRIPTION) AS FULL_PART_TIME, SESSION, CONCAT(M_FUNDING.FUNDING,' - ',M_FUNDING.DESCRIPTION) AS FUNDING, STUDENT_GROUP, '' AS CAMPUS, IF(FT_PT_EFFECTIVE_DATE != '0000-00-00',DATE_FORMAT(FT_PT_EFFECTIVE_DATE, '%m/%d/%Y'),'') AS FT_PT_EFFECTIVE_DATE, IF(MIDPOINT_DATE != '0000-00-00',DATE_FORMAT(MIDPOINT_DATE, '%m/%d/%Y'),'') AS MIDPOINT_DATE, M_FIRST_TERM.FIRST_TERM AS FIRST_TERM, IF(REENTRY = 1,'Yes','No') AS REENTRY, IF(TRANSFER_IN = 1,'Yes','No') AS TRANSFER_IN, IF(TRANSFER_OUT = 1,'Yes','No') AS TRANSFER_OUT, DISTANCE_LEARNING, PLACEMENT_STATUS, IF(STRF_PAID_DATE != '0000-00-00',DATE_FORMAT(STRF_PAID_DATE, '%m/%d/%Y'),'') AS STRF_PAID_DATE, IF(GRADE_DATE != '0000-00-00',DATE_FORMAT(GRADE_DATE, '%m/%d/%Y'),'') AS GRADE_DATE, IF(LDA != '0000-00-00',DATE_FORMAT(LDA, '%m/%d/%Y'),'') AS LDA, IF(DETERMINATION_DATE != '0000-00-00',DATE_FORMAT(DETERMINATION_DATE, '%m/%d/%Y'),'') AS DETERMINATION_DATE, IF(DROP_DATE != '0000-00-00',DATE_FORMAT(DROP_DATE, '%m/%d/%Y'),'') AS DROP_DATE, DROP_REASON, IF(IMPORT_STATUS = 1,'Success',IF(IMPORT_STATUS = 2,'Error',IF(IMPORT_STATUS = 3,'Duplicate',''))) AS IMPORT_STATUS , IMPORT_ERROR
												FROM 
												S_STUDENT_MASTER 
												...
												while (!$res_type->EOF) { 
													...
													<tr>
														...
														<? $res_cust = $db->Execute("select PK_CUSTOM_FIELDS,FIELD_NAME,PK_DATA_TYPES, PK_USER_DEFINED_FIELDS, TAB, IF(TAB = 'info',1,2) as TAB_ORDER from S_CUSTOM_FIELDS WHERE SECTION = 1 AND S_CUSTOM_FIELDS.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ORDER BY TAB_ORDER ASC, PK_CUSTOM_FIELDS ASC");
														while (!$res_cust->EOF) { 
