...
$res_fields = $db->Execute("SELECT FIELDS,FIELD_SIZE from S_CUSTOM_REPORT_DETAIL WHERE PK_CUSTOM_REPORT = '$_GET[id]' AND FIELD_FOR = 'INFO' AND FIELDS != '' ORDER BY SORT_ORDER ASC, PK_CUSTOM_REPORT_DETAIL ASC");	
while (!$res_fields->EOF){
	...
	$txt .= '<td width="'.$res_fields->fields['FIELD_SIZE'].'%" style="border-top:1px solid #000;border-bottom:1px solid #000;"><b><i>'.$val.'</i></b></td>';
	...
	while (!$res->EOF){
		...
		foreach($custom_fields_arr as $indes_2 => $PK_CUSTOM_FIELDS){ 
			...
			if($custom_fields_type_arr[$indes_2] == 1 || $custom_fields_type_arr[$indes_2] == 4) { 
				//Text, Date
				$res_stu_cus = $db->Execute("select FIELD_VALUE FROM S_STUDENT_CUSTOM_FIELDS WHERE PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND PK_CUSTOM_FIELDS = '$PK_CUSTOM_FIELDS' ");
				...
				//Drop Down
				$res_stu_cus = $db->Execute("select OPTION_NAME FROM S_STUDENT_CUSTOM_FIELDS, S_USER_DEFINED_FIELDS_DETAIL WHERE PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND PK_CUSTOM_FIELDS = '$PK_CUSTOM_FIELDS' AND PK_USER_DEFINED_FIELDS_DETAIL =  FIELD_VALUE");
