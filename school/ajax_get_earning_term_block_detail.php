...
<div class="row">
		...
		<div class="tableFixHead" >
			<table class="table table-hover table-striped" id="stud_ledger_table_1" > 
				...
				<tbody>
					...
					$EARNINGS_MONTH 		= "";
					$EARNINGS_YEAR_MONTH 	= "";
					$res = $db->Execute("CALL ACCT20015(".$_SESSION['PK_ACCOUNT'].",".$_REQUEST['id'].")");
					while (!$res->EOF) { 
						if($i == 0) {
							$i++;
	...
	<div class="col-3 col-sm-3">
		...
		if(strtolower($STATUS) == "finalized") { 
			...
			?>
			<button onclick="earning_operation('CALCULATE','<?=$NEXT_EARNINGS_YEAR?>','<?=$NEXT_EARNINGS_MONTH?>','<?=$NEXT_EARNINGS_YEAR_MONTH?>')" style="width: 166px;" type="button" class="btn waves-effect waves-light btn-info"><?=CALCULATE.' '.$NEXT_EARNINGS_YEAR_MONTH ?></button>
			<br /><br />
