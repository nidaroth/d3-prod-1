...
$res_fields = $db->Execute("SELECT FIELDS , SORT_ORDER from S_CUSTOM_REPORT_DETAIL WHERE PK_CUSTOM_REPORT = '$_GET[id]' AND FIELD_FOR = 'INFO' AND FIELDS != '' ORDER BY SORT_ORDER ASC, PK_CUSTOM_REPORT_DETAIL ASC ");	
...
foreach ($original_order_array as $key => $value) {
	$index++;
	$cell_no = $cell[$index].$line;
	$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($value[0]);
	$objPHPExcel->getActiveSheet()->getStyle($cell_no)->getFont()->setBold(true);
	$objPHPExcel->getActiveSheet()->getColumnDimension($cell[$index])->setWidth(15);
}
...
foreach ($sorted_data as $key => $value) {
	...
	foreach ($value as $key1 => $value1) {
		$index++;
		$cell_no = $cell[$index].$line;
		$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValueExplicit($value1,PHPExcel_Cell_DataType::TYPE_STRING);
		echo $cell_no.' --- '.$value1."<br>";
	}
	echo "<br>----------<br>";
	# code...
}
