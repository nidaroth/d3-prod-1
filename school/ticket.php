...
if(!empty($_POST)){
	...
	if($_GET['id'] == '' ){
		...
		$TICKET['PK_TICKET_STATUS']  	= 1;
		$TICKET['IS_PARENT']  			= 1;
		$TICKET['TICKET_NO']  			= $TICKET_NO;
		$TICKET['PK_ACCOUNT']  			= $_SESSION['PK_ACCOUNT'];
		$TICKET['CREATED_BY']  			= $_SESSION['PK_USER'];
	...
	for($k = 0 ; $k < count($_FILES['ATTACHMENT']['name']) ; $k++){
		...
		if (trim($name)!=""){				
			...
			$file11 = $name1.'-'.$_SESSION['PK_USER'].$rand_string.".".$extn[$iindex];						
...
if($_GET['id'] == ''){
	...
	$result = $db->Execute("select * from Z_TICKET WHERE INTERNAL_ID = '$_GET[id]' AND (PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' OR PK_ACCOUNT = '-1') ");
