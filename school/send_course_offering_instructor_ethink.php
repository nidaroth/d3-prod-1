...
if(!empty($_POST)){
	...
	foreach($_POST['PK_COURSE_OFFERING'] as $PK_COURSE_OFFERING_TEMP){
		$temp = explode("_",$PK_COURSE_OFFERING_TEMP);
		$PK_COURSE_OFFERING = $temp[0];
		$PK_EMPLOYEE_MASTER = $temp[1];
		
		assign_instructor_to_course_offering($PK_COURSE_OFFERING,$PK_EMPLOYEE_MASTER,$_SESSION['PK_ACCOUNT'],$RAND_ID);
		
	}
	header("location:send_course_offering_instructor_ethink_result?id=".$RAND_ID);
}
...
<head>
	...
	<title><?=MNU_MOODLE.' - '.MNU_SEND_COURSE_OFFERING_INSTRUCTOR ?> | <?=$title?></title>
            ...
            <div class="container-fluid">
                <div class="row page-titles">
                    <div class="col-md-12 align-self-center" >
                        <h4 class="text-themecolor"><?=MNU_MOODLE.' - '.MNU_SEND_COURSE_OFFERING_INSTRUCTOR?> </h4>
                    </div>
				</div>
					...
					<div class="col-md-2 " style="max-width:14%;flex: 14%;" >
                       <select id="PK_COURSE" name="PK_COURSE[]" multiple class="form-control " onchange="doSearch();" >
						   <? $res_type = $db->Execute("select PK_COURSE, COURSE_CODE, ACTIVE FROM S_COURSE WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ORDER BY ACTIVE DESC, COURSE_CODE ASC ");
							while (!$res_type->EOF) { 
								$str = $res_type->fields['COURSE_CODE'];
								if($res_type->fields['ACTIVE'] == 0)
									$str .= ' (Inactive)'; ?>
								<option value="<?=$res_type->fields['PK_COURSE'] ?>"><?=$res_type->fields['COURSE_CODE'] ?></option>
							<?	$res_type->MoveNext();
							} ?>
						</select>
					</div>  
				...
				<div class="row" style="padding-bottom: 10px;"  >
					<div class="col-md-12 align-self-center text-right" >
					   <button type="button" onclick="window.location.href='send_course_offering_instructor_ethink_result_excel'" name="btn" class="btn waves-effect waves-light btn-info"  ><?=EXPORT_TO_EXCEL?></button>
						<button type="button" onclick="window.location.href='send_course_offering_instructor_ethink_result_pdf'" name="btn" class="btn waves-effect waves-light btn-info"  ><?=EXPORT_TO_PDF?></button>
						...
						<div class="col-12">
							<div class="card">
								<div class="card-body">
									<div class="row">
										<div class="col-md-12">
											<table id="tt" striped = "true" class="easyui-datagrid table table-bordered table-striped" url="grid_send_course_offering_instructor_ethink"
											toolbar="#tb" pagination="true" pageSize = 25 >
												<thead>
													<tr>
														...
														<th field="BEGIN_DATE" width="100px" align="left" sortable="true" ><?=TERM?></th>
