...
?>
	<table class="main-table" id="grade_input_table">
		<thead>
			<tr>
			...
			//print_r($PK_COURSE_OFFERING_GRADE_ARR_123);exit;
		...
		<tr>
			...
			<td>
				<div style="width:200px" id="FINAL_TOTAL_<?=$PK_STUDENT_COURSE?>" >
					...
					$res_stu_grade_1a = $db->Execute("SELECT PK_STUDENT_GRADE FROM S_STUDENT_GRADE_HISTROY WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING_GRADE IN ($PK_COURSE_OFFERING_GRADE_ids) AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND POINTS != ''  AND `CREATED_ON` = '$last_date' "); 
					if($res_stu_grade_1a->RecordCount() > 0) {
					/* Ticket #1505 */
						$FINAL_PERCENTAGE  = number_format(($res_stu->fields['FINAL_TOTAL_OBTAINED'] / $res_stu->fields['FINAL_MAX_TOTAL'] * 100),2);
			...
			<td>
				<div style="width:200px" id="CURRENT_TOTAL_<?=$PK_STUDENT_COURSE?>" >
					...
					if($res_stu_grade_1a->RecordCount() > 0) { //Ticket #1505
						$CURRENT_PERCENTAGE = number_format(($res_stu->fields['CURRENT_TOTAL_OBTAINED'] / $res_stu->fields['CURRENT_MAX_TOTAL'] * 100),2);
			...
			foreach($PK_COURSE_OFFERING_GRADE_ARR_123 as $PK_COURSE_OFFERING_GRADE){
				//echo "SELECT PK_STUDENT_GRADE,POINTS FROM S_STUDENT_GRADE_HISTROY WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING_GRADE = '$PK_COURSE_OFFERING_GRADE' AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' ";
				...
				$PK_STUDENT_GRADE = $res_stu_grade->fields['PK_STUDENT_GRADE']?$res_stu_grade->fields['PK_STUDENT_GRADE']:0;  ?>
				<td >
					<input type="text" data-id="<?=$PK_STUDENT_COURSE?>" onkeyup="check_number_validation(this)" class="form-control grade_check stu_points_<?=$PK_STUDENT_MASTER?> COG_<?=$PK_COURSE_OFFERING_GRADE?>" placeholder="" name="GRADE_INPUT_POINTS[]" id="GRADE_INPUT_POINTS_<?=$PK_STUDENT_GRADE?>" value="<?=$res_stu_grade->fields['POINTS']?>" onchange="calc_grade(<?=$PK_STUDENT_GRADE?>,<?=$PK_STUDENT_COURSE?>,<?=$PK_STUDENT_MASTER?>,0)" style="width:80px;text-align:right;padding:0;margin: 0;height: 21px;min-height: 0;" />
