...
// For stating campus 
if ($_POST['FORMAT'] == 1  && $_REQUEST['REPORT_OPTION'] == '1') {
	/////////////////////////////////////////////////////////////////
	$browser = '';
	...
	$pdf->AddPage();
} else if ($_POST['FORMAT'] == 1  && $_REQUEST['REPORT_OPTION'] == '2') {
	...
	$pdf->SetFont('helvetica', '', 7, '', true);
	$pdf->AddPage();
} else if ($_POST['FORMAT'] == 1  && $_REQUEST['REPORT_OPTION'] == '4') {
	/////////////////////////////////////////////////////////////////
...
if (!empty($_POST)) {
	if ($_POST['FORMAT'] == 2) {
		...
		if($_REQUEST['REPORT_OPTION'] == '2'){
			$file_name 		= 'Disbursed Funds Attendance.xlsx';
		}
		else {
	...
	foreach ($_REQUEST['PK_LEDGER_CODE_GROUP'] as $key1 => $PK_LEDGER_CODE_GROUP1) {
		...
		if ($_REQUEST['REPORT_OPTION'] == '1') {
			...
			if ($_POST['FORMAT'] == 1) {
				...
				$txt .= '<h2 style="page-break-before: always">' . $ar_ledger_codes->fields['LEDGER_CODE_GROUP'] . '</h2>';
				$total 	= 0;
				$summary_arr = [];
				...
				$final_group_summary[$ar_ledger_codes->fields['LEDGER_CODE_GROUP']] = $total;
				...
				while (!$res_ledger->EOF) {
					...
					while (!$res_disp->EOF) {
						...
						$final_group_summary[$ar_ledger_codes->fields['LEDGER_CODE_GROUP']] += $res_disp->fields['DISBURSEMENT_AMOUNT'];
		...
		if ($_REQUEST['REPORT_OPTION'] == '2') {
			...
			if ($_POST['FORMAT'] == 1) {
				...
				if (!empty($summary_arr)) {
					...
					$txt .= '<tr><td style="' . $border_all . '"><b>Total</b></td><td style="' . $border_all . '"style="' . $border_all . '"><b>$ ' . number_format_value_checker($tmp_total, 2) . '</b></td></tr>';
					$txt .= '</table>';
					$final_group_summary[$ar_ledger_codes->fields['LEDGER_CODE_GROUP']] = $tmp_total;
			...
			//DIAM-1763
			include('projected_funds_by_hours_by_code_group.php');
		}
		...
		// DIAM-1666
			...
			if ($_POST['FORMAT'] == 1) {
				...
				$summary_arr = [];
				$res_ledger = $db->Execute("SELECT PK_AR_LEDGER_CODE,CODE FROM M_AR_LEDGER_CODE WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND TYPE = 1 $ledger_cond ORDER BY CODE ASC ");
				$txt .= '<table border="1" cellspacing="0" cellpadding="8" width="100%">
							<tr> 
								<td align="left" style="font-size:18px;" ><i>Ledger Codes</i></td>
								<td align="right" style="font-size:18px;" ><i>Amount</i></td>
							</tr>';
				while (!$res_ledger->EOF) {
					$PK_AR_LEDGER_CODE = $res_ledger->fields['PK_AR_LEDGER_CODE'];
				...
				$txt .= ' 
							<tr>
								...
								<td width="20%" align="left" style="border-top:1px solid #000;font-size:20px;" ><i> Total</i></td>
								<td width="20%" align="right" style="border-top:1px solid #000;font-size:20px;" ><i>$' . number_format_value_checker($total, 2) . '</i></td>
...
if ($_POST['FORMAT'] == 1  && ($_REQUEST['REPORT_OPTION'] == '1')) {
	$html_body_cont = '<!DOCTYPE HTML>
				...
				<body>' . $txt . '</body></html>';
	// echo "<pre>";
	// echo htmlspecialchars($html_body_cont);exit;
	...
	exit;
} else if ($_POST['FORMAT'] == 1  && ($_REQUEST['REPORT_OPTION'] == '2' || $_REQUEST['REPORT_OPTION'] == '4')) {
	...
	$pdf->Output('temp/' . $file_name, 'FD');
	return $file_name;
}else if ($_POST['FORMAT'] == 1  && $_REQUEST['REPORT_OPTION'] == '3'){
	$html_body_cont = '<!DOCTYPE HTML>
				<html>
				<head> <style>
				table{  margin-top: 2px; }
...
#required function for  report option 4  ,  initated here to avoid repeated  declaration
