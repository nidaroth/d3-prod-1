...
// echo realpath(__DIR__.'/../school/').'/';
// exit;

$sql="SELECT PK_STUDENT_DISBURSEMENT, S_STUDENT_DISBURSEMENT.PK_STUDENT_MASTER, S_STUDENT_DISBURSEMENT.PK_STUDENT_ENROLLMENT, S_STUDENT_DISBURSEMENT.PK_ACCOUNT, DISBURSEMENT_DATE, DISBURSEMENT_AMOUNT  
FROM 
...
PK_DISBURSEMENT_STATUS = 2 AND INVOICE = 1 AND ENABLE_AUTO_PAYMENT = 1 AND ENABLE_DIAMOND_PAY = 1 AND 
S_STUDENT_DISBURSEMENT.PK_AR_LEDGER_CODE = M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE AND 
Z_ACCOUNT.PK_ACCOUNT = S_STUDENT_DISBURSEMENT.PK_ACCOUNT AND DISBURSEMENT_DATE = '$date' AND S_STUDENT_DISBURSEMENT.PK_ACCOUNT IN(15,80)";
...
while (!$res_auto_pmt->EOF) {
	...
	if($res_card->RecordCount() > 0) {
		...
		echo "<hr>";
		echo "PK ACCOUNT= ".$res_auto_pmt->fields['PK_ACCOUNT'];
		echo "<br>";
		echo "PK_STUDENT_DISBURSEMENT =".$res_auto_pmt->fields['PK_STUDENT_DISBURSEMENT'];
		echo "<br>";
