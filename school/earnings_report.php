...
if(!empty($_POST)){
	...
	if($_POST['REPORT_TYPE'] == 1){
		if($_POST['FORMAT'] == 1){
			...
			$res = $db->Execute("CALL ACCT20011(".$_SESSION['PK_ACCOUNT'].", ".$_POST['PK_CAMPUS'].", ".$_POST['YEAR'].", ".$_POST['MONTH'].", 0,'ERROR REPORT PDF')");
...
$outputFileName = str_replace(
	...
	$outputFileName );  
			...
			$line 	= 1;				
			$res = $db->Execute("CALL ACCT20011(".$_SESSION['PK_ACCOUNT'].", ".$_POST['PK_CAMPUS'].", ".$_POST['YEAR'].", ".$_POST['MONTH'].", 0,'ERROR REPORT EXCEL')");
			if(count($res->fields) == '0')
			{
				$report_error = "No data in the report for the selections made.";
			}
		...
		if($_POST['FORMAT'] == 1){
			...
			$res = $db->Execute("CALL ACCT20011(".$_SESSION['PK_ACCOUNT'].", ".$_POST['PK_CAMPUS'].", ".$_POST['YEAR'].", ".$_POST['MONTH'].", 0,'MONTH EARNINGS PDF')");
					...
					if($res->fields['RECORD_TYPE'] != 'REPORT TOTAL') {
						if($PROGRAM != $res->fields['PROGRAM']) {
							...
							$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%">
									<thead>
										<tr>
											<th colspan="12" align="left" ><b style="font-size:20px">'.$res->fields['PROGRAM'].' — '.$res->fields['PROGRAM_DESCRIPTION'].' — '.$res->fields['PROGRAM_MONTHS'].' — Earnings Type : '.$res->fields['EARNINGS_TYPE'].'</b><br /></th>
										</tr>
										<tr>
						...
						if($res->fields['RECORD_TYPE'] == 'DETAIL') {
							$txt .= '<tr>
										...
										<td >'.$res->fields['CALCULATION_DATE'].'</td>
										<td >'.$res->fields['FINALIZED_DATE'].'</td>
							...
							$txt .= '<tr>
										...
										<td style="border-top:1px solid #000;" align="right"><b>$'.number_format_value_checker($res->fields['TUITION_CHARGED'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right"><b>$'.number_format_value_checker($res->fields['PREVIOUS_EARNINGS'], 2).'</b></td>
										<td style="border-top:1px solid #000;" ></td>
										<td style="border-top:1px solid #000;" ></td>
										<td style="border-top:1px solid #000;" ></td>
										<td style="border-top:1px solid #000;" ></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['EARNINGS_AMOUNT'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['UNEARNED_TUITION'], 2).'</b></td>
									</tr>';
						}
					}
				...
				if($res->fields['RECORD_TYPE'] == 'REPORT TOTAL') {
					$txt .= '<tr>
							...
							<tr>
								...
								<td style="border-bottom:1px solid #000;border-top:1px solid #000;" align="right"><b>$'.number_format_value_checker($res->fields['TUITION_CHARGED'], 2).'</b></td>
								<td style="border-bottom:1px solid #000;border-top:1px solid #000;" align="right"><b>$'.number_format_value_checker($res->fields['PREVIOUS_EARNINGS'], 2).'</b></td>
								<td style="border-bottom:1px solid #000;border-top:1px solid #000;" ></td>
								<td style="border-bottom:1px solid #000;border-top:1px solid #000;" ></td>
								<td style="border-bottom:1px solid #000;border-top:1px solid #000;" ></td>
								<td style="border-bottom:1px solid #000;border-top:1px solid #000;" ></td>
								<td style="border-bottom:1px solid #000;border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['EARNINGS_AMOUNT'], 2).'</b></td>
								<td style="border-bottom:1px solid #000;border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['UNEARNED_TUITION'], 2).'</b></td>
							</tr>';
				}
		...
		if($_POST['FORMAT'] == 1){
			...
			foreach($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
				...
				$footer = '<table width="100%" >
								<tr>
									<td width="33%" valign="top" style="font-size:10px;" ><i>'.$date.'</i></td>
									<td width="43%" valign="top" style="font-size:10px;" align="center" >
										ACCT20011
										<br>
										Prorated Reasons: &nbsp;&nbsp;&nbsp; L-LOA &nbsp;&nbsp;&nbsp; H-Holiday &nbsp;&nbsp;&nbsp; B-Break   C-Closure &nbsp;&nbsp;&nbsp; F-First Month</td>
									<td width="23%" valign="top" style="font-size:10px;" align="right" ><i>Page {PAGENO} of [pagetotal]</i></td>
								</tr>
							</table>';	
...
$outputFileName = str_replace(
	...
	$outputFileName );  
		...
		$res = $db->Execute("CALL ACCT20011(".$_SESSION['PK_ACCOUNT'].", ".$_POST['PK_CAMPUS'].", ".$_POST['YEAR'].",0 , 0,'YEARLY EARNINGS')");
		
		if($_SESSION['PK_ACCOUNT']==80)
		{
		$row=array();
		...
		while (!$res->EOF) {
			...
			$row[$res->fields['PK_STUDENT_MASTER']][]=array(
				'CAMPUS_CODE'=>$res->fields['CAMPUS_CODE'],
				'EARNINGS_TYPE'=>$res->fields['EARNINGS_TYPE'],
				'EARNINGS_YEAR'=>$res->fields['EARNINGS_YEAR'],
				'PROGRAM'=>$res->fields['PROGRAM'],
				'PROGRAM_DESCRIPTION'=>$res->fields['PROGRAM_DESCRIPTION'],
				'ENROLLMENT_BEGIN_DATE'=>$res->fields['ENROLLMENT_BEGIN_DATE'],
				'STUDENT'=>$res->fields['STUDENT'],
				'STUDENT_ID'=>$res->fields['STUDENT_ID'],
				'SESSION'=>$res->fields['SESSION'],
				'CURRENT_STATUS'=>$res->fields['CURRENT_STATUS'],
				'ENROLLMENT_END_DATE'=>$res->fields['ENROLLMENT_END_DATE'],
				'END_DATE_TYPE'=>$res->fields['END_DATE_TYPE'],
				'PROGRAM_GROUP'=>$res->fields['PROGRAM_GROUP'],
				'TUITION_CHARGED'=>$res->fields['TUITION_CHARGED'],
				'PREVIOUS_EARNINGS'=>$res->fields['PREVIOUS_EARNINGS'],
				'UNEARNED_PRIOR_YEAR'=>$res->fields['UNEARNED_PRIOR_YEAR'],
				'JAN'=>$res->fields['JAN']?$res->fields['JAN']:0,
				'FEB'=>$res->fields['FEB'],
				'MAR'=>$res->fields['MAR'],
				'APR'=>$res->fields['APR'],
				'MAY'=>$res->fields['MAY'],
				...
				'NOV'=>$res->fields['NOV'],
				'DEC'=>$res->fields['DEC'],
				'YEAR_TOTAL'=>$res->fields['YEAR_TOTAL'],
				'UNEARNED_YEAR_END'=>$res->fields['UNEARNED_YEAR_END']
			...
			$s=0;
				foreach($val as $k=>$v){
					...
					$new[$key]=array(
						...
						'EARNINGS_TYPE'=>$v['EARNINGS_TYPE'],
						'EARNINGS_YEAR'=>$v['EARNINGS_YEAR'],
						'PROGRAM'=>$v['PROGRAM'],
						'PROGRAM_DESCRIPTION'=>$v['PROGRAM_DESCRIPTION'],
						'ENROLLMENT_BEGIN_DATE'=>$v['ENROLLMENT_BEGIN_DATE'],
						'STUDENT'=>$v['STUDENT'],
						'STUDENT_ID'=>$v['STUDENT_ID'],
						'SESSION'=>$v['SESSION'],
						'CURRENT_STATUS'=>$v['CURRENT_STATUS'],
						'ENROLLMENT_END_DATE'=>$v['ENROLLMENT_END_DATE'],
						'END_DATE_TYPE'=>$v['END_DATE_TYPE'],
						'PROGRAM_GROUP'=>$v['PROGRAM_GROUP'],
						'TUITION_CHARGED'=>$v['TUITION_CHARGED'],
						'PREVIOUS_EARNINGS'=>$v['PREVIOUS_EARNINGS'],
						'UNEARNED_PRIOR_YEAR'=>$v['UNEARNED_PRIOR_YEAR'],
						'JAN'=>$v['JAN']?$v['JAN']:0,
						'FEB'=>$v['FEB'],
						'MAR'=>$v['MAR'],
						'APR'=>$v['APR'],
						...
						'UNEARNED_YEAR_END'=>$v['UNEARNED_YEAR_END']
					);
				}else{
					...
					$new[$key]['NOV'] +=$v['NOV'];
					$new[$key]['DEC'] +=$v['DEC'];
					$new[$key]['YEAR_TOTAL'] +=$v['YEAR_TOTAL'];
					$new[$key]['PREVIOUS_EARNINGS'] +=$v['PREVIOUS_EARNINGS'];
				}
		...
		if(count($new)>0)
		{
			...
			foreach ($new as $earnings) 
			{
				...
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['CAMPUS_CODE']);

				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['EARNINGS_TYPE']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['EARNINGS_YEAR']);

				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['PROGRAM']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['PROGRAM_DESCRIPTION']);

				$ENROLLMENT_BEGIN_DATE = '';
				if($earnings['ENROLLMENT_BEGIN_DATE'] != '' && $earnings['ENROLLMENT_BEGIN_DATE'] != '0000-00-00')
					$ENROLLMENT_BEGIN_DATE = date("Y-m-d", strtotime($earnings['ENROLLMENT_BEGIN_DATE']));
				...
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['STUDENT']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['STUDENT_ID']);

				if ($earnings['SESSION'] != '') 
				{
					$option_label = substr($earnings['SESSION'],0,1).' - '.$earnings['SESSION'];
				}
				else{
					$option_label = '';
				}
				...
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['CURRENT_STATUS']);
							
				$ENROLLMENT_END_DATE = '';
				if($earnings['ENROLLMENT_END_DATE'] != '' && $earnings['ENROLLMENT_END_DATE'] != '0000-00-00')
					$ENROLLMENT_END_DATE = date("Y-m-d", strtotime($earnings['ENROLLMENT_END_DATE']));
				...
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['END_DATE_TYPE']);

				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['PROGRAM_GROUP']);

				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['PROGRAM_MONTHS']);
							
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['TUITION_CHARGED']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['PREVIOUS_EARNINGS']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['UNEARNED_PRIOR_YEAR']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['JAN']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['FEB']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['MAR']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['APR']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['MAY']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['JUN']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['JUL']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['AUG']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['SEP']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['OCT']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['NOV']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['DEC']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['YEAR_TOTAL']);
				
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($earnings['UNEARNED_YEAR_END']);
					...
					if($res->fields['RECORD_TYPE'] != 'REPORT TOTAL') {
						if($PROGRAM != $res->fields['PROGRAM']) {
							...
							$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%">
									<thead>
										<tr>
											<th colspan="11" align="left" ><b style="font-size:20px">'.$res->fields['PROGRAM'].' — '.$res->fields['PROGRAM_DESCRIPTION'].' — '.$res->fields['PROGRAM_MONTHS'].' — Earnings Type : '.$res->fields['EARNINGS_TYPE'].'</b><br /></th>
										</tr>
										<tr>
											<th width="14%" style="border-bottom:1px solid #000;border-top:1px solid #000;" align="left" >
												<b><i>Program</i></b>
						...
						if($res->fields['RECORD_TYPE'] == 'DETAIL') {
							...
							$txt .= '<tr>
										...
										<td >'.$res->fields['STUDENT_ID'].'</td>
										<td >'.$res->fields['STUDENT_STATUS'].'</td>
										<td align="right" >$'.number_format_value_checker($res->fields['CURRENT_EARNINGS'], 2).'</td>
										<td align="right" >$'.number_format_value_checker($res->fields['TOTAL_EARNINGS'], 2).'</td>
										<td align="right" >$'.number_format_value_checker($res->fields['UNEARNED_TUITION'], 2).'</td>
										<td align="right" >$'.number_format_value_checker($res->fields['TOTAL_TUITION'], 2).'</td>
										<td align="right" >$'.number_format_value_checker($res->fields['NON_TUITION_CHARGED'], 2).'</td>
										<td align="right" >$'.number_format_value_checker($res->fields['TOTAL_RECEIVED'], 2).'</td>
										<td align="right" >$'.number_format_value_checker($res->fields['RECEIVED_NOT_EARNED_POSITIVE'], 2).'</td>
										<td align="right" >$'.number_format_value_checker($res->fields['RECEIVED_NOT_EARNED_NEGATIVE'], 2).'</td>
									</tr>';
						} else if($res->fields['RECORD_TYPE'] == 'PROGRAM TOTAL') {
							$txt .= '<tr>
										<td style="border-top:1px solid #000;" colspan="3" ></td>
										<td style="border-top:1px solid #000;" ><b>Program Totals</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['CURRENT_EARNINGS'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['TOTAL_EARNINGS'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['UNEARNED_TUITION'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['TOTAL_TUITION'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['NON_TUITION_CHARGED'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['TOTAL_RECEIVED'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['RECEIVED_NOT_EARNED_POSITIVE'], 2).'</b></td>
										<td style="border-top:1px solid #000;" align="right" ><b>$'.number_format_value_checker($res->fields['RECEIVED_NOT_EARNED_NEGATIVE'], 2).'</b></td>
									</tr>
									<tr>
										<td style="border-top:1px solid #000;" colspan="10" align="right" >RECEIVED NOT EARNED BALANCE</td>
										<td style="border-top:1px solid #000;" colspan="2" align="right" ><b>$'.number_format_value_checker(($res->fields['RECEIVED_NOT_EARNED_POSITIVE'] - $res->fields['RECEIVED_NOT_EARNED_NEGATIVE']), 2).'</b></td>
									</tr>';
						}
					}
...
<body class="horizontal-nav boxed skin-megna fixed-layout">
   <? require_once("pre_load.php"); ?>
										...
										<div class="col-12 col-sm-12 ">
											<div class="row">
												...
												<div class="col-1 col-sm-1" id="YEAR_DIV" >
													<div class="form-group m-b-40">
														<select id="YEAR" name="YEAR"  class="form-control required-entry" onchange="get_earning_month(this.value)">
															<option value=""></option>
	...
	<script type="text/javascript">
		...
		//DIAM-1815
		function get_earning_month(val){
			jQuery(document).ready(function($) { 
				var data  = 'id='+val;
				var value = $.ajax({
					url: "ajax_get_earning_month",	
					type: "POST",		 
					data: data,		
					async: false,
					cache: false,
