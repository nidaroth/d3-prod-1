<? require_once("../global/config.php"); 
require_once("../language/common.php");
require_once("../language/student.php");
require_once("../language/student_probation.php");

/* Ticket #1066  */
if($_SESSION['PK_ROLES'] == 3){
} else {
	require_once("get_department_from_t.php");
	require_once("check_access.php");

	$ADMISSION_ACCESS 	= check_access('ADMISSION_ACCESS');
	$REGISTRAR_ACCESS 	= check_access('REGISTRAR_ACCESS');
	$FINANCE_ACCESS 	= check_access('FINANCE_ACCESS');
	$ACCOUNTING_ACCESS 	= check_access('ACCOUNTING_ACCESS');
	$PLACEMENT_ACCESS 	= check_access('PLACEMENT_ACCESS');

	if($ADMISSION_ACCESS == 0 && $REGISTRAR_ACCESS == 0 && $FINANCE_ACCESS == 0 && $ACCOUNTING_ACCESS == 0 && $PLACEMENT_ACCESS == 0 ){ 
		header("location:../index");
		exit;
	}
	
	$PK_DEPARTMENT = get_department_from_t($_REQUEST['t']);
}

$search = $_REQUEST['search'];
$sid 	= $_REQUEST['sid'];
$eid 	= $_REQUEST['eid'];
$t 		= $_REQUEST['t'];
$loa_cond = "";
if($search != '')
	$loa_cond .= " AND (REASON like '%$search%' OR S_STUDENT_PROBATION.NOTES like '%$search%' OR PROBATION_TYPE like '%$search%' OR PROBATION_LEVEL like '%$search%' OR PROBATION_STATUS like '%$search%') ";

$s_field = $_REQUEST['field'];
$s_order = $_REQUEST['order'];

if($_REQUEST['field'] == '')
	$_REQUEST['field'] = ' S_TERM_MASTER.BEGIN_DATE DESC ';
	
?>
<table class="table table-hover">
	<thead>
		<tr>
			<? if($s_field == 'BEGIN_DATE_1') {
				if(trim($_REQUEST['order']) == '' || trim($_REQUEST['order']) == 'ASC')
					$s_order = ' DESC '; 
				else
					$s_order = ' ASC ';
			} else 
				$s_order = ' ASC '; ?>
			<th onclick="search_probation('','BEGIN_DATE_1','<?=$s_order?>')" style="cursor: pointer;" ><?=ENROLLMENT?></th>
			
			<? if($s_field == 'PROBATION_BEGIN_DATE') {
				if(trim($_REQUEST['order']) == '' || trim($_REQUEST['order']) == 'ASC')
					$s_order = ' DESC '; 
				else
					$s_order = ' ASC ';
			} else 
				$s_order = ' ASC '; ?>
			<th onclick="search_probation('','PROBATION_BEGIN_DATE','<?=$s_order?>')" style="cursor: pointer;" ><?=BEGIN_DATE?></th>
			
			<? if($s_field == 'PROBATION_END_DATE') {
				if(trim($_REQUEST['order']) == '' || trim($_REQUEST['order']) == 'ASC')
					$s_order = ' DESC '; 
				else
					$s_order = ' ASC ';
			} else 
				$s_order = ' ASC '; ?>
			<th onclick="search_probation('','PROBATION_END_DATE','<?=$s_order?>')" style="cursor: pointer;" ><?=END_DATE?></th>
			
			<? if($s_field == 'PROBATION_TYPE') {
				if(trim($_REQUEST['order']) == '' || trim($_REQUEST['order']) == 'ASC')
					$s_order = ' DESC '; 
				else
					$s_order = ' ASC ';
			} else 
				$s_order = ' ASC '; ?>
			<th onclick="search_probation('','PROBATION_TYPE','<?=$s_order?>')" style="cursor: pointer;" ><?=PROBATION_TYPE?></th>
			
			<? if($s_field == 'PROBATION_LEVEL') {
				if(trim($_REQUEST['order']) == '' || trim($_REQUEST['order']) == 'ASC')
					$s_order = ' DESC '; 
				else
					$s_order = ' ASC ';
			} else 
				$s_order = ' ASC '; ?>
			<th onclick="search_probation('','PROBATION_LEVEL','<?=$s_order?>')" style="cursor: pointer;" ><?=PROBATION_LEVEL?></th>
			
			<? if($s_field == 'PROBATION_STATUS') {
				if(trim($_REQUEST['order']) == '' || trim($_REQUEST['order']) == 'ASC')
					$s_order = ' DESC '; 
				else
					$s_order = ' ASC ';
			} else 
				$s_order = ' ASC '; ?>
			<th onclick="search_probation('','PROBATION_STATUS','<?=$s_order?>')" style="cursor: pointer;" ><?=PROBATION_STATUS?></th>
			
			<? if($s_field == 'REASON') {
				if(trim($_REQUEST['order']) == '' || trim($_REQUEST['order']) == 'ASC')
					$s_order = ' DESC '; 
				else
					$s_order = ' ASC ';
			} else 
				$s_order = ' ASC '; ?>
			<th onclick="search_probation('','REASON','<?=$s_order?>')" style="cursor: pointer;" ><?=REASON?></th>
			
			<? if($s_field == 'NOTES') {
				if(trim($_REQUEST['order']) == '' || trim($_REQUEST['order']) == 'ASC')
					$s_order = ' DESC '; 
				else
					$s_order = ' ASC ';
			} else 
				$s_order = ' ASC '; ?>
			<th onclick="search_probation('','NOTES','<?=$s_order?>')"style="cursor: pointer;"  ><?=NOTES?></th>
			
			<th><?=OPTION?></th>
		</tr>
	</thead>
	
	<tbody>
	<? //AND PK_STUDENT_ENROLLMENT = '$eid'
	//$cond = " AND S_STUDENT_PROBATION.PK_DEPARTMENT = '$PK_DEPARTMENT' ";
	$cond = "";
	$res_type = $db->Execute("select PK_STUDENT_PROBATION,CODE,IF(S_TERM_MASTER.BEGIN_DATE = '0000-00-00','',DATE_FORMAT(S_TERM_MASTER.BEGIN_DATE, '%m/%d/%Y' )) AS BEGIN_DATE_1, S_STUDENT_PROBATION.NOTES, REASON,IF(S_STUDENT_PROBATION.BEGIN_DATE = '0000-00-00','',DATE_FORMAT(S_STUDENT_PROBATION.BEGIN_DATE, '%m/%d/%Y' )) AS PROBATION_BEGIN_DATE ,IF(S_STUDENT_PROBATION.END_DATE = '0000-00-00','',DATE_FORMAT(S_STUDENT_PROBATION.END_DATE, '%m/%d/%Y' )) AS PROBATION_END_DATE, PROBATION_TYPE, PROBATION_LEVEL, PROBATION_STATUS FROM S_STUDENT_PROBATION LEFT JOIN M_PROBATION_TYPE ON M_PROBATION_TYPE.PK_PROBATION_TYPE = S_STUDENT_PROBATION.PK_PROBATION_TYPE LEFT JOIN M_PROBATION_LEVEL ON M_PROBATION_LEVEL.PK_PROBATION_LEVEL = S_STUDENT_PROBATION.PK_PROBATION_LEVEL LEFT JOIN M_PROBATION_STATUS ON M_PROBATION_STATUS.PK_PROBATION_STATUS = S_STUDENT_PROBATION.PK_PROBATION_STATUS LEFT JOIN S_STUDENT_ENROLLMENT ON S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = S_STUDENT_PROBATION.PK_STUDENT_ENROLLMENT LEFT JOIN M_CAMPUS_PROGRAM ON M_CAMPUS_PROGRAM.PK_CAMPUS_PROGRAM =  S_STUDENT_ENROLLMENT.PK_CAMPUS_PROGRAM LEFT JOIN S_TERM_MASTER ON S_TERM_MASTER.PK_TERM_MASTER = S_STUDENT_ENROLLMENT.PK_TERM_MASTER WHERE S_STUDENT_PROBATION.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND S_STUDENT_PROBATION.PK_STUDENT_MASTER = '$sid' $cond $loa_cond ORDER BY $_REQUEST[field] $_REQUEST[order] ");

	while (!$res_type->EOF) { ?>
		<tr <? if($res_type->fields['PK_NOTE_TYPE_MASTER'] == 1 && $res_type->fields['SATISFIED'] == 0) { ?> style="background-color: #F77C7C !important;color: #fff;" <? } ?> >
			<td><?=$res_type->fields['CODE'].'<br />'.$res_type->fields['BEGIN_DATE_1']?></td>
			<td><?=$res_type->fields['PROBATION_BEGIN_DATE']?></td>
			<td><?=$res_type->fields['PROBATION_END_DATE']?></td>
			<td><?=$res_type->fields['PROBATION_TYPE']?></td>
			<td><?=$res_type->fields['PROBATION_LEVEL']?></td>
			<td><?=$res_type->fields['PROBATION_STATUS']?></td>
			<td><?=$res_type->fields['REASON']?></td>
			<td><?=nl2br($res_type->fields['NOTES'])?></td>
			<td>
				<? if($_REQUEST['rd'] == '' && ($REGISTRAR_ACCESS == 2 || $REGISTRAR_ACCESS == 3) ){ ?>
				<a href="student_probation?sid=<?=$sid?>&id=<?=$res_type->fields['PK_STUDENT_PROBATION']?>&t=<?=$_REQUEST['t']?>&eid=<?=$eid?>" title="<?=EDIT?>" class="btn edit-color btn-circle"><i class="far fa-edit"></i> </a>

				<a href="javascript:void(0);" onclick="delete_row('<?=$res_type->fields['PK_STUDENT_PROBATION']?>','PROBATION')" title="<?=DELETE?>" class="btn delete-color btn-circle"><i class="far fa-trash-alt"></i> </a>

				<? } ?>
			</td>
		</tr>
	<?	$res_type->MoveNext();
	} ?>
	</tbody>
</table>