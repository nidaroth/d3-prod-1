		...
		foreach($sheetData as $row ){
			...
			if($PK_COURSE_OFFERING > 0) {
				...
				if($SCHEDULED_CLASS_DATE == ''){
					...
					$SCHEDULED_CLASS_DATE = $year.'/'.$SCHEDULED_CLASS_DATE1[0].'/'.$SCHEDULED_CLASS_DATE1[1];
				}
				if($CLASS_START_TIME == ''){
				...
				if($flag_1 == 1) {
					$res = $db->Execute("select PK_COURSE_OFFERING_SCHEDULE_DETAIL from S_COURSE_OFFERING_SCHEDULE_DETAIL WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING = '$PK_COURSE_OFFERING' AND (DATE_FORMAT(SCHEDULE_DATE,'%Y/%m/%d') = '$SCHEDULED_CLASS_DATE' OR DATE_FORMAT(SCHEDULE_DATE,'%Y/%c/%e') = '$SCHEDULED_CLASS_DATE') AND (DATE_FORMAT(START_TIME,'%h:%i %p') = '$CLASS_START_TIME' OR DATE_FORMAT(START_TIME,'%l:%i %p') = '$CLASS_START_TIME') AND (DATE_FORMAT(END_TIME,'%h:%i %p') = '$CLASS_END_TIME' OR DATE_FORMAT(END_TIME,'%l:%i %p') = '$CLASS_END_TIME')");
					if($res->RecordCount() == 0)
