...
class MYPDF extends TCPDF
{
	...
	public function Header()
	{
									...
									<td style="width:100%" >
										<table border="0" cellspacing="0" cellpadding="3" width="100%" >
											<tr>
												...
												<td style="width:40%">
													<p style="text-align:right;">ID:' . $res_stu->fields['STUDENT_ID'] . '</p>
												</td>
											</tr>
										</table>
									</td> 
		...
		while (!$res_type->EOF) {
			...
			$res_HEADER_DATA = $db->Execute("
									...
									CONCAT(DATE_FORMAT(S_TERM_MASTER.BEGIN_DATE,'%m/%d/%Y' ),' - ',DATE_FORMAT(S_TERM_MASTER.END_DATE,'%m/%d/%Y' )) AS TERM_RANGE, 
									TERM_DESCRIPTION, 
									IF(S_TERM_MASTER.END_DATE = '0000-00-00','',DATE_FORMAT(S_TERM_MASTER.BEGIN_DATE,'%m/%d/%Y' )) AS END_DATE, 
									IF(S_TERM_MASTER.BEGIN_DATE = '0000-00-00','',DATE_FORMAT(S_TERM_MASTER.BEGIN_DATE,'%m/%d/%Y' )) AS BEGIN_DATE, 
									M_ENROLLMENT_STATUS.DESCRIPTION as FULL_PART_TIME, 
									M_FUNDING.FUNDING, IF(EXPECTED_GRAD_DATE = '0000-00-00','',DATE_FORMAT(EXPECTED_GRAD_DATE,'%m/%d/%Y' )) AS EXPECTED_GRAD_DATE, 
									IF(GRADE_DATE = '0000-00-00','',DATE_FORMAT(GRADE_DATE,'%m/%d/%Y' )) AS GRADE_DATE, 
									S_STUDENT_CONTACT.HOME_PHONE, IF(LDA = '0000-00-00','',DATE_FORMAT(LDA,'%m/%d/%Y' )) AS LDA, 
									IF(MIDPOINT_DATE = '0000-00-00','',DATE_FORMAT(MIDPOINT_DATE,'%m/%d/%Y' )) AS MIDPOINT_DATE, 
									M_CAMPUS_PROGRAM.HOURS, M_CAMPUS_PROGRAM.MONTHS, M_CAMPUS_PROGRAM.UNITS, M_CAMPUS_PROGRAM.WEEKS, SESSION, SESSION_ABBREVIATION, SSN as SSN_1, SSN as SSN_2, STUDENT_STATUS, STUDENT_ID, STUDENT_GROUP, M_CAMPUS_PROGRAM.CODE as PROGRAM_CODE, M_CAMPUS_PROGRAM.PROGRAM_TRANSCRIPT_CODE , M_CAMPUS_PROGRAM.DESCRIPTION as PROGRAM_DESCRIPTION, BADGE_ID, S_STUDENT_CONTACT.ADDRESS as STUD_ADDRESS_1, S_STUDENT_CONTACT.ADDRESS_1 as STUD_ADDRESS_2, CONCAT(S_STUDENT_CONTACT.CITY, ', ', STATE_CODE, ' - ', S_STUDENT_CONTACT.ZIP) as STUD_CITY_STATE_ZIP   ,
	...
	WHERE S_STUDENT_MASTER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT'  $order_by");
			...
			if ($res_HEADER_DATA->fields['DROP_DATE'] != '0000-00-00' && $res_HEADER_DATA->fields['DROP_DATE'] != '') {
				$STR_GRAD_DATE = "Drop Date : " . $res_HEADER_DATA->fields['DROP_DATE'];
			}
			if ($res_HEADER_DATA->fields['DETERMINATION_DATE'] != '0000-00-00' && $res_HEADER_DATA->fields['DETERMINATION_DATE'] != '') {
				$STR_GRAD_DATE = "Determination : " . $res_HEADER_DATA->fields['DETERMINATION_DATE'];
			}
			// if($res_HEADER_DATA->fields['LDA'] != '0000-00-00' && $res_HEADER_DATA->fields['LDA'] != ''){
			// 	$STR_GRAD_DATE = "LDA : ".$res_HEADER_DATA->fields['LDA'];
			// } 
			if ($res_HEADER_DATA->fields['GRADE_DATE'] != '0000-00-00' && $res_HEADER_DATA->fields['GRADE_DATE'] != '') {
				$STR_GRAD_DATE = "Grad Date : " . $res_HEADER_DATA->fields['GRADE_DATE'];
			}
...
function student_transcript_list_pdf($PK_STUDENT_MASTERS, $one_stud_per_pdf, $report_name)
{
	...
	foreach ($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
		...
		if ($_GET['eid'] != '') {
			$en_cond = " AND PK_STUDENT_ENROLLMENT = $_GET[eid] ";
		...
		// dd($_GET[eid]  , $en_cond ,$PK_STUDENT_ENROLLMENTS_OF_CURRENT_PK_STUDENT_MASTER);
		// $extra_enrollment_offset = count(explode(',' , $PK_STUDENT_ENROLLMENTS_OF_CURRENT_PK_STUDENT_MASTER)) -1;
		$extra_enrollment_offset = 0;
...
if ($_GET['id'] == '') {
	...
	$report_name = "Official Academic Transcript";
	...
	if ($_GET['zip'] == 1) {
		...
		foreach ($exploded_865 as $exploded_std) {
			...
			// dump($_GET , $_SESSION);
			// dd($exploded_std);
			$enrollements = StudentEnrollment::where('PK_STUDENT_MASTER', $exploded_std)->get();

			foreach ($enrollements as $enrollement) {
				// dump($enrollement);
				$_GET['eid'] = $enrollement->PK_STUDENT_ENROLLMENT;
				// dump($enrollement->PK_STUDENT_ENROLLMENT);
