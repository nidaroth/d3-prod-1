...
function pdf_custom_header($PK_STUDENT_MASTER, $PK_STUDENT_ENROLLMENT, $type){
	...
	$EMAIL ='';
	if(!empty($res_pdf_header->fields['EMAIL']))
		$EMAIL = '<br />'.$res_pdf_header->fields['EMAIL'];


	$CONTENT = '<table border="0" cellspacing="0" cellpadding="3" width="100%" >
					<tr>
					<td style="width:'.$logo_width.'%" ></td>
						<td colspan="" style="font-size:'.$school_font.'" ><i>'.$res_pdf_header->fields['NAME'].'</i></td>
					</tr>
					<tr>
						<td style="width:'.$logo_width.'%" >'.$LOGO.'</td>
						<td style="width:'.$address_width.'%" >
							<span style="line-height:5px;'.$address_font.'" >'.$res_pdf_header->fields['ADDRESS'].' '.$res_pdf_header->fields['ADDRESS_1'].'<br />'.$res_pdf_header->fields['CITY'].' '.$res_pdf_header->fields['STATE_CODE'].' '.$res_pdf_header->fields['ZIP'].'<br />'.$res_pdf_header->fields['PHONE'].$EMAIL.'<br />'.$res_pdf_header->fields['WEBSITE'].'</span>
						</td>
					</tr>
				</table>';
