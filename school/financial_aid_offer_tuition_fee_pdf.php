    ...
    public function Header() {
		...
		if($_GET['id'] != ''){
			if($this->PageNo() > 0) {
				...
				$CONTENT = '<b>'.str_replace("<br /><br />", "<br />", $CONTENT).'</b>'; //DAIM-1163
		...
		$this->SetX(137);
		$this->SetTextColor(000, 000, 000);
		$this->Cell(65, 8, "Cost of Attendance (COA)", 0, false, 'R', 0, '', 0, false, 'M', 'L');//DIAM-1163
...
// $en_cond = "";
// if($_GET['en'] != '')
// 	$en_cond = " AND PK_STUDENT_ENROLLMENT IN ($_GET[en]) ";
...
$txt = '<table border="0" cellspacing="0" cellpadding="3" width="100%" style="font-size:30px;">
		...
		$res_prog_fee_tot = $db->Execute("select SUM(FEE_AMOUNT) AS FEE_AMOUNT from S_STUDENT_FEE_BUDGET WHERE PK_STUDENT_MASTER = '$_GET[id]' AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_FEE_TYPE = 2 AND PK_ESTIMATE_FEE_STATUS IN (1,2,3) $ay_cond $enroll_cond1 $ap_cond "); //DIAM-1163
		
		if($_GET['type'] == 1) {
			$txt .='<tr>
						<td width="65%" style="border-bottom:3px solid #a5a5a5;border-top:3px solid #a5a5a5;"><b><br />Cost of Attendance (COA)</b></td>
						<td width="35%" style="text-align:right; border-bottom:3px solid #a5a5a5;border-top:3px solid #a5a5a5;" ><b><br />Amount</b></td>
					</tr>';//DIAM-1163
			
			$res_prog_fee_tot1 = $db->Execute("select SUM(FEE_AMOUNT) AS FEE_AMOUNT, CONCAT(M_AR_LEDGER_CODE.CODE, ' - ', M_AR_LEDGER_CODE.LEDGER_DESCRIPTION),M_AR_LEDGER_CODE.LEDGER_DESCRIPTION AS LEDGER from S_STUDENT_FEE_BUDGET LEFT JOIN M_AR_LEDGER_CODE ON M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE = S_STUDENT_FEE_BUDGET.PK_AR_LEDGER_CODE WHERE PK_STUDENT_MASTER = '$_GET[id]' AND S_STUDENT_FEE_BUDGET.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_FEE_TYPE = 2 AND PK_ESTIMATE_FEE_STATUS IN (1,2,3) $ay_cond $enroll_cond1 $ap_cond GROUP BY S_STUDENT_FEE_BUDGET.PK_AR_LEDGER_CODE   ORDER BY LEDGER ASC "); //DIAM-1163
			while (!$res_prog_fee_tot1->EOF) { 
				$txt .= '<tr>
							<td width="65%" >'.$res_prog_fee_tot1->fields['LEDGER'].'</td>
							<td width="35%" style="text-align:right;" >'.$res_prog_fee_tot1->fields['FEE_AMOUNT'].'</td>
						</tr>';
					
				$res_prog_fee_tot1->MoveNext();
			}
			//DIAM-1163
			$txt .='<tr>
						<td width="65%" style="border-top:0.5px solid #000;" ><b>Total Cost of Attendance (COA)<br /></b></td>
						<td width="35%" style="text-align:right;border-top:0.5px solid #000;" ><b>$'.number_format_value_checker($res_prog_fee_tot->fields['FEE_AMOUNT'],2).'</b></td>
					</tr>';
			
		} else if($_GET['type'] == 2) {
			/*$txt .='<tr>
						<td width="65%" style="border-bottom:0.5px solid #000;" ><b><br />Cost of Attendance (COA)<br /></b></td>
						<td width="15%" style="text-align:right;border-bottom:0.5px solid #000;" ><b><br />$'.number_format_value_checker($res_prog_fee_tot->fields['FEE_AMOUNT'],2).'<br /></b></td>
					</tr>';*/ //DIAM-1163
		...
		$txt .='<tr>
				<td width="65%" style="border-bottom:3px solid #a5a5a5;border-top:3px solid #a5a5a5;"><b><br />Award</b></td>
				<td width="35%" style="text-align:right;border-bottom:3px solid #a5a5a5;border-top:3px solid #a5a5a5;" ><b><br />Amount</b></td>
			</tr>';
			
			$total = 0;
			//$res_prog_fee = $db->Execute("select SUM(FEE_AMOUNT) AS FEE_AMOUNT, CONCAT(M_AR_LEDGER_CODE.CODE, ' - ', M_AR_LEDGER_CODE.LEDGER_DESCRIPTION) AS LEDGER, S_STUDENT_FEE_BUDGET.PK_AR_LEDGER_CODE from S_STUDENT_FEE_BUDGET LEFT JOIN M_AR_LEDGER_CODE ON M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE = S_STUDENT_FEE_BUDGET.PK_AR_LEDGER_CODE WHERE PK_STUDENT_MASTER = '$_GET[id]' AND S_STUDENT_FEE_BUDGET.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' $ay_cond AND PK_FEE_TYPE = 2 AND AWARD_LETTER = 1 GROUP BY S_STUDENT_FEE_BUDGET.PK_AR_LEDGER_CODE ");
			
			//DIAM-1163
			$res_prog_fee = $db->Execute("select SUM(DISBURSEMENT_AMOUNT) AS FEE_AMOUNT, IF(DISBURSEMENT_DATE = '0000-00-00','', DATE_FORMAT( DISBURSEMENT_DATE, '%m/%d/%Y' )) AS DISBURSEMENT_DATE_1, CONCAT(M_AR_LEDGER_CODE.CODE, ' - ', M_AR_LEDGER_CODE.LEDGER_DESCRIPTION) AS LEDGER, S_STUDENT_DISBURSEMENT.PK_AR_LEDGER_CODE from S_STUDENT_DISBURSEMENT LEFT JOIN M_AR_LEDGER_CODE ON M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE = S_STUDENT_DISBURSEMENT.PK_AR_LEDGER_CODE WHERE PK_STUDENT_MASTER = '$_GET[id]' AND S_STUDENT_DISBURSEMENT.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' $ay_cond $ap_cond $enroll_cond1 AND PK_DISBURSEMENT_STATUS IN (1,2,3,4) AND AWARD_LETTER = 1 GROUP BY S_STUDENT_DISBURSEMENT.PK_AR_LEDGER_CODE  ORDER BY LEDGER ASC,DISBURSEMENT_DATE ASC");
			while (!$res_prog_fee->EOF) { 
				$total += $res_prog_fee->fields['FEE_AMOUNT'];
				$txt .='<tr>
							<td width="65%" >'.$res_prog_fee->fields['LEDGER'].'</td>
							<td width="35%" style="text-align:right;" >$'.number_format_value_checker($res_prog_fee->fields['FEE_AMOUNT'],2).'</td>
						</tr>';
				$res_prog_fee->MoveNext();
			}
			
		$txt .= '<tr>
					<td width="65%" style="border-top:0.5px solid #000;"><b>Total Awards</b></td>
					<td width="35%" style="border-top:0.5px solid #000;text-align:right;" ><b>$'.number_format_value_checker($total,2).'</b></td>
				</tr>';
		...
		$txt .= '<tr>
				...
				<tr>
					<td width="65%" style="border-top:0.5px solid #000;border-bottom:0.5px solid #000;"><b>Total Cost of Attendance (COA) - Total Awards</b></td>
					<td width="35%" style="border-top:0.5px solid #000;border-bottom:0.5px solid #000;text-align:right;" ><b>$'.number_format_value_checker(($res_prog_fee_tot->fields['FEE_AMOUNT'] - $total),2).'</b></td>
				</tr>';//DIAM-1163
	...
	$txt .= '<table width="100%"><tr><td style="white-space: pre-wrap;text-align: justify;" width="100%">'.nl2br($res_acc->fields['CONTENT']).'</td></tr></table>';
	/* Ticket # 1234 */
