...
if(!empty($_POST)){
	if($_POST['PK_STUDENT_MASTER'] != '') {
		...
		$PK_TERM_MASTER = implode(",",$_POST['COURSE_PK_TERM_MASTER']); //Ticket # 1786 
		$_SESSION['REP_SEARCH_PK_TERM_MASTER'] = $PK_TERM_MASTER;
		
		/* Ticket # 1875 */
		if($_POST['REPORT_TYPE'] == 1)
			header("location:student_schedule_pdf?id=".$PK_STUDENT_MASTER."&t_id_from_ses=1");
		else
			header("location:student_schedule_with_books_pdf?id=".$PK_STUDENT_MASTER."&t_id_from_ses=1");
		/* Ticket # 1875 */
			
		//header("location:student_schedule_pdf?id=".$PK_STUDENT_MASTER);
		/* Ticket # 1214 */
...
<head>
	...
	<title><?=MNU_STUDENT_SCHEDULE ?> | <?=$title?></title>
	<style>
		li > a > label{position: unset !important;}
		#advice-required-entry-COURSE_PK_TERM_MASTER{position: absolute;top: 38px;} /* Ticket # 1786  */
		.dropdown-menu>li>a { white-space: nowrap; } /* Ticket # 1786 */
...
<body class="horizontal-nav boxed skin-megna fixed-layout">
   <? require_once("pre_load.php"); ?>
            ...
            <div class="container-fluid">
                 <div class="row page-titles">
                    <div class="col-md-12 align-self-center">
                        <h4 class="text-themecolor"><?=MNU_STUDENT_SCHEDULE?></h4>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
									...
									<div class="row" style="padding-bottom:10px;" >
										...
										<div class="col-md-2">
											...
											<select id="REPORT_TYPE" name="REPORT_TYPE"  class="form-control" >
												<option value="1" selected >Student Schedule</option>
												<option value="2" >Student Schedule with Books</option>
											</select>
										</div>
										...
										<div class="col-md-2 ">
											<!-- Ticket # 1786 -->
											<br />
											<select id="COURSE_PK_TERM_MASTER" name="COURSE_PK_TERM_MASTER[]" multiple  class="form-control required-entry" onchange="search()" >
	...
	<script type="text/javascript">
		...
		function search(){
			jQuery(document).ready(function($) {
				var data  = 'PK_STUDENT_GROUP='+$('#PK_STUDENT_GROUP').val()+'&PK_TERM_MASTER='+$('#PK_TERM_MASTER').val()+'&PK_CAMPUS_PROGRAM='+$('#PK_CAMPUS_PROGRAM').val()+'&PK_STUDENT_STATUS='+$('#PK_STUDENT_STATUS').val()+'&PK_CAMPUS='+$('#PK_CAMPUS').val()+'&show_check=1&show_count=1&COURSE_PK_TERM_MASTER='+$('#COURSE_PK_TERM_MASTER').val(); //Ticket # 1552; //Ticket # 1214
				var value = $.ajax({
