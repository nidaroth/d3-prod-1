...
$file_name 		= 'Student Invoice.xlsx';
...
$group_by = " GROUP BY S_STUDENT_DISBURSEMENT.PK_STUDENT_ENROLLMENT ";
...
foreach($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
	...
	//echo $sql_query="select S_STUDENT_DISBURSEMENT.PK_STUDENT_ENROLLMENT, S_STUDENT_DISBURSEMENT.PK_STUDENT_DISBURSEMENT, DISBURSEMENT_AMOUNT, IF(DISBURSEMENT_DATE != '0000-00-00', DATE_FORMAT(DISBURSEMENT_DATE,'%Y-%m-%d'),'') AS  DISBURSEMENT_DATE, CODE, INVOICE_DESCRIPTION from S_STUDENT_DISBURSEMENT, M_AR_LEDGER_CODE WHERE S_STUDENT_DISBURSEMENT.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND PK_PAYMENT_BATCH_DETAIL = 0 AND PK_DISBURSEMENT_STATUS IN (2) AND INVOICE = 1  AND S_STUDENT_DISBURSEMENT.PK_AR_LEDGER_CODE = M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE $cond ORDER BY DISBURSEMENT_DATE ASC";
	//exit;
	$log_array=array('SQL_QUERY'=>$sql_query,'PK_ACCOUNT'=>$_SESSION['PK_ACCOUNT'],'PK_USER'=>$_SESSION['PK_USER'],'REPORT_NAME'=>'Student Invoice Excel Report');
	log_query($log_array);
	// End store query in table
	$res_ledger = $db->Execute("SELECT S_STUDENT_DISBURSEMENT.PK_STUDENT_ENROLLMENT, S_STUDENT_DISBURSEMENT.PK_STUDENT_DISBURSEMENT, DISBURSEMENT_AMOUNT, IF(DISBURSEMENT_DATE != '0000-00-00', DATE_FORMAT(DISBURSEMENT_DATE,'%Y-%m-%d'),'') AS  DISBURSEMENT_DATE, CODE, INVOICE_DESCRIPTION 
	from S_STUDENT_DISBURSEMENT, M_AR_LEDGER_CODE 
	WHERE S_STUDENT_DISBURSEMENT.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' 
	AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' 
	AND PK_PAYMENT_BATCH_DETAIL = 0 
	AND PK_DISBURSEMENT_STATUS=2 AND INVOICE = 1  
	AND S_STUDENT_DISBURSEMENT.PK_AR_LEDGER_CODE = M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE $cond $group_by ORDER BY DISBURSEMENT_DATE ASC");
	...
	while (!$res_ledger->EOF) {
		...
		$res_campus = $db->Execute("SELECT OFFICIAL_CAMPUS_NAME FROM S_STUDENT_CAMPUS, S_CAMPUS WHERE PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT' AND S_CAMPUS.PK_CAMPUS = S_STUDENT_CAMPUS.PK_CAMPUS  $campus_cond1 ");
		...
		$index++;
		$cell_no = $cell[$index].$line;
		$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($res_campus->fields['OFFICIAL_CAMPUS_NAME']);
		...
		$index++;
		$cell_no = $cell[$index].$line;
		$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($res_ledger->fields['DISBURSEMENT_DATE']);
