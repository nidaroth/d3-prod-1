...
// Edited By 
$ress = $db->Execute("SELECT EDITED_ON, EDITED_BY FROM NYOEDS_SETUP WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
if ($ress->RecordCount() == 0) {
    header("location:oeds_report_setup");
    exit;
}
$EDITED_ON_1        = '';
...
$res = $db->Execute("SELECT OEDS FROM Z_ACCOUNT_REPORTS WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' "); 
if($res->fields['OEDS'] == 0 || $res->fields['OEDS'] == '') {
	header("location:../index");
	exit;
}
...
if(!empty($_POST))
{
  ...
  //Begin Reports 
    ...
    //Admission     
    include('oeds_admission_report_excel.php');

    }else if($REPORT_OPTION == 2){

    //Enrollment 
    include('oeds_enrollment_report_excel.php');
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">    
                                                <div class="button-adjustment">
                                                    <button type="button" onclick="window.location.href='oeds_report_setup'" class="btn waves-effect waves-light btn-info">Report Setup</button>
                                                ...
                                                <div id="report_option_div"> 
                                                    ...
                                                    <select id="REPORT_OPTION" name="REPORT_OPTION" onchange="ReportFilters(this.value)" class="form-control">
                                                        <?php
                                                         ...
                                                         $ro_selected="";
                                                         ?>
                                                        <option value="1" <?=$ro_selected?>>Admissions</option>
                                                        <option value="2"  <?=$ro_selected?>>Enrollment, Outcome Status, Graduation Employment, Financial Assistance</option>
                                                        <!-- <option value="3"  <?=$ro_selected?>>Form A Section 1 (PDF)</option>
                                                        <option value="4"  <?=$ro_selected?>>Form A Section 2 (PDF)</option>
                                                        <option value="5"  <?=$ro_selected?>>Form B Section 1 & 2 (PDF)</option>
                                                        <option value="6"  <?=$ro_selected?>>Setup Review (PDF)</option> -->
