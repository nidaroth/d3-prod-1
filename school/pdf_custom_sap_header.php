...
function pdf_custom_header($PK_STUDENT_MASTER, $PK_STUDENT_ENROLLMENT, $type){
	...
	$account_array=array('15','99'); //for 15 & 99 FOCUS ACC
	if(in_array($_SESSION['PK_ACCOUNT'],$account_array) && in_array($actual_URL, $domain_name)){
		if($res_pdf_header->fields['HIDE_ACCOUNT_ADDRESS_ON_REPORTS']==0){
			$original_text = '**Original Record**';
			$school_reg_email ='registrar@fpti.edu';
		}
	}		
	$CONTENT = '<table border="0" cellspacing="0" cellpadding="3" width="100%" >
					<tr>
						<td colspan="2" style="font-size:'.$school_font.'" ><b><i>'.$res_pdf_header->fields['NAME'].'</i></b></td>
					</tr>
					<tr>
						<td style=" line-height:5px;width:'.$logo_width.'%" >'.$LOGO.'</td>
						<td style="width:'.$address_width.'%;font-size:'.$address_font.'" >
							<span style="line-height:5px;" > <br /> '.$original_text.' <br />'.$res_pdf_header->fields['ADDRESS'].' '.$res_pdf_header->fields['ADDRESS_1'].'<br />'.$res_pdf_header->fields['CITY'].' '.$res_pdf_header->fields['STATE_CODE'].' '.$res_pdf_header->fields['ZIP'].'<br /><br />'.$res_pdf_header->fields['PHONE'].'<br /><br />'.$school_reg_email.'<br /><br />'.$res_pdf_header->fields['WEBSITE'].'</span>
						</td>
					</tr>
				</table>';
