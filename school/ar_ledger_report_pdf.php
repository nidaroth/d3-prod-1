...
function formatPhoneNumber($phone)
{
	...
	return $phonedata[0] . ':' . $phoneNumber;
}

//specific site condtion
if (has_custom_sap_report($_SESSION['PK_ACCOUNT'])) {
	require_once("pdf_custom_ledger_header.php"); //DIAM-944
	$student_ledger = 'AR Student Ledger Detail'; //DIAM-944

	// exit;
} else {
	require_once("pdf_custom_header.php"); //Ticket # 1588
	$student_ledger = 'Student Ledger'; //DIAM-944
}
class MYPDF extends TCPDF
{
	public function Header()
	{
		global $db, $student_ledger; //DIAM-944
		...
		if (has_custom_sap_report($_SESSION['PK_ACCOUNT'])) { //DIAM-944		
			$this->SetFont('helvetica', 'I', 18);
			$this->SetY(6);
			$this->SetTextColor(000, 000, 000);
			$this->SetX(155);
			$this->MultiCell(40, 8, $student_ledger, 0, 'R', 0, 0, '', '', true, null, true); //DIAM-944	
			...
			$this->SetTextColor(000, 000, 000);
			$this->SetX(155);
			$this->Cell(55, 8, $student_ledger, 0, false, 'L', 0, '', 0, false, 'M', 'L');
		} //DIAM-944		
...
$_SESSION['temp_id'] = '';
function student_ledger($PK_STUDENT_MASTERS, $PK_STUDENT_ENROLLMENT, $one_stud_per_pdf)
{
	global $db;
	...
	$REPORT_ALL_ENROLLMENT = false;
	if ($PK_STUDENT_ENROLLMENT == '') {
		$REPORT_ALL_ENROLLMENT = true;
	}
	...
	foreach ($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
		...
		if ($PK_STUDENT_ENROLLMENT != '' && $REPORT_ALL_ENROLLMENT != true)
			$led_cond = " AND S_STUDENT_LEDGER.PK_STUDENT_ENROLLMENT IN ($PK_STUDENT_ENROLLMENT) ";

		if ($REPORT_ALL_ENROLLMENT == true) {
			$res = $db->Execute("SELECT PK_STUDENT_ENROLLMENT FROM S_STUDENT_ENROLLMENT WHERE PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND IS_ACTIVE_ENROLLMENT = 1");
			$PK_STUDENT_ENROLLMENT = $res->fields['PK_STUDENT_ENROLLMENT'];
		}
		...
		$pdf->startPageGroup();
		$pdf->AddPage();
		if (has_custom_sap_report($_SESSION['PK_ACCOUNT'])) { //DIAM-944	PK acc 99 FOCUS
			...
			$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="80%">
				...
				<tr>
					<td width="100%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;border-right:0.5px solid #000;" >' . transcript_header("PROGRAM_CODE_DESCRIPTION", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . '</td>
				</tr>
				<tr>
					<td style="border-left:0.5px solid #000;border-top:0.5px solid #000" width="100%" width="34%" >
						' . str_replace("Student ", "", transcript_header("STUDENT_ID", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ")) . '
					</td>
					<td style="border-left:0.5px solid #000;border-top:0.5px solid #000" width="100%" width="34%" >
						' . transcript_header("STUDENT_STATUS", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . '
					</td>
					<td style="border-left:0.5px solid #000;border-right:0.5px solid #000;border-top:0.5px solid #000" width="100%" width="32%" >
						' . transcript_header("STUD_ADDRESS_1", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . ' ' . transcript_header("STUD_ADDRESS_2", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . '
					</td>
				</tr>
				<tr>
					<td width="34%" style="border-left:0.5px solid #000;" >
						' . transcript_header("DATE_OF_BIRTH", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . '
					</td>
					<td  width="34%" style="border-left:0.5px solid #000;" >
						' . transcript_header("BEGIN_DATE", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . '
					</td>
					<td  width="32%" style="border-left:0.5px solid #000;border-right:0.5px solid #000;" >
						' . str_replace("- ", " ", transcript_header("STUD_CITY_STATE_ZIP", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ")) . '
					</td>
				</tr>
				<tr>
					<td width="34%" style="border-left:0.5px solid #000;" >
						' . formatPhoneNumber(transcript_header("HOME_PHONE", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ")) . '
					</td>
					<td  width="34%" style="border-left:0.5px solid #000;" >
						' . transcript_header("GRADE_DATE", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . '
					</td>
					<td  width="32%" style="border-left:0.5px solid #000;border-right:0.5px solid #000;" ></td>
				</tr>
				<tr>
					<td width="34%" style="border-left:0.5px solid #000;border-bottom:0.5px solid #000" >
						' . transcript_header("LDA", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . '
					</td>
					<td  width="34%" style="border-left:0.5px solid #000;border-bottom:0.5px solid #000" >
						' . transcript_header("FUNDING", " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT2' ") . '
					</td>
					<td  width="32%" style="border-left:0.5px solid #000;border-right:0.5px solid #000;border-bottom:0.5px solid #000" width="100%" width="32%"  ></td>
				</tr>				
			</table>';
		} else { //DIAM-944		
		...
		if ($_GET['do'] == 1) {
			...
			$do_2_order = ' ACTUAL_AY , ACTUAL_AP , ';
		...
		/* Ticket # 1208 */
		$query_new = "SELECT 
						COALESCE(ACADEMIC_YEAR, S_TUITION_BATCH_MASTER.AY, S_MISC_BATCH_DETAIL.AY) AS ACTUAL_AY,
						COALESCE(ACADEMIC_PERIOD, S_TUITION_BATCH_MASTER.AP, S_MISC_BATCH_DETAIL.AP) AS ACTUAL_AP,
						PK_STUDENT_LEDGER, 
						LEDGER_DESCRIPTION, 
						IF(S_STUDENT_LEDGER.TRANSACTION_DATE = '0000-00-00', '', 
							DATE_FORMAT(
								S_STUDENT_LEDGER.TRANSACTION_DATE, 
								'%m/%d/%Y') 
						) AS TRANSACTION_DATE_1, 
						S_STUDENT_LEDGER.CREDIT, 
						S_STUDENT_LEDGER.DEBIT, 
						M_AR_LEDGER_CODE.CODE, 
						RECEIPT_NO, 
						CHECK_NO, 
						IF(
							S_STUDENT_LEDGER.PK_PAYMENT_BATCH_DETAIL > 0, 
							S_PAYMENT_BATCH_DETAIL.BATCH_DETAIL_DESCRIPTION, 
							IF(
								S_STUDENT_LEDGER.PK_MISC_BATCH_DETAIL > 0, 
								S_MISC_BATCH_DETAIL.BATCH_DETAIL_DESCRIPTION, 
								IF
								(
								S_STUDENT_LEDGER.PK_TUITION_BATCH_DETAIL > 0, 
								S_TUITION_BATCH_DETAIL.BATCH_DETAIL_DESCRIPTION, 
								''
								)
							)
						) AS BATCH_DESCRIPTION, 
						AWARD_YEAR, 
						S_STUDENT_LEDGER.PK_PAYMENT_BATCH_DETAIL, 
						S_STUDENT_LEDGER.PK_TUITION_BATCH_DETAIL, 
						S_STUDENT_LEDGER.PK_MISC_BATCH_DETAIL, 
						GROSS_AMOUNT, 
						FEE_AMOUNT 
					FROM 
						S_STUDENT_LEDGER 
						...
						LEFT JOIN S_TUITION_BATCH_MASTER ON S_TUITION_BATCH_MASTER.PK_TUITION_BATCH_MASTER = S_TUITION_BATCH_DETAIL.PK_TUITION_BATCH_MASTER
						LEFT JOIN S_MISC_BATCH_MASTER ON S_MISC_BATCH_MASTER.PK_MISC_BATCH_MASTER = S_MISC_BATCH_DETAIL.PK_MISC_BATCH_MASTER 
						LEFT JOIN M_AR_LEDGER_CODE On M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE = S_STUDENT_LEDGER.PK_AR_LEDGER_CODE 
					WHERE 
						S_STUDENT_LEDGER.PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' 
						AND S_STUDENT_LEDGER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' 
						AND (
						S_STUDENT_LEDGER.PK_PAYMENT_BATCH_DETAIL > 0 
						OR S_STUDENT_LEDGER.PK_MISC_BATCH_DETAIL > 0 
						OR S_STUDENT_LEDGER.PK_TUITION_BATCH_DETAIL > 0
						) $led_cond 
					ORDER BY 
					$do_2_order	S_STUDENT_LEDGER.TRANSACTION_DATE ASC ";
		$res_ledger = $db->Execute($query_new);
		// echo $query_new;exit;
		/* Ticket # 1208 */
		...
		while (!$res_ledger->EOF) {
			...
			//Ticket # 1208 
			if ($_GET['desc_type'] == 1)
				$DESC = $res_ledger->fields['BATCH_DESCRIPTION'];
			else
			...
			$txt .= '<td width="10%" align="right">$ ' . number_format_value_checker($res_ledger->fields['DEBIT'], 2) . '</td>
							<td width="10%" align="right">$ ' . number_format_value_checker($res_ledger->fields['CREDIT'], 2) . '</td>
	...
	if ($one_stud_per_pdf == 0) {
		$file_name  = 'Student Ledger Report.pdf';
...
if ($_GET['id'] == '') {
	...
	student_ledger($PK_STUDENT_MASTERS, $PK_STUDENT_ENROLLMENT, 0);
} else {
	if ($_GET['t'] == 2) {
		...
		// $folder = '../backend_assets/school/school_'.$_SESSION['PK_ACCOUNT'].'/other/student_ledger';
		$folder = '../backend_assets/tmp_upload/student_ledger';
		...
		if ($res === TRUE) {
			...
			foreach ($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
				$file_name_1 = student_ledger($PK_STUDENT_MASTER, '', 1);
		...
		student_ledger($_GET['id'], $_GET['eid'], 0);
}
