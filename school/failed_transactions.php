	...
	if($_POST['FORMAT'] == 1) {
		...
		class MYPDF extends TCPDF {
			public function Header() {
				...
				$this->SetX(130);
				$this->SetTextColor(000, 000, 000);
				$this->Cell(55, 8, "Failed Transactions", 0, false, 'L', 0, '', 0, false, 'M', 'L');
		...
		$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%">
					<thead>
						<tr>
							...
							<td width="10%" style="border-top:1px solid #000;border-bottom:1px solid #000;" >Payment Date</td>
							<td width="10%" style="border-top:1px solid #000;border-bottom:1px solid #000;" >Name on Card</td>
							<td width="10%" style="border-top:1px solid #000;border-bottom:1px solid #000;" >Last 4 of CC</td>
							<td width="10%" style="border-top:1px solid #000;border-bottom:1px solid #000;" align="right" >Amount</td>
							<td width="27%" style="border-top:1px solid #000;border-bottom:1px solid #000;" >Message</td>
						</tr>
					</thead>';
		...
		while (!$res->EOF) {
			...
			$txt .= '<tr>
						...
						<td width="10%" >'.$res->fields['PAID_ON_1'].'</td>
						<td width="10%" >'.$res->fields['CARD_NAME'].'</td>
						<td width="10%" >'.$CARD_NO.'</td>
						<td width="10%" align="right" >$ '.number_format_value_checker($res->fields['AMOUNT_CHARGED'],2).'</td>
						<td width="27%" >'.$res->fields['FAILED_MESSAGE'].'</td>
					</tr>';
		...
		$file_name = 'Failed Transactions.pdf';
		...
		$file_name 		= "Failed Transactions.xlsx";
		$dir 			= 'temp/';
...
$outputFileName = str_replace(
	...
	$outputFileName );  
		...
		while (!$res->EOF) {
			...
			$index++;
			$cell_no = $cell[$index].$line;
			$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($res->fields['CARD_NAME']);
			...
			$index++;
			$cell_no = $cell[$index].$line;
			$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($res->fields['FAILED_MESSAGE']);
...
<head>
	...
	<title><?=MNU_FAILED_TRANSACTIONS ?> | <?=$title?></title>
	<style>
            ...
            <div class="container-fluid">
                 <div class="row page-titles">
                    <div class="col-md-12 align-self-center">
						...
						<? echo MNU_FAILED_TRANSACTIONS ?> </h4>
                    </div>
                </div>
							...
							<div class="card">
								<div class="card-body">
									...
									<div class="row" >
										...
										<div class="col-md-2 ">
											<?=LAST_4_CC?>
											<input type="text" class="form-control" id="LAST_4_CC" name="LAST_4_CC" value="" >
										</div>
										<div class="col-md-2 ">
											<?=NAME_ON_CARD?>
		...
		function do_search(){
			jQuery(document).ready(function($) { 
				...
				if(result == true){ 
					...
					var data  = 'START_DATE='+START_DATE+'&END_DATE='+END_DATE+'&AMOUNT='+AMOUNT+'&STUDENT='+STUDENT+'&NAME_ON_CARD='+NAME_ON_CARD+'&PK_CAMPUS='+PK_CAMPUS+'&LAST_4_CC='+LAST_4_CC;;
					var value = $.ajax({
						url: "ajax_failed_transactions",
						type: "POST",		 
						data: data,		
						async: false,
						cache: false,
