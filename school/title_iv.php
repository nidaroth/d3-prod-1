...
if(!empty($_POST)){
	...
	, S_STUDENT_LEDGER, M_AR_LEDGER_CODE, S_STUDENT_CAMPUS 
	...
	if($_POST['FORMAT'] == 1){
		...
		class MYPDF extends TCPDF {
			public function Header() {
				...
				$this->SetX(146);
				$this->SetTextColor(000, 000, 000);
				$this->Cell(55, 8, "Title IV Recipients", 0, false, 'L', 0, '', 0, false, 'M', 'L');
		...
		$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%">
					<thead>
						<tr>
							<td width="15%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br />Student</td>
							<td width="10%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br />SSN</td>
							<td width="8%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br />Student ID</td>
							<td width="9%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br />First Term</td>
							<td width="13%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br />Program</td>
							<td width="10%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br />Status</td>
							<td width="9%" style="border-top:1px solid #000;border-bottom:1px solid #000;" >Expected<br />Grad Date</td>
							<td width="9%" style="border-top:1px solid #000;border-bottom:1px solid #000;" >Determination<br />Date</td>
							<td width="9%" style="border-top:1px solid #000;border-bottom:1px solid #000;" >Grad<br />Date</td>
							<td width="9%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br />LDA</td>
						</tr>
					</thead>';
		...
		while (!$res_ledger->EOF) {
			...
			$txt 	.= '<tr>
						<td width="15%" >'.$sno.'. '.$res_ledger->fields['NAME'].'</td>
						<td width="10%" >'.$SSN_DE.'</td>
						<td width="8%" >'.$res_ledger->fields['STUDENT_ID'].'</td>
						<td width="9%" >'.$res_ledger->fields['BEGIN_DATE_1'].'</td>
						<td width="13%" >'.$res_ledger->fields['PROGRAM_CODE'].'</td>
						
						<td width="10%" >'.$res_ledger->fields['STUDENT_STATUS'].'</td>
						<td width="9%" >'.$res_ledger->fields['EXPECTED_GRAD_DATE'].'</td>
						<td width="9%" >'.$res_ledger->fields['DETERMINATION_DATE'].'</td>
						<td width="9%" >'.$res_ledger->fields['GRADE_DATE'].'</td>
						<td width="9%" >'.$res_ledger->fields['LDA'].'</td>
					</tr>';
		...
		$file_name = 'Title IV Recipients.pdf';
		/*if($browser == 'Safari')
		...
		$file_name 		= 'Title IV Recipients.xlsx';
...
<head>
	...
	<title><?=MNU_TITLE_IV_RECIPIENTS ?> | <?=$title?></title>
	<style>
