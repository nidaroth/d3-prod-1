...
if(!empty($_POST)){
   ...
   INNER JOIN S_STUDENT_LEDGER ON S_STUDENT_LEDGER.PK_STUDENT_ENROLLMENT = S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT
   INNER JOIN  M_AR_LEDGER_CODE ON  M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE = S_STUDENT_LEDGER.PK_AR_LEDGER_CODE
   INNER JOIN  S_STUDENT_CAMPUS ON S_STUDENT_CAMPUS.PK_STUDENT_ENROLLMENT = S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT
	WHERE 
	S_STUDENT_MASTER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' 
	AND M_AR_LEDGER_CODE.TYPE = 1 
	AND TITLE_IV = 1 $cond $campus_cond1  
	GROUP BY S_STUDENT_LEDGER.PK_STUDENT_ENROLLMENT ORDER BY CONCAT(LAST_NAME,', ',FIRST_NAME) ";
	
	$query2 = "select CONCAT(LAST_NAME,', ',FIRST_NAME) AS NAME,IF(BEGIN_DATE = '0000-00-00','',DATE_FORMAT(BEGIN_DATE, '%Y-%m-%d' )) AS  BEGIN_DATE_1, SSN, PK_STUDENT_LEDGER, IF(EXPECTED_GRAD_DATE = '0000-00-00','', DATE_FORMAT(EXPECTED_GRAD_DATE, '%Y-%m-%d' )) AS EXPECTED_GRAD_DATE, SSN ,IF(DETERMINATION_DATE = '0000-00-00','', DATE_FORMAT(DETERMINATION_DATE, '%Y-%m-%d' )) AS DETERMINATION_DATE ,IF(GRADE_DATE = '0000-00-00','', DATE_FORMAT(GRADE_DATE, '%Y-%m-%d' )) AS GRADE_DATE, M_CAMPUS_PROGRAM.CODE as PROGRAM_CODE, M_CAMPUS_PROGRAM.DESCRIPTION as PROGRAM_DESCRIPTION, FUNDING, STUDENT_STATUS,  IF(LDA = '0000-00-00','', DATE_FORMAT(LDA, '%Y-%m-%d' )) AS LDA, STUDENT_ID, IF(TRANSACTION_DATE = '0000-00-00','', DATE_FORMAT(TRANSACTION_DATE, '%Y-%m-%d' )) AS TRANSACTION_DATE_1, sum(IF(CREDIT != 0, CREDIT, DEBIT )) AS AMOUNT, M_AR_LEDGER_CODE.CODE AS LEDGER_CODE, CAMPUS_CODE,S_STUDENT_MASTER.PK_STUDENT_MASTER   
	from 
	S_STUDENT_MASTER 
	...
	if($_POST['FORMAT'] == 1){
		...
		class MYPDF extends TCPDF {
			public function Header() {
				...
				$this->SetX(200);
				$this->SetTextColor(000, 000, 000);
				$this->Cell(55, 8, "Title IV Recipients - Student Info", 0, false, 'L', 0, '', 0, false, 'M', 'L');
		...
		while (!$res_en->EOF) {
			...
			$k = 0;
			$res_ledger = $db->Execute($query2." AND S_STUDENT_LEDGER.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT' GROUP BY  S_STUDENT_LEDGER.PK_STUDENT_ENROLLMENT,    LEDGER_CODE  ORDER BY TRANSACTION_DATE ASC");
		...
		$file_name = 'Title IV Recipients Student Info.pdf';
		/*if($browser == 'Safari')
		...
		$file_name 		= 'Title IV Recipients Student Info.xlsx';
	...
	$outputFileName );  
		...
		while (!$res_en->EOF) {
			...
			while (!$res_ledger->EOF) {
				...
				$index++;
				$cell_no = $cell[$index].$line;
				$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($res_ledger->fields['PROGRAM_DESCRIPTION']);
		...
		if ($res_address_new->RecordCount() > 0) {
			...
			$PK_STUDENT_CONTACT_TYPE_MASTER = $res_address_new->fields['PK_STUDENT_CONTACT_TYPE_MASTER'];
			   ...
			   if($PK_STUDENT_CONTACT_TYPE_MASTER==1){
				   ...
				   $HOME_PHONE				= $res_address_new->fields['HOME_PHONE'];		
				   $WORK_PHONE					= $res_address_new->fields['WORK_PHONE'];
				   $CELL_PHONE			= $res_address_new->fields['CELL_PHONE'];
		...
		$index++;
		$cell_no = $cell[$index].$line;
		$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($WORK_PHONE);
...
<head>
	...
	<title><?=MNU_TITLE_IV_RECIPIENTS_STUDENT_INFO ?> | <?=$title?></title>
	<style>
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h4 class="text-themecolor">
							<?=MNU_TITLE_IV_RECIPIENTS_STUDENT_INFO ?>
						</h4>
                    </div>
                </div>
