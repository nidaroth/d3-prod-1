...
if(!empty($_POST)){
	...
	/*else*/ if($_POST['REPORT_TYPE'] == 2)
		header("location:student_transcript_list_pdf_arlington?id=".$stud_id.'&exclude_tc='.$_POST['EXCLUDE_TRANSFERS_COURSE'].'&inc_att='.$_POST['DISPLAY_ATTENDNACE'].'&zip='.$zip);	
									...
									<div class="row" style="padding-bottom:10px;" >
										...
										<div class="col-md-2 ">
											<select id="PK_CAMPUS_PROGRAM" name="PK_CAMPUS_PROGRAM[]" multiple class="form-control" onchange="clear_search()" >
												<? $res_type = $db->Execute("select PK_CAMPUS_PROGRAM,CODE,DESCRIPTION from M_CAMPUS_PROGRAM WHERE ACTIVE = 1 AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND (CODE like 'NA' OR CODE like '%-NA' OR CODE like 'NA-%' OR CODE like '%-NA-%') order by CODE ASC");
												while (!$res_type->EOF) { ?>
	...
	<script type="text/javascript">
		...
		function search(){
			jQuery(document).ready(function($) {
				...
				var data  = 'PK_STUDENT_GROUP='+$('#PK_STUDENT_GROUP').val()+'&PK_CAMPUS='+$('#PK_CAMPUS').val()+'&PK_TERM_MASTER='+$('#PK_TERM_MASTER').val()+'&PK_CAMPUS_PROGRAM='+NA_PROGRAMS+'&PK_STUDENT_STATUS='+$('#PK_STUDENT_STATUS').val()+'&PK_COURSE='+$('#PK_COURSE').val()+'&PK_COURSE_OFFERING='+$('#PK_COURSE_OFFERING').val()+'&show_check=1&show_count=1&group_by=sid&ENROLLMENT=1';
