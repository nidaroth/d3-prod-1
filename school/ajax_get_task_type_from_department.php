...
if($_REQUEST['show_inactive'] != 1) {	
	$cond .= " AND M_TASK_TYPE.ACTIVE = 1 ";
}

$res_type = $db->Execute("select PK_TASK_TYPE,TASK_TYPE,DESCRIPTION from M_TASK_TYPE WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' $cond order by trim(TASK_TYPE) ASC"); ?>
<select id="PK_TASK_TYPE" name="PK_TASK_TYPE" class="form-control">
	<option></option>
	<? while (!$res_type->EOF) { ?>
