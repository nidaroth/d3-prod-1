    ...
    class MYPDF extends TCPDF
    {
        ...
        public function Header()
        {
           ...
           // $this->co_id_class_iterator_index = $this->co_id_class_iterator_index + 1;
            ...
            $this->SetX(108);
            $this->SetTextColor(000, 000, 000);
            $this->Cell(55, 8, "Course Offering Grade Book Analysis By SSN", 0, false, 'L', 0, '', 0, false, 'M', 'L');


            $this->SetFillColor(0, 0, 0);
            $style = array('width' => 0.2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0));
            $this->Line(108, 13, 206, 13, $style);

            $cond = "";
           // $res_campus = $db->Execute("select CAMPUS_CODE from S_CAMPUS, S_COURSE_OFFERING WHERE S_COURSE_OFFERING.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND  S_COURSE_OFFERING.PK_CAMPUS = S_CAMPUS.PK_CAMPUS AND S_COURSE_OFFERING.PK_COURSE_OFFERING IN ($GET_co_id_2) $campus_cond  order by CAMPUS_CODE ASC");
           $PK_CAMPUS   = $_GET['campus'];
...
if ($_GET['format'] == 2) 
{
    ...
    $file_name      = 'Course Offering Grade Book Analysis By SSN.xlsx';
    ...
    if ($_GET['format'] == 1) {
        ...
        $txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%">
                    <thead>
                        ...
                        <tr>
                            ...
                            <td width="20%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br /><b>SSN</b></td>
                            <td width="20%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br /><b>Program</b></td>
                            <td width="20%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><br /><br /><b>Status</b></td>
                            <td width="20%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><b>Course Offering<br />Student Status</b></td>
        ...
        while (!$res_en->EOF) {
            ...
            $txt    .= '<tr>
                            ...
                            <td width="20%" >' . $SSN . '</td>
                            <td width="20%" >' . $res_en->fields['CODE'] . '</td>
                            <td width="20%" >' . $res_en->fields['STUDENT_STATUS'] . '</td>
                            <td width="20%" >' . $res_en->fields['COURSE_OFFERING_STATUS'] . '</td>
        ...
        while (!$res_en->EOF) {
            ...
            foreach ($PK_COURSE_OFFERING_GRADE_ARR as $PK_COURSE_OFFERING_GRADE) {
                $res_stu_grade = $db->Execute("SELECT SUM(POINTS) as POINTS FROM S_STUDENT_GRADE WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING_GRADE = '$PK_COURSE_OFFERING_GRADE' AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' ");
                if ($res_stu_grade->fields['POINTS'] == 0) {
                    $Fail_Point = '';
                }
                else{
...
if ($_GET['format'] == 1) {
    $file_name = 'Course Offering Grade Book Analysis By SSN.pdf';
    $pdf->Output('temp/' . $file_name, 'FD');
    return  $file_name;
}
