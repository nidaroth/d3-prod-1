...
if(!empty($_POST)){
	...
	$res = $db->Execute("select * from S_IPEDS_SPRING_COLLECTION WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
	...
	$IPEDES_FALL_COLLECTION['EXCLUDED_DROP_REASON'] 	= implode(",",$_POST['EXCLUDED_DROP_REASON']);
	$IPEDES_FALL_COLLECTION['FOUR_YEAR_PROGRAMS'] 		= implode(",",$_POST['FOUR_YEAR_PROGRAMS']);
	$IPEDES_FALL_COLLECTION['TWO_YEAR_PROGRAMS'] 		= implode(",",$_POST['TWO_YEAR_PROGRAMS']);
	...
	if($res->RecordCount() == 0){
		...
		db_perform('S_IPEDS_SPRING_COLLECTION', $IPEDES_FALL_COLLECTION, 'insert');
		...
		db_perform('S_IPEDS_SPRING_COLLECTION', $IPEDES_FALL_COLLECTION, 'update'," PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
		$PK_IPEDES_SPRING_COLLECTION = $_GET['id'];
	}
	header("location:ipeds_spring_collection_setup");
}
$res = $db->Execute("select * from S_IPEDS_SPRING_COLLECTION WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
...
$FOUR_YEAR_PROGRAMS_ARR 	 = explode(",",$res->fields['FOUR_YEAR_PROGRAMS']);
$TWO_YEAR_PROGRAMS_ARR 		 = explode(",",$res->fields['TWO_YEAR_PROGRAMS']);
...
<head>
	...
	<title><?=MNU_IPEDS_SPRING_COLLECTIONS_SETUP_TITLE?> | <?=$title?></title>
	<style>
		li > a > label{position: unset !important;}

		/* added color for inactive text -  DIAM 24 */
		.red a > label { 
			color: red !important;
		}
	</style>
...
<body class="horizontal-nav boxed skin-megna fixed-layout">
   <? require_once("pre_load.php"); ?>
            ...
            <div class="container-fluid">
                 <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h4 class="text-themecolor"><?=MNU_IPEDS_SPRING_COLLECTIONS_SETUP_TITLE?></h4>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
									...
									<div class="d-flex">
										<div class="col-6 col-sm-6 ">
											...
											<div class="d-flex">
												<div class="col-12 col-sm-12 focused">
													<span class="bar"></span> 
													<label ><?=FOUR_YEAR_PROGRAMS?></label>
												</div>
											</div>
											<div class="d-flex">
												<div class="col-12 col-sm-12 form-group">
													<select id="FOUR_YEAR_PROGRAMS" name="FOUR_YEAR_PROGRAMS[]" multiple class="form-control" >
														...
														while (!$res_type->EOF) { 
															...
															foreach($FOUR_YEAR_PROGRAMS_ARR as $FOUR_YEAR_PROGRAMS){
																if($FOUR_YEAR_PROGRAMS == $PK_CAMPUS_PROGRAM) {
																	$selected = 'selected';
																	break;
																}
															} ?>
											...
											<div class="d-flex">
												<div class="col-12 col-sm-12 focused">
													<span class="bar"></span> 
													<label ><?=TWO_YEAR_PROGRAMS?></label>
												</div>
											</div>
											<div class="d-flex">
												<div class="col-12 col-sm-12 form-group">
													<select id="TWO_YEAR_PROGRAMS" name="TWO_YEAR_PROGRAMS[]" multiple class="form-control" >
														...
														while (!$res_type->EOF) { 
															...
															foreach($TWO_YEAR_PROGRAMS_ARR as $TWO_YEAR_PROGRAMS){
																if($TWO_YEAR_PROGRAMS == $PK_CAMPUS_PROGRAM) {
																	$selected = 'selected';
																	break;
																}
															} ?>
	...
	jQuery(document).ready(function($) {
		...
		$('#FOUR_YEAR_PROGRAMS').multiselect({
			includeSelectAllOption: true,
			allSelectedText: 'All <?=FOUR_YEAR_PROGRAMS?>',
			nonSelectedText: '',
			numberDisplayed: 3,
			nSelectedText: '<?=FOUR_YEAR_PROGRAMS?> selected'
		});
		
		$('#TWO_YEAR_PROGRAMS').multiselect({
			includeSelectAllOption: true,
			allSelectedText: 'All <?=TWO_YEAR_PROGRAMS?>',
			nonSelectedText: '',
			numberDisplayed: 3,
			nSelectedText: '<?=TWO_YEAR_PROGRAMS?> selected'
		});
