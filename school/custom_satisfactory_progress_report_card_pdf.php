...
foreach($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
	...
	/* Ticket # 1588 */
				...
				$txt .= '<br /><br />
						<table border="0" cellspacing="0" cellpadding="3" width="100%" >
							...
							<tr>
								...
								<td  width="15%" style="border-bottom:0.5px solid #000000" align="right" >'.number_format_value_checker($SCHEDULED_HOUR,2).'</td>
										...
										if($res->RecordCount() > 0) {
											if($res->fields['CODE'] == 'I'){
												...
												if($res->fields['COMPLETED'] == 1 || $res->fields['PK_SCHEDULE_TYPE'] == 2)
													$ATTENDANCE_HOURS = ($res->fields['PK_ATTENDANCE_CODE'] != '') ? $res->fields['CODE'] : '';
												else
													$ATTENDANCE_HOURS = '';
											}
										}
		...
		$txt .= '<br /><i><span style="font-size:35px" >'.$res_stu->fields['NAME'].'</span></i><br /><br />';
		$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%" >
					<thead>
						<tr>
							...
							<td width="8%" style="border-bottom:0.5px solid #000;border-top:0.5px solid #000;" align="right" ><br /><br /><b>Absent Count</b></td>
							<td width="7%" style="border-bottom:0.5px solid #000;border-top:0.5px solid #000;border-right:0.5px solid #000;" align="right" ><br /><br /><b>Present Count</b></td>';

							/*$txt .=<td width="8%" style="border-bottom:0.5px solid #000;border-top:0.5px solid #000;" align="right" ><br /><br /><b>Hours Attended</b></td>
		...
		while (!$res_course->EOF) { 
			...
			$res_attended = $db->Execute("SELECT IFNULL(SUM(ATTENDANCE_HOURS),0) AS ATTENDED_HOUR,COUNT(S_STUDENT_ATTENDANCE.PK_STUDENT_SCHEDULE) as PRESENT FROM S_STUDENT_ATTENDANCE,S_STUDENT_SCHEDULE WHERE S_STUDENT_SCHEDULE.PK_STUDENT_COURSE = '$PK_STUDENT_COURSE' AND S_STUDENT_SCHEDULE.PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND S_STUDENT_SCHEDULE.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND S_STUDENT_SCHEDULE.PK_STUDENT_SCHEDULE = S_STUDENT_ATTENDANCE.PK_STUDENT_SCHEDULE AND COMPLETED = 1 AND S_STUDENT_SCHEDULE.PK_STUDENT_ENROLLMENT IN ($PK_STUDENT_ENROLLMENT1) AND S_STUDENT_ATTENDANCE.PK_ATTENDANCE_CODE IN ($present_att_code) ");
			...
			$total_present 		+= $res_attended->fields['PRESENT'];
			...
			$txt .=	'<tr>
					...
					<td width="8%" align="right" >'.$res_abs->fields['ABSENT'].'</td>
					<td width="7%" align="right" style="border-right:0.5px solid #000;">'.$res_attended->fields['PRESENT'].'</td>';

				/*	$txt .=	'<td width="8%" align="right" >'.number_format_value_checker($res_attended->fields['ATTENDED_HOUR'],2).'</td>
					<td width="7%" align="right" >'.number_format_value_checker(($missed),2).'</td>
					<td width="8%" align="right" style="border-right:0.5px solid #000;" >'.number_format_value_checker($SCHEDULED_HOUR,2).'</td>
		...
		if($include_tc==1) {
			$str_txt =	'<td width="24%" style="border-top:0.5px solid #000;" >Transferred: </td><td width="13%" style="border-top:1px solid #000;"  >'.number_format_value_checker($res_tc_1->fields['HOUR'],2).'</td>';
			$strmsg='';
		}else{
		...
		$txt .=	'
			...
			$txt .=	'<tr><td width="24%" style="border-top:0.5px solid #000;"></td>
				<td width="8%" align="right" style="border-top:0.5px solid #000;" >'.number_format_value_checker($total_absent,2).'</td>
				<td width="7%" align="right" style="border-top:0.5px solid #000;" >'.number_format_value_checker($total_present,2).'</td>
			</tr>';
		$txt .=	'</table>';
...
$file_name = 'Satisfactory Progress Report.pdf';
/*
if($browser == 'Safari')
