...
function student_transcript_list_pdf($PK_STUDENT_MASTERS, $one_stud_per_pdf, $report_name){
	...
	$atttendance_type=($_REQUEST['ATTENDANCE_TYPE_1']?$_REQUEST['ATTENDANCE_TYPE_1']:'');
	  ...
	  if($res_course->RecordCount() > 0) // DIAM-2239
	  {
		...
		$txt .= '<div style="border-top: 2px #c0c0c0 solid" ></div>
				<table border="0" cellspacing="0" cellpadding="3" width="100%" >
					...
					<tr>
						...
						if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 2 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
							$txt .= '<td width="7%" align="right" ><b>Numeric</b><br /><u><b>Grade</b></u></td>';
				...
				$txt .= '<td width="10%" align="right" ><b>Units</b><br /><u><b>Attempted</b></u></td>
					...
					if($_GET['exclude_tc'] != 1) {
						...
						while (!$res_tc->EOF) {
							...
							$txt .= '<tr>
									...
									if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 2 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
										$txt .= '<td width="7%" align="right" >'.$res_tc->fields['TC_NUMERIC_GRADE'].'</td>';
								//DIAM-781
								$txt .= '<td width="10%" align="right" >'.number_format_value_checker($ATTEMPTED_UNITS,2).'</td>
										...
										<td width="10%" align="right" >'.number_format_value_checker(($summation_of_gpa/$summation_of_weight),2).'</td>
									</tr>';
							/* ticket #1240 */
					...
					if($res_course->RecordCount() > 0 && $_GET['json_check'] == '1') // DIAM-2239
					{
						$data_count[] = $res_course->RecordCount();
					}
					// End DIAM-2239
					while (!$res_course->EOF) { 
						...
						if($res_course->fields['CALCULATE_GPA'] == 1) {
							...
							if(has_wvjc_access_transcript_desc($_SESSION['PK_ACCOUNT'],1)){ 
								if(has_wvjc_access_show_only_term_desc($_SESSION['PK_ACCOUNT'],1)){ 
									if(!in_array($_SESSION['PK_ACCOUNT'],$acc_array)){ //DIAM-1818
									$BEGIN_DATE 	= ($res_course->fields['TERM_DESCRIPTION'] != '') ? $res_course->fields['TERM_DESCRIPTION'] : $res_course->fields['BEGIN_DATE'];
									}
								}else{
								$dash_desc ='';
						...
						$txt .= '<tr>
									...
									if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 2 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
										$txt .= '<td width="7%" align="right" >'.$res_course->fields['NUMERIC_GRADE'].'</td>';
			...
			$txt .= '<tr>
						...
						if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 2 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
							$txt .= '<td width="7%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker(($c_in_num_grade_tot/$total_rec),2).'</b></i></td>';
						//DIAM-781
				$txt .= '<td width="10%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker($c_in_att_tot,2).'</b></i></td>
						...
						<td width="10%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker(($summation_of_gpa/$summation_of_weight),2).'</b></i></td>
					</tr>
					</table> ';
					/* ticket #1240 */
		...
		if($_GET['inc_att'] == 1) {
			...
			if($atttendance_type==2)
				{	
					$txt .='<br /><br /><br /><br /><div style="font-size:35px"><b>Attendance Percentage: '.number_format_value_checker(($res_attended->fields['ATTENDED_HOUR'] / $COMP_SCHEDULED_HOUR * 100),2).'%</b></div>';
