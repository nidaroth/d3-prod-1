		...
		<tr nobr="true">
			<td width="100%" >
				...
				$txt .= '<tr>';
					...
					if(has_custom_sap_report($_SESSION['PK_ACCOUNT'])==1 && $_GET['type']==3){
						$cond="";
					}
					...
					while (!$res_course->EOF) { 
						...
						if( has_custom_sap_report($_SESSION['PK_ACCOUNT'])==1 && $COMP_SCHEDULED_HOUR > 0){
							...
							$attended_percentage = $res_attended->fields['ATTENDED_HOUR'] / $COMP_SCHEDULED_HOUR * 100;  //DIAM-2300
							$total_attended_percentage += $attended_percentage;
							$per_index++;
						} else 
						...
						if($_GET['report_type'] == 1) {
							...
							if(has_custom_sap_report($_SESSION['PK_ACCOUNT'])==1)
							{
								...
								$focus_scheduled_hours=$COMP_SCHEDULED_HOUR;  //DIAM-2300
							}
					...
					if(has_custom_sap_report($_SESSION['PK_ACCOUNT'])==1){
						...
						$total_schedule=$tot_com_sch; //DIAM-2300
					}
...
if($_GET['id'] == '') {
	...
	if($_GET['zip'] == 1) {
		...
		co_grade_book_progress_pdf($_GET['id'], 0);
}
