<?php  require_once("../global/config.php");
require_once("../language/common.php");
require_once("../language/menu.php");
require_once("../language/course_offering.php");

function dfrmt($datestr , $datefrmt = "m/d/Y"){
	return date($datefrmt, strtotime($datestr));
}
function tfrmt($timestr ,$timefrmt = 'h:i A'){
	return date( $timefrmt, strtotime(' 01/01/2024 '.$timestr));
}
function logtime(&$fp, $start_time_exe, $msg)
{
	$time_spent = ( microtime(true) - $start_time_exe) /* / 60*/;
	fwrite($fp, "\n" . $msg . " - <b>" . round($time_spent,2) . "</b>" . "\n");
	// dump($msg ,$time_spent);
};
if (!empty($_POST) || !empty($_GET)) {
//  ENABLE_DEBUGGING(TRUE);
	$start_time_exe = microtime(true);
	$fp = fopen('temp/attendance_roaster_time_log.txt', 'w');
	logtime($fp, $start_time_exe, "Started report generation");
	global $db;
	$res = $db->Execute("SELECT PK_TIMEZONE FROM Z_ACCOUNT WHERE  PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
	$timezone = $res->fields['PK_TIMEZONE'];
	if ($timezone == '' || $timezone == 0)
		$timezone = 4;
	$res = $db->Execute("SELECT TIMEZONE FROM Z_TIMEZONE WHERE  PK_TIMEZONE = '$timezone' ");

	$TIMEZONE = $res->fields['TIMEZONE'];
	$date_in_footer = convert_to_user_date(date('Y-m-d H:i:s'), 'l, F d, Y h:i A', $TIMEZONE, date_default_timezone_get());
	$date_in_header = convert_to_user_date(date('Y-m-d H:i:s'), 'm/d/Y h:i A', $TIMEZONE, date_default_timezone_get());

	//Ticket # 1194  
	// echo "<pre>";print_r($_POST);exit;
	$cond = "";

	/* Ticket # 1194   */
	if (!empty($_GET)) {
		$_POST['PRINT_TYPE'] 			= $_GET['pt'];
		$_POST['PK_COURSE_OFFERING'] 	= explode(",", $_GET['co']);
		$_POST['INSTRUCTOR'] 			= explode(",", $_GET['ins']);
		$_POST['PK_TERM_MASTER'] 		= $_GET['tm'];
		$_POST['FORMAT'] 				= 1;
	}
	/* Ticket # 1194   */

	if ($_POST['PRINT_TYPE'] == 1) {
		$PK_COURSE_OFFERING = implode(",", $_POST['PK_COURSE_OFFERING']);
		$cond .= " AND S_COURSE_OFFERING.PK_COURSE_OFFERING IN ($PK_COURSE_OFFERING) ";
	} else if ($_POST['PRINT_TYPE'] == 2) {
		$INSTRUCTOR = implode(",", $_POST['INSTRUCTOR']);
		$cond .= " AND S_COURSE_OFFERING.PK_TERM_MASTER = '$_POST[PK_TERM_MASTER]' AND INSTRUCTOR IN ($INSTRUCTOR) ";
	}
	logtime($fp, $start_time_exe, "---> Getting Course offerings - sql");
	$query = "SELECT S_COURSE_OFFERING.PK_COURSE_OFFERING, TRANSCRIPT_CODE, COURSE_DESCRIPTION
	
	FROM
	S_COURSE, S_COURSE_OFFERING, S_COURSE_OFFERING_SCHEDULE_DETAIL    
	WHERE 
	S_COURSE_OFFERING.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND 
	S_COURSE_OFFERING.PK_COURSE_OFFERING = S_COURSE_OFFERING_SCHEDULE_DETAIL.PK_COURSE_OFFERING $cond  
	GROUP BY S_COURSE_OFFERING.PK_COURSE_OFFERING ORDER By TRANSCRIPT_CODE ";
// echo $query ; exit;
	$res_course = $db->Execute($query);
	while (!$res_course->EOF) {
		$PK_COURSE_OFFERING_ARR[] 	= $res_course->fields['PK_COURSE_OFFERING'];

		$res_course->MoveNext();
	}
	logtime($fp, $start_time_exe, "**** E - Getting Course offerings - sql");
	$group_by = " GROUP BY S_STUDENT_MASTER.PK_STUDENT_MASTER ";
	$order_by = " ORDER BY CONCAT(LAST_NAME,', ',FIRST_NAME, ' ',S_STUDENT_MASTER.MIDDLE_NAME) ASC ";

	if ($_POST['FORMAT'] == 1) {
		$wkhtml = '';
		/*
			#Algorithem - AV

			1. Iterate for COURSE_OFFERING / INSTRUCTORS 

			2. Get all students , make student row htmls and return all enrollment IDs

			3. Loop for Date range 

			4. Iterate for week while , while checking if week is valid and have minimum one scheduled class date.

			5. If yes increase $week_count ; Render the week's data 

			6. Repeat ;  CHECK IF WEEK_COUNT > 4 If YES RE-RENDER TABLE WHILE SAVING OLD HTML


		*/
		logtime($fp, $start_time_exe, "---> Iterating PK COURSE OFERINGS -  sql ");
		#1. Iterate for COURSE_OFFERING / INSTRUCTORS
		$Output_Per_Offering = [];
		$COURSE_OFFERING_HEADERS = [];
		foreach ($PK_COURSE_OFFERING_ARR as $PK_COURSE_OFFERING) {
			// echo "-------------------PK_COURSE_OFFERING :  $PK_COURSE_OFFERING ----------------- ";

			$course_info = get_course_info($PK_COURSE_OFFERING);
			list($START_DATE, $END_DATE) = get_schedule_start_and_end_dates($course_info, $PK_COURSE_OFFERING);
			
			if($_POST['START_DATE_NEW'] != ''){
				$START_DATE = date("Y-m-d" , strtotime($_POST['START_DATE_NEW']));
			}
			if($_POST['AS_OF_DATE'] != ''){
				$END_DATE = date("Y-m-d" , strtotime($_POST['AS_OF_DATE']));
			}
			// print_r([$START_DATE, $END_DATE]);

			$course_code_row_with_date_ranges_of_week = '';
			$language_row_with_date_numbers = '';
			$term_start_row_with_s_m_t_w_t_f_s = '';

			$COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING] = '<h1 style="font-size:0px !important">'.$PK_COURSE_OFFERING.'</h1>';

			$border_right_important = 'border-right : 2px solid black !important; font-size:12px !important;padding : 6px';

			##NEW LOGIC TO SHOW GROUPED SCHEDULED DATA
			$START_DATE_SCH = date('Y-m-d',strtotime($START_DATE));	//DIAM-1958
			$END_DATE_SCH = date('Y-m-d',strtotime($END_DATE)); //DIAM-1958

			$grouped_schedule = 
			"SELECT
				MIN(SCHEDULE_DATE) AS G_START_DATE, 
				MAX(SCHEDULE_DATE) AS G_END_DATE,
				SUM(HOURS) AS G_TOTAL_HOURS,
				START_TIME,
				END_TIME,
				HOURS,
				COUNT(PK_COURSE_OFFERING_SCHEDULE_DETAIL) AS G_TOTAL_MEETINGS
			FROM
				`S_COURSE_OFFERING_SCHEDULE_DETAIL`
			WHERE
			PK_ACCOUNT = $_SESSION[PK_ACCOUNT]
			AND PK_COURSE_OFFERING = $PK_COURSE_OFFERING
				AND SCHEDULE_DATE BETWEEN '$START_DATE_SCH' AND '$END_DATE_SCH'
			GROUP BY
				PK_COURSE_OFFERING_SCHEDULE,
				START_TIME,
				END_TIME;
			"; //DIAM-1958
			// $grouped_schedule = 
			// "SELECT
			// 	MIN(SCHEDULE_DATE) AS G_START_DATE,
			// 	MAX(SCHEDULE_DATE) AS G_END_DATE,
			// 	SUM(HOURS) AS G_TOTAL_HOURS,
			// 	START_TIME,
			// 	END_TIME,
			// 	HOURS,
			// 	COUNT(PK_COURSE_OFFERING_SCHEDULE_DETAIL) AS G_TOTAL_MEETINGS
			// FROM (
			// 	SELECT
			// 		SCHEDULE_DATE,
			// 		PK_COURSE_OFFERING_SCHEDULE_DETAIL,
			// 		START_TIME,
			// 		END_TIME,
			// 		HOURS,
			// 		ROW_NUMBER() OVER (ORDER BY SCHEDULE_DATE) -
			// 		ROW_NUMBER() OVER (PARTITION BY START_TIME, END_TIME ORDER BY SCHEDULE_DATE) AS grp
			// 	FROM
			// 		`S_COURSE_OFFERING_SCHEDULE_DETAIL`
			// 	WHERE
			// 		PK_ACCOUNT = $_SESSION[PK_ACCOUNT]
			// 		AND PK_COURSE_OFFERING = $PK_COURSE_OFFERING
			// 		AND SCHEDULE_DATE BETWEEN '$START_DATE' AND '$END_DATE'
			// ) AS grouped
			// GROUP BY
			// 	START_TIME,
			// 	END_TIME,
			// 	grp
			// ORDER BY G_START_DATE ASC
			// "; 
			$res_grouped_schedule = $db->Execute($grouped_schedule);
			// echo $grouped_schedule;exit;
			##END OF LOGIC S.G.S.D

$border2px = '; border : 2px solid black !important;border-collapse: collapse; padding:3px;padding-top:7px;padding-bottom:7px;';
			$JAVASCRIPT_COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING] = 
			'<table  style="border-right : 0px !important;border-collapse: collapse;" cellspacing="0" cellpadding="3" width="100%">
			<tr> 
	<td width="16.66%" align="center" style="'.$border2px.'"><b> Course  :   '. $course_info->fields['COURSE_CODE'] . '(' . $course_info->fields['SESSION'][0] . '-' . $course_info->fields['SESSION_NO'] . ')</b><br>'.$course_info->fields['COURSE_DESCRIPTION'].'</td>
	<td width="16.66%" align="center" style="'.$border2px.';"><b> Term Dates  </b> <br> '.$course_info->fields['TERM_BEGIN_DATE'].' - '.$course_info->fields['TERM_END_DATE'].' </td>
	
	<td width="16.66%" align="center" style="'.$border2px.'"><b>Instructor </b> <br>' . ($course_info->fields['INSTRUCTOR_NAME']) .'&nbsp;</td>
	<td width="7%" align="center" style="'.$border2px.'"><b>Room </b><br>' . $course_info->fields['ROOM_NO'] . '&nbsp;</td>
	<td width="10.66%" align="center" style="'.$border2px.'; "><b>Attendance  </b><br>'.$course_info->fields['ATTENDANCE_TYPE'].'&nbsp;</td>
	<td width="6.66%" align="center" style="'.$border2px.'; "><b>Status  </b><br>'.$course_info->fields['COURSE_OFFERING_STATUS'].'&nbsp;</td>';


	$JAVASCRIPT_COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING] .= '</tr>';
	$JAVASCRIPT_COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING] .="</table>"; 
 
$secondary_line_of_scheduled_hours = '';
$secondary_line_of_scheduled_hours .= '<table width="100%" style="border-spacing:0px;border-collapse: collapse;">';
// Start the loop using $res_grouped_schedule->EOF
$border1px = '; border : 2px solid black !important;border-top : 0px !important;border-collapse: collapse; padding:3px;padding-top:7px;padding-bottom:7px;';

while (!$res_grouped_schedule->EOF) {
    // Open a new table row
    $secondary_line_of_scheduled_hours .= '<tr>';

    // Output the total_amount property in a table cell
    $secondary_line_of_scheduled_hours .= '<td style="width : 25%; '.$border1px.' text-align : left !important;"><b>Course : </b> '.dfrmt($res_grouped_schedule->fields['G_START_DATE']).' to '. dfrmt($res_grouped_schedule->fields['G_END_DATE']).' - '.$res_grouped_schedule->fields['G_TOTAL_HOURS'].' hours - '.$res_grouped_schedule->fields['G_TOTAL_MEETINGS'].' meetings
	<br>
	<b>Class : </b> '.tfrmt($res_grouped_schedule->fields['START_TIME']).' to '. tfrmt($res_grouped_schedule->fields['END_TIME']).' - '.$res_grouped_schedule->fields['HOURS'].' hours </td>';

    // Check if there are more items in the result set
    $res_grouped_schedule->MoveNext();

    // Output additional items up to a maximum of three
    for ($itemCount = 1; $itemCount < 4; $itemCount++) {
        // Check if there are more items in the result set
        if (!$res_grouped_schedule->EOF) {
            // Output the total_amount property in a table cell
            $secondary_line_of_scheduled_hours .= '<td style="width : 25%; '.$border1px.' text-align : left !important; "><b>Course : </b> '.dfrmt($res_grouped_schedule->fields['G_START_DATE']).' to '. dfrmt($res_grouped_schedule->fields['G_END_DATE']).' - '.$res_grouped_schedule->fields['G_TOTAL_HOURS'].' hours - '.$res_grouped_schedule->fields['G_TOTAL_MEETINGS'].' meetings
			<br>
			<b>Class : </b> '.tfrmt($res_grouped_schedule->fields['START_TIME']).' to '. tfrmt($res_grouped_schedule->fields['END_TIME']).' - '.$res_grouped_schedule->fields['HOURS'].' hours
			</td>';

            // Move to the next row in the result set
            $res_grouped_schedule->MoveNext();
        } else {
            // If no more items in the result set, output an empty cell
            $secondary_line_of_scheduled_hours .= '<td style="width : 25%; '.$border1px.' text-align : left !important; "></td>';
        }
    }

    // Close the table row
    $secondary_line_of_scheduled_hours .= '</tr>';
}

// Close the table tag
$secondary_line_of_scheduled_hours .= '</table>';
$JAVASCRIPT_COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING] .= $secondary_line_of_scheduled_hours; 
//  print_r($JAVASCRIPT_COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING]["secondary_header"]);
//  print($SECONDARY_HEADERS[$PK_COURSE_OFFERING]["secondary_header"]);exit;	 
/*Appends header secton into body , use only for testing 
				$COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING]	.= $JAVASCRIPT_COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING];*/
 

			#################~~~~~~~~~~~###############
			#################~~~~~~~~~~~####################################
			#################~~~~~~~~~~~##################################################


			$week_count = 1;
			#2.1 Get all students - Query
			logtime($fp, $start_time_exe, "---> Get student info");

			$students = get_students_info($PK_COURSE_OFFERING);
			$STUDENT_ARR = [];
			$student_table_static='';
			while(!$students->EOF){
				$IMAGE = $students->fields['IMAGE'];
				$NAME = $students->fields['STUD_NAME'];
				$ID = $students->fields['STUDENT_ID'] ?? $students->fields['BADGE_ID'] ;
				$student_table_static   .=  
				"
				<tr>
				<td width='75px' align='left' style='border-bottom : 1px solid #dee2e6; padding-top : 5px;'><img src='".$IMAGE."' style='width : 70px ; height:70px;'> </td>
				<td width='25%' align='left' style='border-bottom : 1px solid #dee2e6'>$NAME</td>
				<td width='25%' align='left' style='border-bottom : 1px solid #dee2e6'>$ID</td>
				<td width='25%' align='left' style='border-bottom : 1px solid #dee2e6'>_________________________</td>
				</tr>
				
				";
				 

				$students->MoveNext();
			 }
			// dd($students);
			if($student_table_static != '')
			{
			logtime($fp, $start_time_exe, " **** E- Get student info");

			#2.2 Make student row htmls and return all enrollment IDs
			// list($PK_STUDENT_ENROLLMENTS, $PK_STUDENT_COURSES, $student_rows) = get_student_rows($students);
			//min-width : 17.96% ; max-witdh : 400px; 
			#check if enrollment and course is not empty 
			// dd($PK_STUDENT_ENROLLMENTS , $PK_STUDENT_COURSES);
			// if (!empty($PK_STUDENT_ENROLLMENTS) && !empty($PK_STUDENT_COURSES)) {
				#Loop for date range
				$DATE_ITERATOR = $START_DATE;
				$temp_student_rows = $student_rows;
				#reset total_hours
				// foreach ($temp_student_rows as $temp_student_key => $temp_student_val) {
				// 	$temp_student_rows[$temp_student_key]['total_hours'] = 0;
				// }
				$Output_Per_4_weeks  = [];
				$count_of_week = 1 ;
				// echo "PK OFFERING FORM -> $DATE_ITERATOR	 $END_DATE <br>"; 
				while (strtotime($DATE_ITERATOR) <= strtotime($END_DATE)) {
					# code...
					// echo " -  $DATE_ITERATOR  - ";
					if (check_valid_week($DATE_ITERATOR, $PK_STUDENT_ENROLLMENTS, $PK_STUDENT_COURSES)) {

						//MAIN LOGIC  DIAM-1461 

						#Add class schedule header
						global  $CLASS_TIME_IN;
						global $CLASS_TIME_OUT ; 
						try {
							$CLASS_TIME_IN = date( 'h:i A', strtotime(' 01/01/2024 '.$CLASS_TIME_IN));
							$CLASS_TIME_OUT = date( 'h:i A', strtotime(' 01/01/2024 '.$CLASS_TIME_OUT));
						} catch (\Throwable $th) {
							//throw $th;
							$CLASS_TIME_IN = '';
							$CLASS_TIME_OUT = '';
						}
						
						$att_table  =  "
						<style>
						.break-after {
							page-break-after: always;
						}
						.table_std {
							border-collapse: collapse; 
    						border-spacing: 0px;
						}

						</style>
						<table width='100%'  >
						<tr>
						<td> <b style='font-size : 14px'>". date("m/d/Y - l", strtotime($DATE_ITERATOR))."</b></td>
						<td><b style='font-size : 14px'>".$CLASS_TIME_IN."</b></td>
						<td><b style='font-size : 14px'>".$CLASS_TIME_OUT."</b></td>
						<td></td>
						</tr>
						</table>
						<div style='height:15px;width : 70px;'> &nbsp; </div>";
					 	$att_table  .=  "<table width='100%' class='break-after table_std' border='0'>
						<tr>
						<td width='75px' align='left' style='border:none !important'  ><b style='font-size : 14px'>Students</b> </td>
						<td width='25%' align='left'  style='border:none !important' ></td>
						<td width='25%' align='left' style='border:none !important'  ></td>
						<td width='25%' align='left'  style='border:none !important' ><b style='font-size : 14px'>Signature</b></td>
						</tr>
						".$student_table_static."
						<tr>
						<td width='75px' align='left'  ><div style='width : 70px ; height:70px;'> </div> </td>
						<td width='25%' align='left' ></td>
						<td width='25%' align='left'  ><b style='font-size : 16px'>Done</b></td>
						<td width='25%' align='left'  >_________________________</td>
						</tr>
						</table>";
						// echo $att_table;exit;
						$Output_Per_Offering[$PK_COURSE_OFFERING] = $Output_Per_Offering[$PK_COURSE_OFFERING].$att_table;
					
					}
					$DATE_ITERATOR = date("Y-m-d", strtotime($DATE_ITERATOR . ' + 1 days'));
				}
				// exit;
				 
			// }
			if(isset($_REQUEST['SHOW_TOTAL_HOURS'])){
				$Output_Per_Offering[$PK_COURSE_OFFERING]['total_hour_table'] = prepare_total_table($student_rows);
			}
		}
		} //<----------- Closing PK_COURSE_OFFERING_ARR 
		// exit;
		// echo "debug";
		logtime($fp, $start_time_exe, "*** E - Iterating PK COURSE OFERINGS -  sql l ");
		//  print_r($Output_Per_Offering); 
		$html_to_print = prepare_html_for_print($Output_Per_Offering, $COURSE_OFFERING_HEADERS , true , $SECONDARY_HEADERS);
		// exit;
		// echo $html_to_print;exit;
		
		printwkhtml($html_to_print , $JAVASCRIPT_COURSE_OFFERING_HEADERS);
		dd("End of code");

		#################~~~~~~~~~~~##################################################
		#################~~~~~~~~~~~####################################
		#################~~~~~~~~~~~############### 
	} else {
		//do nothing ,,,, maybe ??
	}
}
function add_total_hours_from_temp_to_original_student_array( &$temp_student_rows, &$student_rows){

	foreach ($student_rows as $key => $value) {
		$student_rows[$key]['total_hours'] = $temp_student_rows[$key]['total_hours'];
	}
}

function prepare_total_table($student_rows){
	$total_table = "<table>
	<thead>
		<th colspan='2' style='font-size : 14px;'>Total Hours</th> 
	</thead>
	<tbody>
";
	foreach ($student_rows as $key => $value) {
		 

		if($student_rows[$key]['total_hours'] == ''){
			$student_rows[$key]['total_hours'] = '0';
		}

		$total_table .= $student_rows[$key]['html'].'<td style="width: 120px !important">'. number_format($student_rows[$key]['total_hours'] , 2).'</td></tr>';
		
	}
	return $total_table .= "</tbody></table>";
}

function save_this_4_weeks_go_to_next(

	&$week_count,
	&$table,
	&$course_info,
	&$course_code_row_with_date_ranges_of_week,
	&$language_row_with_date_numbers,
	&$term_start_row_with_s_m_t_w_t_f_s,
	&$temp_student_rows,
	&$Output_Per_4_weeks,
	&$student_rows


) {
	$week_count = 1;

	$table = '';
	$table .=  '<style>
				table, th, td {
					/* border: 1px solid black; */
					border-collapse: collapse;
				}
				</style>
				<table style="table-layout: fixed;" >
				<thead>
				<tr>
				<td style="width:182px; text-align:left !important; padding : 5px;" rowspan="3">
					<b>Course: ' . $course_info->fields['TRANSCRIPT_CODE'] . '(' . $course_info->fields['SESSION'][0] . '-' . $course_info->fields['SESSION_NO'] . ')<br>' . $course_info->fields['COURSE_DESCRIPTION'] . '<br> Term Start:' . $course_info->fields['BEGIN_DATE_1'] . '</b>
				</td>';


	$table .= "$course_code_row_with_date_ranges_of_week </tr>";
	$table .= "<tr>$language_row_with_date_numbers</tr>";
	$table .= "<tr>$term_start_row_with_s_m_t_w_t_f_s</tr>
	</thead>";


	$course_code_row_with_date_ranges_of_week = '';
	$language_row_with_date_numbers = '';
	$term_start_row_with_s_m_t_w_t_f_s = '';

	foreach ($temp_student_rows as $temp_student_row) {
		$table .=  $temp_student_row['html'] . '</tr>';
	}
	$table .=   '</table> <br>';

	$Output_Per_4_weeks[] = $table;
	// echo "<br>---------========= END OF FOUR WEEKS ===========-----<br>";
	// print_r($Output_Per_4_weeks);
	$table = '';
	$temp_student_rows = $student_rows;
}

function invoice_pdf_custom_header()
{
	global $db;
	global $http_path;

	$res = $db->Execute("SELECT PDF_LOGO,SCHOOL_NAME, ADDRESS, ADDRESS_1, CITY, STATE_CODE, ZIP, PHONE, WEBSITE FROM Z_ACCOUNT LEFT JOIN Z_STATES ON Z_STATES.PK_STATES = Z_ACCOUNT.PK_STATES WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
	$SCHOOL_NAME 	= $res->fields['SCHOOL_NAME'];
	$PDF_LOGO 	 	= $res->fields['PDF_LOGO'];

	$logo = "";
	if ($PDF_LOGO != '') {
		//$logo = '<img src="'.$PDF_LOGO.'" height="50px" />';
		$PDF_LOGO = str_replace('../', $http_path, $PDF_LOGO);
		$logo = '<img src="' . $PDF_LOGO . '" width="100px" />';
	}

	$date1 = $_REQUEST['START_DATE_NEW'];
	$date2 = $_REQUEST['AS_OF_DATE'];
	$date_string  = 'All Class Dates';
	// print_r($_REQUEST['START_DATE_analysis']);
	if ($_REQUEST['START_DATE_NEW'] != '' && $_REQUEST['AS_OF_DATE'] != '') {
		$date_string =  'Class Dates Between ' . $date1 . ' and ' . $date2;
	} else if ($_REQUEST['START_DATE_NEW'] != '' && $_REQUEST['AS_OF_DATE'] == '') {
		$date_string =  'Class Dates From ' . $date1;
	} else if ($_REQUEST['START_DATE_NEW'] == '' && $_REQUEST['AS_OF_DATE'] != '') {
		$date_string =  'Class Dates Till ' . $date2;
	}

	// $header = '<table width="100%" >
	// 		<tr>
	// 			<td width="20%" valign="top" >' . $logo . '</td>
	// 			<td width="40%" valign="top" style="font-family:helvetica;font-weight:normal;" >
	// 			<table width="100%" >
	// 				<tr><td valign="top" style="font-size:28px; text-align:left !important" >' . $SCHOOL_NAME . '</td></tr>
	// 				<tr><td valign="top" style="font-size:14px; text-align:left !important" >' . $res->fields['ADDRESS'] . ' ' . $res->fields['ADDRESS_1'] . '</td></tr>
	// 				<tr><td valign="top" style="font-size:14px; text-align:left !important" >' . $res->fields['CITY'] . ',' . $res->fields['STATE_CODE'] . ',' . $res->fields['ZIP'] . '</td></tr>
	// 				<tr><td valign="top" style="font-size:14px;text-align:left !important" >' . $res->fields['PHONE'] . '</td></tr>
	// 			</table>
	// 			</td>
	// 			<td width="40%" valign="top" >
	// 				<table width="100%" >
	// 					<tr>
	// 						<td width="100%" align="right" style="font-size:32px;border-bottom:1px solid #000;font-style: italic;font-family:helvetica;font-weight:normal;" >Monthly Attendance Analysis</td>
	// 					</tr>
	// 					<tr><td width="100%" align="right" style="font-size:16px;solid #000;font-style: italic;font-family:helvetica;font-weight:normal;" >' . $date_string . '</td></tr>										
	// 				</table>
	// 			</td>
	// 		</tr>							
	// </table>';

	global $date_in_header;
	$header = ' 
	<table width="100%" >
			<tr>
				<td width="15%" valign="top" style="text-align:left" >' . $logo . '</td>
				<td width="40%" valign="top" style="font-family:helvetica;font-weight:normal;" >
				<table width="100%" >
					<tr><td valign="top" style="font-size:28px; text-align: left; margin-left 20px;padding-top:25px;" >' . $SCHOOL_NAME . '</td></tr>
					</table>
				</td>
				<td width="40%" valign="top" >
				<table width="100%" >
									<tr>
										<td width="100%" align="right" style="font-size:22px;border-bottom:1px solid #000;font-style: italic;font-family:helvetica;font-weight:normal;padding-top : 20px;text-align:right !important" >Daily Attendance Sheet</td>
									</tr>
									<tr><td width="100%" align="right" style="font-size:14px;solid #000;font-style: italic;font-family:helvetica;font-weight:normal;text-align:right !important" >Printed: ' .$date_in_header  . '</td></tr>	
									<tr><td width="100%" align="right" style="font-size:14px;solid #000;font-style: italic;font-family:helvetica;font-weight:normal;text-align:right !important" >' . $date_string . '</td></tr>										
								</table>
				</td>
			</tr>							
	</table>';

	return $header;
}

function pdf_footer()
{
	global $TIMEZONE;
	global $db;
	global $date_in_footer;
	// dd($TIMEZONE);
	$date = convert_to_user_date(date('Y-m-d H:i:s'), 'l, F d, Y h:i A', $TIMEZONE, date_default_timezone_get());


	$PK_PDF_FOOTER = 17;
	$res_type = $db->Execute("SELECT FOOTER_LOC, CONTENT FROM S_PDF_FOOTER,S_PDF_FOOTER_CAMPUS WHERE S_PDF_FOOTER.ACTIVE = 1 AND S_PDF_FOOTER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PDF_FOR = '$PK_PDF_FOOTER' AND S_PDF_FOOTER.PK_PDF_FOOTER = S_PDF_FOOTER_CAMPUS.PK_PDF_FOOTER "); //AND PK_CAMPUS = '$PK_CAMPUS'

	$CONTENT = nl2br($res_type->fields['CONTENT']);

$footer = "";

	// $footer = '				<table width="100%" border="0"> 
	// 							<tr>
	// 								<td valign="top" style="font-size:10px ; text-align:center">' . $CONTENT . '</td>
	// 							</tr>
	// 						</table>
	// 						';
							global $TIMEZONE;
							$footer .= '	
							<table width="100%" border="0"> 
								<tr>
									<td valign="top" style="font-size:10px"><i>' . $date_in_footer . '</i></td>
									<td valign="top" style="font-size:10px;" align="right" colspan="2">Page <span id="page"></span> of
										<span id="topage"></span>
									</td>
									<td valign="top" style="font-size:10px;"></td>
								</tr>
							</table>';
	return $footer;
} 


function get_course_info($PK_COURSE_OFFERING)
{
	global $db;
	$course_info_sql =
		"SELECT 
			DATE_FORMAT(DEF_START_TIME,'%h:%i %p') AS START_TIME, 
			DATE_FORMAT(DEF_END_TIME,'%h:%i %p') AS END_TIME, 
			HOURS, ROOM_NO,FA_UNITS,  
			UNITS, 
			CONCAT(S_EMPLOYEE_MASTER_INST.LAST_NAME,', ',S_EMPLOYEE_MASTER_INST.FIRST_NAME) AS INSTRUCTOR_NAME,ATTENDANCE_TYPE, 
			IF(BEGIN_DATE = '0000-00-00','',DATE_FORMAT(BEGIN_DATE, '%m/%d/%Y' )) AS  BEGIN_DATE_1 ,  
			IF(S_TERM_MASTER.END_DATE = '0000-00-00','',DATE_FORMAT(S_TERM_MASTER.END_DATE, '%m/%d/%Y' )) AS  END_DATE_1,
			SESSION,
			SESSION_NO,
			COURSE_OFFERING_STATUS, 
			COURSE_CODE,
			TRANSCRIPT_CODE, 
			IF(S_TERM_MASTER.BEGIN_DATE != '0000-00-00',DATE_FORMAT(S_TERM_MASTER.BEGIN_DATE,'%m/%d/%Y'),'') AS TERM_BEGIN_DATE,
			IF(S_TERM_MASTER.END_DATE != '0000-00-00',DATE_FORMAT(S_TERM_MASTER.END_DATE,'%m/%d/%Y'),'') AS TERM_END_DATE,
			COURSE_DESCRIPTION, 
			IF(S_COURSE_OFFERING_SCHEDULE.START_DATE = '0000-00-00','',DATE_FORMAT(S_COURSE_OFFERING_SCHEDULE.START_DATE, '%m/%d/%Y' )) AS  SCHEDULE_START_DATE, 
			IF(S_COURSE_OFFERING_SCHEDULE.END_DATE = '0000-00-00','',DATE_FORMAT(S_COURSE_OFFERING_SCHEDULE.END_DATE, '%m/%d/%Y' )) AS  SCHEDULE_END_DATE
		FROM S_COURSE_OFFERING 
			LEFT JOIN M_COURSE_OFFERING_STATUS ON M_COURSE_OFFERING_STATUS.PK_COURSE_OFFERING_STATUS = S_COURSE_OFFERING.PK_COURSE_OFFERING_STATUS 
			LEFT JOIN M_ATTENDANCE_TYPE ON M_ATTENDANCE_TYPE.PK_ATTENDANCE_TYPE = S_COURSE_OFFERING.PK_ATTENDANCE_TYPE 
			LEFT JOIN M_SESSION ON M_SESSION.PK_SESSION = S_COURSE_OFFERING.PK_SESSION 
			LEFT JOIN S_TERM_MASTER ON S_TERM_MASTER.PK_TERM_MASTER = S_COURSE_OFFERING.PK_TERM_MASTER 
			LEFT JOIN S_EMPLOYEE_MASTER AS S_EMPLOYEE_MASTER_INST ON S_EMPLOYEE_MASTER_INST.PK_EMPLOYEE_MASTER = INSTRUCTOR 
			LEFT JOIN M_CAMPUS_ROOM ON M_CAMPUS_ROOM.PK_CAMPUS_ROOM = S_COURSE_OFFERING.PK_CAMPUS_ROOM
			LEFT JOIN S_COURSE_OFFERING_SCHEDULE ON S_COURSE_OFFERING_SCHEDULE.PK_COURSE_OFFERING = S_COURSE_OFFERING.PK_COURSE_OFFERING 
			,S_COURSE 
			WHERE 
			S_COURSE_OFFERING.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND 
			S_COURSE_OFFERING.PK_COURSE_OFFERING = '$PK_COURSE_OFFERING' AND 
			S_COURSE.PK_COURSE = S_COURSE_OFFERING.PK_COURSE
		";
	// dds($course_info_sql);
	return $db->Execute($course_info_sql);
}

function get_schedule_start_and_end_dates(&$course_info, $PK_COURSE_OFFERING)
{
	global $db;

	if ($course_info->fields['SCHEDULE_START_DATE'] == '') {
		$res_det = $db->Execute("select IF(MIN(SCHEDULE_DATE) = '0000-00-00','',DATE_FORMAT(MIN(SCHEDULE_DATE), '%m/%d/%Y' )) AS SCHEDULE_DATE FROM S_COURSE_OFFERING_SCHEDULE_DETAIL WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING = '$PK_COURSE_OFFERING'  ");
		$SCHEDULE_START_DATE = $res_det->fields['SCHEDULE_DATE'];
	} else
		$SCHEDULE_START_DATE = $course_info->fields['SCHEDULE_START_DATE'];

	if ($course_info->fields['SCHEDULE_END_DATE'] == '') {
		$res_det = $db->Execute("select IF(MAX(SCHEDULE_DATE) = '0000-00-00','',DATE_FORMAT(MAX(SCHEDULE_DATE), '%m/%d/%Y' )) AS SCHEDULE_DATE FROM S_COURSE_OFFERING_SCHEDULE_DETAIL WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING = '$PK_COURSE_OFFERING'  ");
		$SCHEDULE_END_DATE = $res_det->fields['SCHEDULE_DATE'];
	} else
		$SCHEDULE_END_DATE = $course_info->fields['SCHEDULE_END_DATE'];

	#get latest monday w.r.t. either DB's MIN SCHEDULE_DATE or USER Inputed START DATE
	if ($SCHEDULE_START_DATE == null) {
		if (isset($_REQUEST['START_DATE_NEW'])) {
			if ($_REQUEST['START_DATE_NEW'] != null || $_REQUEST['START_DATE_NEW'] != '') {
				$START_DATE = getThisWeeksMondayOrSelected($_REQUEST['START_DATE_NEW']);
			}
		} else {
			$START_DATE = null;
		}
	} else {

		if ((strtotime($_REQUEST['START_DATE_NEW']) >  strtotime($SCHEDULE_START_DATE)) && $_REQUEST['START_DATE_NEW'] != '') {
			$START_DATE = getThisWeeksMondayOrSelected($_REQUEST['START_DATE_NEW']);
		} else {
			$START_DATE = getThisWeeksMondayOrSelected($SCHEDULE_START_DATE);
		}
	}
	#get latest monday w.r.t. either DB's MAX SCHEDULE_DATE or USER Inputed END DATE

	if ($SCHEDULE_END_DATE == null) {
		if (isset($_REQUEST['AS_OF_DATE'])) {
			if ($_REQUEST['AS_OF_DATE'] != null || $_REQUEST['AS_OF_DATE'] != '') {
				$END_DATE = getThisWeeksFridayOrSelected($_REQUEST['AS_OF_DATE']);
			}
		} else {
			$END_DATE = null;
		}
	} else {
		if ((strtotime($_REQUEST['AS_OF_DATE']) <  strtotime($SCHEDULE_END_DATE)) && $_REQUEST['AS_OF_DATE'] != '') {
			$END_DATE = getThisWeeksFridayOrSelected($_REQUEST['AS_OF_DATE']);
		} else {
			$END_DATE = getThisWeeksFridayOrSelected($SCHEDULE_END_DATE);
		}
	}
	if($_REQUEST['START_DATE_NEW']){
		$START_DATE = getThisWeeksMondayOrSelected($_REQUEST['START_DATE_NEW']);
	}
	if($_REQUEST['AS_OF_DATE'] != ''){
		$END_DATE = getThisWeeksFridayOrSelected($_REQUEST['AS_OF_DATE']);
	}
	return [$START_DATE, $END_DATE];
}
function get_students_info($PK_COURSE_OFFERING)
{
	global $db;
	$students_query = "SELECT CONCAT(LAST_NAME,', ',FIRST_NAME, ' ', SUBSTRING(S_STUDENT_MASTER.MIDDLE_NAME, 1, 1)) as STUD_NAME, STUDENT_ID, S_STUDENT_COURSE.PK_STUDENT_ENROLLMENT,  PK_STUDENT_COURSE , IMAGE
					FROM
					S_STUDENT_COURSE,  S_STUDENT_MASTER, S_STUDENT_ACADEMICS   
					WHERE 
					S_STUDENT_COURSE.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND 
					S_STUDENT_COURSE.PK_STUDENT_MASTER = S_STUDENT_MASTER.PK_STUDENT_MASTER AND 
					S_STUDENT_MASTER.PK_STUDENT_MASTER = S_STUDENT_ACADEMICS.PK_STUDENT_MASTER AND 
					S_STUDENT_COURSE.PK_COURSE_OFFERING = '$PK_COURSE_OFFERING' 
					ORDER BY CONCAT(LAST_NAME,', ',FIRST_NAME, ' ',S_STUDENT_MASTER.MIDDLE_NAME) ASC 
					";
	return $students = $db->Execute($students_query);
}
function getThisWeeksMondayOrSelected($selectedDate)
{
	// Check if the selected date is a Monday
	if (date("N", strtotime($selectedDate)) == 1) {
		return $selectedDate; // Return the selected date if it's Monday
	} else if (date("N", strtotime($selectedDate)) == 7) {
		$pastMonday = date("Y-m-d", strtotime('next Monday', strtotime($selectedDate)));
		return $pastMonday; //Return closest monday
	} else {
		// Calculate the past Monday from the selected date
		$pastMonday = date("Y-m-d", strtotime('last Monday', strtotime($selectedDate)));
		return $pastMonday;
	}
}

function getThisWeeksFridayOrSelected($selectedDate)
{
	// Check if the selected date is a Friday (5 corresponds to Friday)
	if (date("N", strtotime($selectedDate)) == 5) {
		return $selectedDate; // Return the selected date if it's Friday
	} else {
		// Calculate this week's Friday from the selected date
		$thisWeeksFriday = date("Y-m-d", strtotime('this Friday', strtotime($selectedDate)));
		return $thisWeeksFriday;
	}
}
function get_student_rows(&$students_res)
{

	$PK_STUDENT_ENROLLMENTS = [];
	$PK_STUDENT_COURSES = [];
	$student_rows = [];
	$width = '252px';
	if ($_REQUEST['AR_DAYS'] == 'mon_fri') {
		$width = '282px';
	}
	while (!$students_res->EOF) {
		# code ... 
		$PK_STUDENT_ENROLLMENTS[] = $PK_STUDENT_ENROLLMENT 	= $students_res->fields['PK_STUDENT_ENROLLMENT'];
		$PK_STUDENT_COURSES[] = $PK_STUDENT_COURSE 		= $students_res->fields['PK_STUDENT_COURSE'];
		$student_rows[$PK_STUDENT_ENROLLMENT] =
			[
				'PK_STUDENT_ENROLLMENT' => $PK_STUDENT_ENROLLMENT,
				'PK_STUDENT_COURSE' => $PK_STUDENT_COURSE,
				'html' => '<tr>
			<td style="width:'.$width.' !important;text-align:left !important;padding : 5px;">' .
					$students_res->fields['STUDENT_ID'] . ' ' . $students_res->fields['STUD_NAME']
					. '</td>'
			];
		$students_res->MoveNext();
	}

	return [$PK_STUDENT_ENROLLMENTS, $PK_STUDENT_COURSES, $student_rows];
}
$CLASS_TIME_IN; 
$CLASS_TIME_OUT;
function check_valid_week($WEEK_START_DATE, $PK_STUDENT_ENROLLMENTS, $PK_STUDENT_COURSES)
{
	global $db;
	global $CLASS_TIME_IN;
	global $CLASS_TIME_OUT;
	global $PK_COURSE_OFFERING;
	$WEEK_END_DATE = date("Y-m-d", strtotime($WEEK_START_DATE));

	$check_week_sql = "SELECT SCHEDULE_DATE,START_TIME,END_TIME FROM S_COURSE_OFFERING_SCHEDULE_DETAIL WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING = '$PK_COURSE_OFFERING' AND SCHEDULE_DATE BETWEEN '$WEEK_START_DATE' AND '$WEEK_END_DATE' LIMIT 1";
	
	$check_week_res = $db->Execute($check_week_sql);
	$CLASS_TIME_IN = '';
	$CLASS_TIME_OUT = '';
	$CLASS_TIME_IN  = $check_week_res->fields['START_TIME'];
	$CLASS_TIME_OUT = $check_week_res->fields['END_TIME'];
	if ($check_week_res->RecordCount() > 0) {
		return true;
	} else {
		#check if data exists on S_STUDENT_SCHEDULE
		$check_week_sql_s = "SELECT SCHEDULE_DATE FROM S_STUDENT_SCHEDULE WHERE S_STUDENT_SCHEDULE.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND S_STUDENT_SCHEDULE.PK_STUDENT_ENROLLMENT IN (" . implode(',', $PK_STUDENT_ENROLLMENTS) . ") AND  S_STUDENT_SCHEDULE.PK_STUDENT_COURSE IN (" . implode(',', $PK_STUDENT_COURSES) . ") AND  SCHEDULE_DATE BETWEEN '$WEEK_START_DATE' AND '$WEEK_END_DATE' LIMIT 1";
		$check_week_res = $db->Execute($check_week_sql_s);
		if ($check_week_res->RecordCount() > 0) {
			return true;
		}

		// dds($check_week_sql , $check_week_sql_s);
		return false;
	}
}

function is_scheduled_date($DATE, $PK_STUDENT_ENROLLMENTS, $PK_STUDENT_COURSES)
{
	global $db;
	global $PK_COURSE_OFFERING;

	$check_week_sql = "SELECT SCHEDULE_DATE FROM S_COURSE_OFFERING_SCHEDULE_DETAIL WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_COURSE_OFFERING = '$PK_COURSE_OFFERING' AND SCHEDULE_DATE = '$DATE' LIMIT 1";
	$check_week_res = $db->Execute($check_week_sql);
	if ($check_week_res->RecordCount() > 0) {
		return true;
	} else {
		#check if data exists on S_STUDENT_SCHEDULE
		$check_week_sql_s = "SELECT SCHEDULE_DATE FROM S_STUDENT_SCHEDULE WHERE S_STUDENT_SCHEDULE.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND S_STUDENT_SCHEDULE.PK_STUDENT_ENROLLMENT IN (" . implode(',', $PK_STUDENT_ENROLLMENTS) . ") AND  S_STUDENT_SCHEDULE.PK_STUDENT_COURSE IN (" . implode(',', $PK_STUDENT_COURSES) . ") AND  SCHEDULE_DATE = '$DATE' LIMIT 1";
		$check_week_res = $db->Execute($check_week_sql_s);
		if ($check_week_res->RecordCount() > 0) {

			return true;
		}

		// dds($check_week_sql , $check_week_sql_s);
		return false;
	}
}

function add_week_data_to_std_rows(
	$START_DATE,
	&$count_of_week,
	&$student_rows,
	&$course_code_row_with_date_ranges_of_week,
	&$language_row_with_date_numbers,
	&$term_start_row_with_s_m_t_w_t_f_s,
	$PK_STUDENT_ENROLLMENTS,
	$PK_STUDENT_COURSES
) {
	global $db;
	/* Styles Setting */
	if ($_REQUEST['AR_DAYS'] == 'mon_fri') {
		$header_size = '182px;';
		$dt_range_width = '256px !important;';
	} else {
		$header_size = '182px;';
		$dt_range_width = '256px !important;';
	}


	$colspan = $_REQUEST['AR_DAYS'] == 'mon_fri' ? 5 : 7;
	$course_code_row_with_date_ranges_of_week .= '<td style=" width :  ' . $dt_range_width . ' ; padding : 5px; border : 2px solid black;font : 15px !important " colspan="' . $colspan . '"> Week '.$count_of_week.' : ' . date("M d, Y", strtotime($START_DATE)) . ' - ' . date("M d, Y", strtotime($START_DATE . ' + 6 days')) . '</td>';
	$count_of_week++;
	$END_DATE = date("Y-m-d", strtotime($START_DATE . ' + 6 days'));
	$first_flag = true;
	// dump('student_rows b4' , $student_rows);

	for ($i = 0; $i <= 0; $i++) {
		echo ">> <b> $i  </b><<";
		$DATE = date("Y-m-d", strtotime($START_DATE . ' + ' . $i . ' days'));
		if (
			!($_REQUEST['AR_DAYS'] == 'mon_fri'
				&&
				in_array(strtolower(date("l", strtotime($START_DATE . ' + ' . $i . ' days'))), ['sunday', 'saturday'])
			)
			&& (strtotime($DATE) <= strtotime($END_DATE))
		) {
			$styler = " ";
			if (is_scheduled_date($DATE, $PK_STUDENT_ENROLLMENTS, $PK_STUDENT_COURSES)) {
				$styler = ' style="border : 2px solid black;background : #cbcbcb" ';
			} else {
				$styler = " style='border : 2px solid black;' ";
			}
			$language_row_with_date_numbers .= "<td $styler  class='att_field'>Hi1" . (date('M j', strtotime($DATE))) . "</td>";
			$term_start_row_with_s_m_t_w_t_f_s .= "<td $styler  class='att_field'>Hi2" . (date('l', strtotime($DATE))) . "</td>";
		}
	}
	for ($i = 0; $i <= 0; $i++) {
		$DATE = date("Y-m-d", strtotime($START_DATE . ' + ' . $i . ' days'));

		foreach ($student_rows as $row_key => $student_row) {

			if (
				!($_REQUEST['AR_DAYS'] == 'mon_fri'
					&&
					in_array(strtolower(date("l", strtotime($START_DATE . ' + ' . $i . ' days'))), ['sunday', 'saturday'])
				)
				&& (strtotime($DATE) <= strtotime($END_DATE))
			) {


				$PK_STUDENT_ENROLLMENT = $student_row['PK_STUDENT_ENROLLMENT'];
				$PK_STUDENT_COURSE = $student_row['PK_STUDENT_COURSE'];
				$res = $db->Execute("select ATTENDANCE_HOURS, S_STUDENT_ATTENDANCE.COMPLETED, M_ATTENDANCE_CODE.CODE  from S_STUDENT_ATTENDANCE, M_ATTENDANCE_CODE, S_STUDENT_SCHEDULE WHERE S_STUDENT_SCHEDULE.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND S_STUDENT_SCHEDULE.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT' AND PK_STUDENT_COURSE = '$PK_STUDENT_COURSE' AND SCHEDULE_DATE = '$DATE' AND S_STUDENT_ATTENDANCE.PK_STUDENT_SCHEDULE = S_STUDENT_SCHEDULE.PK_STUDENT_SCHEDULE AND M_ATTENDANCE_CODE.PK_ATTENDANCE_CODE = S_STUDENT_ATTENDANCE.PK_ATTENDANCE_CODE  AND (S_STUDENT_ATTENDANCE.COMPLETED = 1 OR PK_SCHEDULE_TYPE = 2)");

				$ATTENDANCE_HOURS = '';
				if ($res->RecordCount() > 0) {
					if ($res->fields['CODE'] == 'I')
						$ATTENDANCE_HOURS = 'I';
					else
						{ 
							$ATTENDANCE_HOURS = number_format($res->fields['ATTENDANCE_HOURS'], 2);
							// $student_rows[$row_key]['total_hours'] = $student_rows[$row_key]['total_hours'] + $res->fields['ATTENDANCE_HOURS'];
						}
				}

				/* Ticket # 1270 */
				if ($ATTENDANCE_HOURS === '0.00') {
					// $ATTENDANCE_HOURS = '<table width="100%" ><tr><td style="border: 1px solid #FF0000;" >' . $ATTENDANCE_HOURS . '</td></tr></table>';
					$ATTENDANCE_HOURS = '<td width="36px" style="border: 2px solid #FF0000; border-collapse: collapse; "  class="att_field">' . $ATTENDANCE_HOURS . '</td>';
				} else {

					$ATTENDANCE_HOURS = '<td class="att_field" width="36px">' . $ATTENDANCE_HOURS . '</td>'; 
					$student_rows[$row_key]['total_hours'] = $student_rows[$row_key]['total_hours'] + $res->fields['ATTENDANCE_HOURS'];
				
				}
				// dump('before',$student_row);
				$student_rows[$row_key]['html'] = $student_rows[$row_key]['html'] . ' ' . $ATTENDANCE_HOURS;
			 
				// dump('$ATTENDANCE_HOURS' , $ATTENDANCE_HOURS);
				// dump('student_row after' , $student_row);
				/* Ticket # 1270 */
			}
		}
		// print_r($student_rows );exit;
		$first_flag = false;
	}
	// dump('Changed rows AFTER', $student_rows);
	return $student_row;
}


function printwkhtml($html_to_print , $JAVASCRIPT_COURSE_OFFERING_HEADERS)
{
	$header = invoice_pdf_custom_header();
	$footer = pdf_footer();



	$header_cont = '<!DOCTYPE HTML>
						<html>
						
						<head>
							<style>
								div {
									padding-bottom: 20px !important;
								}
								 
								body,html {
									padding: 0;
									margin: 0;
									font-size:0;
								}
								table { page-break-inside:auto }
								tr    { page-break-inside:avoid; page-break-after:auto }
								thead { display:table-header-group }
								tfoot { display:table-footer-group }
							</style>
						</head>
						
						<body>
						'.$header.'
						<div class="custom_header" style="font-size : 10px !important"></div> 
						</body>
						<script>
							var vars = {};
							var x = window.location.search.substring(1).split("&");
							for (var i in x) {
								var z = x[i].split("=", 2);
								vars[z[0]] = unescape(z[1]);
							}
							var dynamic_headers = []; ';
							

foreach ($JAVASCRIPT_COURSE_OFFERING_HEADERS as $course_offering => $header_html_content) {
	$no_spaces_allowed = str_replace(array("\r", "\n"), '', $header_html_content);   
	$header_cont.= " dynamic_headers['$course_offering'] = '<br><br>$no_spaces_allowed' ;";

}

							$header_cont.= '

							//document.getElementById("custom_header").innerHTML = vars.section;
							document.getElementsByClassName("custom_header")[0].innerHTML = dynamic_headers[vars.section];
						</script>
						</html>';

	$footer_cont = '
						<!DOCTYPE HTML>
						<html>
						
						<head>
							<style>
								tbody td {
									font-size: 14px !important;
								}
								body,html {
									padding: 0;
									margin: 0;
									font-size: 0;
								}
								table { page-break-inside:auto }
								tr    { page-break-inside:avoid; page-break-after:auto }
								thead { display:table-header-group }
								tfoot { display:table-footer-group }
							</style>
						</head>
						
						<body> <span></span>' . $footer . '
						
							<script>
								var vars = {};
								var x = window.location.search.substring(1).split("&");
								for (var i in x) {
									var z = x[i].split("=", 2);
									vars[z[0]] = unescape(z[1]);
								}
								document.getElementById("page").innerHTML = vars.sitepage;
								document.getElementById("topage").innerHTML = vars.sitepages;
							</script>
						</body>
						
						</html>';
							
	if(isset($_REQUEST['PRECALCULATED_FOOTER_HEIGHT'])){
		$footer_height = $_REQUEST['PRECALCULATED_FOOTER_HEIGHT'].'mm';
	}else{
		$footer_height = '20mm';
	}


	$header_path = create_html_file('header_attendance_daily_attendance_sheet.html', $header_cont, "invoice");
	$footer_path = create_html_file('footer_attendance_daily_attendance_sheet.html', $footer_cont, "invoice");
	$file_name = "Monthly_Attendance_Analysis_".uniqid().".pdf";
	$exec = 'xvfb-run -a wkhtmltopdf -T 0 -R 0 -B 0 -L 0 --enable-local-file-access --orientation landscape --page-size A4 --page-width 296  --page-height 210  --margin-top 62mm --margin-left 7mm --margin-right 5mm  --margin-bottom '.$footer_height.' --footer-font-size 8 --header-html ' . $header_path . ' --footer-html  ' . $footer_path . ' ' . $html_to_print . ' ../school/temp/invoice/' . $file_name . ' 2>&1'; 
	global $http_path;
	$pdfdata = array('filepath' => 'temp/invoice/' . $file_name, 'exec' => $exec, 'filename' => $file_name, 'filefullpath' => $http_path . 'school/temp/invoice/' . $file_name);
	exec($pdfdata['exec'], $output, $retval); 
	header('Content-type: application/json; charset=UTF-8');
	$data_res = [];
	$data_res['path'] = 'temp/invoice/' . $file_name;
	echo json_encode($data_res);

	unlink('../school/temp/invoice/header_attendance_daily_attendance_sheet.html');
	unlink('../school/temp/invoice/footer_attendance_daily_attendance_sheet.html');
	exit;
}

function prepare_html_for_print($Output_Per_Offering, $COURSE_OFFERING_HEADERS , $total_table_flag = false)
{
 
 
	$multi_part_command = ' ';
	$Total_table_html = '';
	$section_indexer = 1;
	foreach ($Output_Per_Offering as $PK => $Course_offering) {
		
		$new_table = '';
		$Total_table_html .= $COURSE_OFFERING_HEADERS[$PK];
		$new_table .= $COURSE_OFFERING_HEADERS[$PK];
		 
			$new_table .= $Course_offering; 
			
		
		$new_content =  prepare_sections($new_table);
		$path = create_html_file('content_pdf_'.$section_indexer.'.html', $new_content, "invoice"); 
		$multi_part_command .= " $path ";
		$section_indexer++;
	} 
	// print_r($Output_Per_Offering);exit; 
	// dd($multi_part_command);
	return $multi_part_command;
}



function prepare_sections($section_content){

	return $html_body_cont = '
	<!DOCTYPE HTML>
	<html>
		<head>
			<style>
				.new-page {
				page-break-before: always;
				}
				body,html {
				padding: 0 !important;
				margin: 0 !important; 
				}
				body {
				font-size: 21px;
				}
				table{
				table-layout: fixed;
				}
				table td{
				table-layout: fixed;
				}
				td:not(:first-child){
				max-width : 74px !important;
				}
				td:first-child{
				max-width : 252px !important;
				}
				tr:first-child{
				padding : 10px !important;
				}
				table tr {
				padding-top: 15px !important;
				border-bottom : solid 1px black 
				}
				.att_field{ width : 74px !important; height : 37px !important; padding : 0px !important} 
				td{
				
				/* padding : 5px !important; */
				font-size : 12px !important; 
				} 
				thead { display: table-header-group; }
				tfoot { display: table-row-group; }
				tr { page-break-inside: avoid; }
				thead { display: table-header-group; }
				tfoot { display: table-row-group; }
				tr { page-break-inside: avoid; }
			</style>
		</head>
		<body>'.$section_content.'</body></html>';

}


function print_selected_header($prepared_header_array){

}

?>