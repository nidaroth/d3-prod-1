...
/////////////////////////////////////////////////////////////////
        ...
        $file_name 		= 'Units_Attempted_By_Term_'.time().'.xlsx';
        ...
        //echo $wh_cond; die;
    $sql_query ="SELECT CONCAT(IF(CO_TERM.BEGIN_DATE = '0000-00-00','',DATE_FORMAT(CO_TERM.BEGIN_DATE, '%m/%d/%Y' )),'-',IF(CO_TERM.END_DATE = '0000-00-00','',DATE_FORMAT(CO_TERM.END_DATE, '%m/%d/%Y' ))) AS COURSE_TERM_DATE
,CONCAT(S.LAST_NAME, ', ', S.FIRST_NAME) AS STUDENT
,COALESCE(SA.STUDENT_ID,'NO STUDENT ID') AS STUDENT_ID
,SC.CAMPUS_CODE
...
,SUM((CASE WHEN G.UNITS_COMPLETED = 1 THEN C.FA_UNITS  ELSE  0 END)) AS FA_UNITS_COMPLETED
...
,SUM((CASE WHEN G.UNITS_COMPLETED = 1 THEN C.HOURS  ELSE  0 END)) AS HOURS_COMPLETED
...
,SS.STUDENT_STATUS";	//exit;
        ...
        if(count($res->fields) == 0){
            ...
            header('Location:units_attempted_by_term?m=1');
        }else{
        $heading = array_keys($res->fields);		
       // print_r($heading); exit;
		$heading_d[]='Course Term Date';
        $heading_d[]= 'Student Name';
        $heading_d[]= 'Student ID';
        $heading_d[]= 'Campus';   
        $heading_d[]= 'First Term';
		$heading_d[]= 'Program';
		$heading_d[]= 'Status';
		$heading_d[]= 'Enrollment Status';
	    $heading_d[]= 'FA Units Attempted';
	    $heading_d[]= 'FA Units Completed';
        $heading_d[]= 'Units Attempted';
        $heading_d[]= 'Units Completed';
      	$heading_d[]= 'Hours Attempted';
        $heading_d[]= 'Hours Completed';

        foreach ($heading_d as $key) 
        {
        ...
        while (!$res->EOF){
            ...
            foreach ($heading as $key) 
            {
                ...
                // $cellValue =  str_replace("<BOLD>","",$cellValue);
					...
					if($c==13){
					$cellValue = number_format_value_checker( $studentdata['ATTENDACE_HOURS'] , 2);
