...
if(!empty($_POST)){
	...
	if (trim($_FILES['FILE_XLX']['name'])!= ""){
		...
		if(strtolower($extn[$ii]) == 'xlsx' || strtolower($extn[$ii]) == 'xls' || strtolower($extn[$ii]) == 'csv'){ 	
			$newfile1 = 'temp/ar_code_'.date("Y-m-d-H-i-s").'.'.$extn[$ii];
			...
			foreach($sheetData as $row){
				...
				header("location:ar_leder_code_upload_mapping?id=".$PK_MAP_MASTER);
				exit;
			}
		}else{
...
<head>
	...
	<title><?=AR_LEDGER_CODE_PAGE_TITLE.' '.UPLOAD?> | <?=$title?></title>
</head>
            ...
            <div class="container-fluid">
                 <div class="row page-titles">
                    <div class="col-md-12 align-self-center">
                        <h4 class="text-themecolor"><?=AR_LEDGER_CODE_PAGE_TITLE.' '.UPLOAD?> </h4>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
