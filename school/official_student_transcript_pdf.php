...
function student_transcript_pdf($PK_STUDENT_MASTERS, $one_stud_per_pdf, $report_name){
	...
	foreach($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
		...
		/* Ticket # 1588 */
								...
								$find_date = ["Exp Grad Date", "Grad Date"];
								$replace_date   = ["Conferred Date","Conferred Date"];
								...
								while (!$res_type->EOF) {
									...
									$txt .= '<tr>
											...
											<tr>
												...
												$STUDENT_SATUS_SQL = "SELECT S_STUDENT_MASTER.PK_STUDENT_MASTER,STUDENT_STATUS,M_STUDENT_STATUS.PK_STUDENT_STATUS FROM S_STUDENT_MASTER LEFT JOIN S_STUDENT_ENROLLMENT ON S_STUDENT_ENROLLMENT.PK_STUDENT_MASTER = S_STUDENT_MASTER.PK_STUDENT_MASTER LEFT JOIN M_ENROLLMENT_STATUS ON M_ENROLLMENT_STATUS.PK_ENROLLMENT_STATUS = S_STUDENT_ENROLLMENT.PK_ENROLLMENT_STATUS LEFT JOIN M_STUDENT_STATUS ON M_STUDENT_STATUS.PK_STUDENT_STATUS = S_STUDENT_ENROLLMENT.PK_STUDENT_STATUS WHERE S_STUDENT_MASTER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND S_STUDENT_ENROLLMENT.PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT ='$PK_STUDENT_ENROLLMENT' AND COMPLETED=1"; 
												$res_status = $db->Execute($STUDENT_SATUS_SQL);
												$student_status = $res_status->fields['PK_STUDENT_STATUS'];
												
												
												if(!empty($student_status)){
												$txt .= '<td  width="34%" >
													'.str_replace($find_date, $replace_date,transcript_header($res_report_header->fields['BOX_5'], " AND S_STUDENT_ENROLLMENT.PK_STUDENT_ENROLLMENT = '$PK_STUDENT_ENROLLMENT' ")).'
												</td>';
												}else{
													$txt .= '<td  width="34%" >
		...
		$txt .= '<div style="border-top: 2px #c0c0c0 solid" ></div>
				...
				$txt .= '<td width="10%" align="right" ><b>Units</b><br /><u><b>Attempted</b></u></td>
					...
					while (!$res_term->EOF) {
						...
						/* ticket #1240 */
						$txt .= '<tr>
								<td width="'.$width_1.'%" style="border-top:1px solid #c0c0c0" align="right" ><i>Term '.$BEGIN_DATE.' Total: </i></td>';
						...
						<tr>
							<td width="'.$width_1.'%" colspan="2" align="right" ><i><b style="font-size:35px">Cumulative Total: </b></i></td>';
