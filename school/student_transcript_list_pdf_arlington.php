...
class MYPDF extends TCPDF {
    ...
    public function Footer() {
		...
		$res_type = $db->Execute("SELECT FOOTER_LOC, CONTENT FROM S_PDF_FOOTER,S_PDF_FOOTER_CAMPUS WHERE S_PDF_FOOTER.ACTIVE = 1 AND S_PDF_FOOTER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PDF_FOR = 5 AND S_PDF_FOOTER.PK_PDF_FOOTER = S_PDF_FOOTER_CAMPUS.PK_PDF_FOOTER AND PK_CAMPUS = '$PK_CAMPUS'  ");
...
function student_transcript_list_pdf($PK_STUDENT_MASTERS, $one_stud_per_pdf, $report_name){
	...
	$STUDENTS_HAVING_NA_PROGRAMS = $db->Execute("SELECT GROUP_CONCAT(PK_STUDENT_MASTER) AS PK_STUDENT_MASTERS  FROM S_STUDENT_ENROLLMENT WHERE PK_ACCOUNT = '".$_SESSION['PK_ACCOUNT']."' AND PK_CAMPUS_PROGRAM IN ($NA_PROGRAMS) AND PK_STUDENT_MASTER IN ($PK_STUDENT_MASTERS) GROUP BY PK_ACCOUNT")->fields['PK_STUDENT_MASTERS']; 
	...
	foreach($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER) {
		...
		if($_GET['eid'] != ''){
			...
			$PK_STUDENT_ENROLLMENTS_OF_CURRENT_PK_STUDENT_MASTER = $db->Execute("SELECT GROUP_CONCAT(PK_STUDENT_ENROLLMENT) AS PK_STUDENT_ENROLLMENTS  FROM S_STUDENT_ENROLLMENT WHERE PK_ACCOUNT = '".$_SESSION['PK_ACCOUNT']."' AND PK_CAMPUS_PROGRAM IN ($NA_PROGRAMS) AND PK_STUDENT_MASTER IN ($PK_STUDENT_MASTER) GROUP BY PK_ACCOUNT")->fields['PK_STUDENT_ENROLLMENTS']; 
		...
		$txt .= '<div style="border-top: 2px #c0c0c0 solid" ></div>
				<table border="0" cellspacing="0" cellpadding="3" width="100%" >
					<tr>
						<td width="100%" align="center" ><b><i style="font-size:50px">'.$report_name.'</i></b><br /></td>
					</tr>
					<tr>
						<td width="18%" ><br /><br /><u><b>Term</b></u></td>
						<td width="10%" ><br /><br /><u><b>Course</b></u></td>
						<td width="30%" ></td>';
						
						if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 1 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
							$txt .= '<td width="7%" ><br /><br /><u><b>Grade</b></u></td>';

						if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 2 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
							$txt .= '<td width="17%" align="right" ><b>Clock</b><br /><u><b>Hours</b></u></td>';
						
				/*$txt .= '<td width="10%" align="right" ><b>Units</b><br /><u><b>Attempted</b></u></td>
						<td width="10%" align="right" ><b>Units</b><br /><u><b>Completed</b></u></td>';*/
						$txt .= '<td width="20%" align="right" ><br /><br /><u><b>GPA</b></u></td>
					</tr>';
					/* ticket #1240 */
					...
					if($_GET['exclude_tc'] != 1) {
						...
						while (!$res_tc->EOF) {
							...
							$txt .= '<tr>
										...
										<td width="10%" >'.$res_tc->fields['TRANSCRIPT_CODE'].'</td>
										<td width="30%" >'.$res_tc->fields['COURSE_DESCRIPTION'].'</td>';
									
									if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 1 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
										$txt .= '<td width="7%" >'.$res_tc->fields['GRADE'].'</td>';
										
									if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 2 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
										$txt .= '<td width="17%" align="right" >'.$res_tc->fields['HOURS'].'</td>';
								//DIAM-781
								/*$txt .= '<td width="10%" align="right" >'.number_format_value_checker($ATTEMPTED_UNITS,2).'</td>
										<td width="10%" align="right" >'.number_format_value_checker($COMPLETED_UNITS,2).'</td>'; */
										$txt .= '<td width="20%" align="right" >'.number_format_value_checker(($TC_GPA_VALULE/$TC_GPA_WEIGHT),2).'</td>
									</tr>';
							/* ticket #1240 */
					...
					while (!$res_course->EOF) { 
						...
						/* ticket #1240 */
						$txt .= '<tr>
									<td width="18%" >'.$BEGIN_DATE.'</td>
									<td width="10%" >'.$res_course->fields['TRANSCRIPT_CODE'].'</td>
									<td width="30%" >'.$res_course->fields['COURSE_DESCRIPTION'].'</td>';
									
									if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 1 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
										$txt .= '<td width="7%" >'.$res_course->fields['GRADE'].'</td>';
									
									if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 2 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
										$txt .= '<td width="17%" align="right" >'.$res_course->fields['HOURS'].'</td>';
									
							/*$txt .= '<td width="10%" align="right" >'.number_format_value_checker($ATTEMPTED_UNITS,2).'</td>
									<td width="10%" align="right" >'.number_format_value_checker($COMPLETED_UNITS,2).'</td>'; */
									$txt .= '<td width="20%" align="right" >'.number_format_value_checker(($GPA_VALULE/$GPA_WEIGHT),2).'</td>
								</tr>';
						/* ticket #1240 */
			...
			/* ticket #1240 */
			$txt .= '<tr>
						<td width="58%" style="border-top:0.5px solid #c0c0c0" align="right"><i><b style="font-size:30px">Student Transcript Total</b></i></td>';
						
						if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 1 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
							$txt .= '<td width="7%" style="border-top:0.5px solid #c0c0c0" ></td>';
						
						if($res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 2 || $res_sp_set->fields['GRADE_DISPLAY_TYPE'] == 3)
							$txt .= '<td width="17%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker(($total_clock_hours),2).'</b></i></td>';
						//DIAM-781
				/*$txt .= '<td width="10%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker($c_in_att_tot,2).'</b></i></td>
						<td width="10%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker($c_in_comp_tot,2).'</b></i></td>';*/
						$txt .= '<td width="20%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker(($gpa_weighted),2).'</b></i></td>
					</tr>
					</table> ';
					/* ticket #1240 */
...
if($_GET['id'] == '') {
	...
	$report_name = "Student Transcript List";
	...
	if($_GET['zip'] == 1) {
		...
		if($res === TRUE) {
			...
			$STUDENTS_HAVING_NA_PROGRAMS_ZIP = $db->Execute("SELECT GROUP_CONCAT(PK_STUDENT_MASTER) AS PK_STUDENT_MASTERS  FROM S_STUDENT_ENROLLMENT WHERE PK_ACCOUNT = '".$_SESSION['PK_ACCOUNT']."' AND PK_CAMPUS_PROGRAM IN ($NA_PROGRAMS) AND PK_STUDENT_MASTER IN ($PK_STUDENT_MASTERS_ZIP) GROUP BY PK_ACCOUNT")->fields['PK_STUDENT_MASTERS']; 
		...
		student_transcript_list_pdf($_GET['id'], 0, $report_name);
}
