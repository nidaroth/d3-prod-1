...
$rs = mysql_query("SELECT DISTINCT(PK_TEXT_TEMPLATE) FROM S_TEXT_TEMPLATE WHERE " . $where. " ")or die(mysql_error());
$row = mysql_fetch_row($rs);
$result["total"] = mysql_num_rows($rs);
	
$query = "SELECT PK_TEXT_TEMPLATE,TEMPLATE_NAME,IF(ACTIVE = 1,'<i class=\'fa fa-square round_green icon_size_active\' ></i>','<i class=\'fa fa-square round_red icon_size_active\' ></i>') AS ACTIVE FROM S_TEXT_TEMPLATE WHERE " . $where ." order by $sort $order " ;
// echo $query;exit;	
