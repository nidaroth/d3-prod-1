...
if(!empty($_POST)){
	...
	$EVENT_OTHER = $_POST;
	if($_GET['id'] == ''){
		$EVENT_OTHER['TYPE']  	  	= $_GET['t'];
		$EVENT_OTHER['PK_ACCOUNT']  = $_SESSION['PK_ACCOUNT'];
		$EVENT_OTHER['CREATED_BY']  = $_SESSION['PK_USER'];
		$EVENT_OTHER['CREATED_ON']  = date("Y-m-d H:i");
		db_perform('M_EVENT_OTHER', $EVENT_OTHER, 'insert');
	} else {
		$EVENT_OTHER['EDITED_BY'] = $_SESSION['PK_USER'];
		$EVENT_OTHER['EDITED_ON'] = date("Y-m-d H:i");
		db_perform('M_EVENT_OTHER', $EVENT_OTHER, 'update'," PK_EVENT_OTHER = '$_GET[id]' AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND TYPE = '$_GET[t]' ");
	}
...
if($_GET['id'] == ''){
	...
	$res = $db->Execute("SELECT * FROM M_EVENT_OTHER WHERE PK_EVENT_OTHER = '$_GET[id]' AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND TYPE = '$_GET[t]' "); 
            ...
            <div class="container-fluid">
                 <div class="row page-titles">
                    <div class="col-md-12 align-self-center">
                        <h4 class="text-themecolor"><? if($_GET['id'] == '') echo ADD; else EDIT; ?> <? if($_GET['t'] == 1) echo TASK_OTHER_PAGE_TITLE; else if($_GET['t'] == 2) echo EVENT_OTHER_PAGE_TITLE; ?></h4>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
