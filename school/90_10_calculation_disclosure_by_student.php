...
if(!empty($_POST)){
	...
	$res = $db->Execute("CALL ACCT90102(".$_SESSION['PK_ACCOUNT'].", '".$campus_id."', '".$ST."','".$ET."', $BY_PROGRAM)");
	...
	if($_POST['RUN_BY'] == 1 || $_POST['RUN_BY'] == 2){
		$res = $db->Execute("SELECT DISTINCT(PROGRAM) as PROGRAM FROM S_TEMP_ACCT90102 WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND BATCH_ID = '$BATCH_ID' ORDER BY PROGRAM ASC");
		while (!$res->EOF) {
	...
	if($_POST['FORMAT'] == 1) {
		...
		class MYPDF extends TCPDF {
			public function Header() {
				...
				$this->SetX(121);
				$this->SetTextColor(000, 000, 000);
				$this->Cell(55, 8, "90/10 Calculation Disclosure by Student", 0, false, 'L', 0, '', 0, false, 'M', 'L');
			...
			public function Footer() {
				...
				$this->SetY(-15);
				$this->SetX(100);
				$this->Cell(30, 10, 'ACCT90103', 0, false, 'C', 0, '', 0, false, 'T', 'M');
			}
		}
		...
		foreach($PROGRAM_ARR as $PROGRAM){
			...
			$CATEGORY_ARR = array();
			$res = $db->Execute("SELECT DISTINCT(CATEGORY) as CATEGORY FROM S_TEMP_ACCT90102 WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND BATCH_ID = '$BATCH_ID' $prog_cond ORDER BY CATEGORY ASC");
			while (!$res->EOF) {
			...
			foreach($CATEGORY_ARR as $CATEGORY){
				$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%">
							<thead>
								...
								<tr>
									<td width="20%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><b>Student</b></td>
									<td width="20%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><b>Student ID</b></td>
									<td width="20%" style="border-top:1px solid #000;border-bottom:1px solid #000;" ><b>Status</b></td>
				...
				$sub_total = 0;
				$res_led = $db->Execute("SELECT * FROM S_TEMP_ACCT90102 WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND BATCH_ID = '$BATCH_ID' $prog_cond AND CATEGORY = '$CATEGORY' ORDER BY CONCAT(STUDENT_LAST_NAME,', ',STUDENT_FIRST_NAME) ASC");
				while (!$res_led->EOF) {
					...
					$txt 	.= '<tr>
								<td width="20%" >'.$res_led->fields['STUDENT_LAST_NAME'].', '.$res_led->fields['STUDENT_FIRST_NAME'].'</td>
								<td width="20%" >'.$res_led->fields['STUDENT_ID'].'</td>
								<td width="20%" >'.$res_led->fields['STUDENT_STATUS'].'</td>
		...
		$db->Execute("DELETE FROM S_TEMP_ACCT90102 WHERE BATCH_ID = '$BATCH_ID' AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
		
		$file_name = '90_10 Calculation Disclosure By Student'.'.pdf';
		/*if($browser == 'Safari')
		...
		$file_name 		= '90_10 Calculation Disclosure By Student.xlsx';
		...
		if($_POST['RUN_BY'] == 1 || $_POST['RUN_BY'] == 2)
			...
			while (!$res->EOF) {
				...
				while (!$res_led->EOF) {
						...
						$index++;
						$cell_no = $cell[$index].$line;
						$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($res_led->fields['STUDENT_LAST_NAME'].', '.$res_led->fields['STUDENT_FIRST_NAME']);

						$index++;
						$cell_no = $cell[$index].$line;
						$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($res_led->fields['STUDENT_ID']);

						$index++;
						$cell_no = $cell[$index].$line;
						$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue($res_led->fields['STUDENT_STATUS']);
...
<head>
	...
	<title><?=MNU_90_10_CALCULATION_DISCLOSURE_BY_STUDENT ?> | <?=$title?></title>
	<style>
                 ...
                 <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h4 class="text-themecolor">
							<?=MNU_90_10_CALCULATION_DISCLOSURE_BY_STUDENT ?>
						</h4>
                    </div>
                </div>
