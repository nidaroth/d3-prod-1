...
$file_name 		= 'Attendance Make Up Hours.xlsx';
...
while (!$res_stud->EOF) {
	...
	while (!$res_course_schedule->EOF) {
		...
		$index++;
		$cell_no = $cell[$index].$line;
		$objPHPExcel->getActiveSheet()->getCell($cell_no)->setValue(number_format_value_checker($res_course_schedule->fields['ATTENDANCE_HOURS'],2));
