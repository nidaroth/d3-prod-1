	...
	if ($_POST['FORMAT'] == 1) {
		...
		*/
		...
		foreach ($PK_COURSE_OFFERING_ARR as $PK_COURSE_OFFERING) {
			...
			$COURSE_OFFERING_HEADERS[$PK_COURSE_OFFERING] = '<h1 style="font-size:0px !important">'.$PK_COURSE_OFFERING.'</h1><br /><br />';
...
$border2px = '; border : 2px solid black !important;border-collapse: collapse; padding:3px;padding-top:7px;padding-bottom:7px;';
			...
			<tr> 
	<td width="16.66%" style="'.$border2px.'"><b> Course  :   '. $course_info->fields['COURSE_CODE'] . '(' . $course_info->fields['SESSION'][0] . '-' . $course_info->fields['SESSION_NO'] . ')</b><br>'.$course_info->fields['COURSE_DESCRIPTION'].'</td>
	<td width="16.66%" style="'.$border2px.';"><b> Term Dates  :</b> <br> '.$course_info->fields['TERM_BEGIN_DATE'].' - '.$course_info->fields['TERM_END_DATE'].' </td>
	
	<td width="16.66%" style="'.$border2px.'"><b>Instructor: </b> <br>' . ($course_info->fields['INSTRUCTOR_NAME']) .'</td>
	<td width="7%" style="'.$border2px.'"><b>Room: </b><br>' . $course_info->fields['ROOM_NO'] . '</td>
	<td width="16.66%" style="'.$border2px.'; "><b>Attendance : </b><br>'.$course_info->fields['ATTENDANCE_TYPE'].'</td>';
...
while (!$res_grouped_schedule->EOF) {
    ...
    $secondary_line_of_scheduled_hours .= '<td style="width : 25%; '.$border1px.' text-align : left !important;padding-left:15px;"><b>Course : </b> '.$res_grouped_schedule->fields['G_START_DATE'].' to '. $res_grouped_schedule->fields['G_END_DATE'].' - '.$res_grouped_schedule->fields['G_TOTAL_HOURS'].' hours - '.$res_grouped_schedule->fields['G_TOTAL_MEETINGS'].' meetings
	<br>
	<b>Class : </b> '.$res_grouped_schedule->fields['START_TIME'].' to '. $res_grouped_schedule->fields['END_TIME'].' - '.$res_grouped_schedule->fields['HOURS'].' hours </td>';
    ...
    for ($itemCount = 1; $itemCount < 3; $itemCount++) {
        ...
        if (!$res_grouped_schedule->EOF) {
            ...
            $secondary_line_of_scheduled_hours .= '<td style="width : 25%; '.$border1px.' text-align : left !important;padding-left:15px; "><b>Course : </b> '.$res_grouped_schedule->fields['G_START_DATE'].' to '. $res_grouped_schedule->fields['G_END_DATE'].' - '.$res_grouped_schedule->fields['G_TOTAL_HOURS'].' hours - '.$res_grouped_schedule->fields['G_TOTAL_MEETINGS'].' meetings
			<br>
			<b>Class : </b> '.$res_grouped_schedule->fields['START_TIME'].' to '. $res_grouped_schedule->fields['END_TIME'].' - '.$res_grouped_schedule->fields['HOURS'].' hours
			</td>';
            ...
            $secondary_line_of_scheduled_hours .= '<td style="width : 25%; '.$border1px.' text-align : left !important;padding-left:15px; "></td>';
        }
    }
			...
			if (!empty($PK_STUDENT_ENROLLMENTS) && !empty($PK_STUDENT_COURSES)) {
				...
				while (strtotime($DATE_ITERATOR) < strtotime($END_DATE)) {
					...
					if (check_valid_week($DATE_ITERATOR, $PK_STUDENT_ENROLLMENTS, $PK_STUDENT_COURSES)) {
						...
						add_total_hours_from_temp_to_original_student_array($temp_student_rows,$student_rows);
						// echo "=-=-==-=-=-=-=-=-=-=-==-=-";
...
function invoice_pdf_custom_header()
{
	...
	<table width="100%" >
			<tr>
				...
				<table width="100%" >
									<tr>
										<td width="100%" align="right" style="font-size:22px;border-bottom:1px solid #000;font-style: italic;font-family:helvetica;font-weight:normal;padding-top : 20px;text-align:right !important" >Attendance Course Offering 2 Week</td>
									</tr>
...
function add_week_data_to_std_rows(
	...
	for ($i = 0; $i <= 6; $i++) {
		...
		if (
			...
			$language_row_with_date_numbers .= "<td $styler  class='att_field'>" . (date('M j', strtotime($DATE))) . "</td>";
			$term_start_row_with_s_m_t_w_t_f_s .= "<td $styler  class='att_field'>" . (date('l', strtotime($DATE))) . "</td>";
		}
	}
	for ($i = 0; $i <= 6; $i++) {
		$DATE = date("Y-m-d", strtotime($START_DATE . ' + ' . $i . ' days'));
...
foreach ($JAVASCRIPT_COURSE_OFFERING_HEADERS as $course_offering => $header_html_content) {
	...
	$header_path = create_html_file('header_attendance_roster_weekly_signin_ETC.html', $header_cont, "invoice");
	$footer_path = create_html_file('footer_attendance_roster_weekly_signin_ETC.html', $footer_cont, "invoice");
	$file_name = "Monthly_Attendance_Analysis_".uniqid().".pdf";
	$exec = 'xvfb-run -a wkhtmltopdf -T 0 -R 0 -B 0 -L 0 --enable-local-file-access --orientation landscape --page-size A4 --page-width 210 --page-height 296 --margin-top 62mm --margin-left 7mm --margin-right 5mm  --margin-bottom '.$footer_height.' --footer-font-size 8 --header-html ' . $header_path . ' --footer-html  ' . $footer_path . ' ' . $html_to_print . ' ../school/temp/invoice/' . $file_name . ' 2>&1'; 
	global $http_path;
	...
	unlink('../school/temp/invoice/header_attendance_roster_weekly_signin_ETC.html');
	unlink('../school/temp/invoice/footer_attendance_roster_weekly_signin_ETC.html');
	exit;
}
...
function prepare_html_for_print($Output_Per_Offering, $COURSE_OFFERING_HEADERS , $total_table_flag = false)
{
	...
	foreach ($Output_Per_Offering as $PK => $Course_offering) {
		// dump($PK,$Course_offering);
		$new_table = '';
		$Total_table_html .= $COURSE_OFFERING_HEADERS[$PK];
		$new_table .= $COURSE_OFFERING_HEADERS[$PK];
		foreach ($Course_offering as $table_key => $table) {	 
			if($table_key == 'total_hour_table' && (isset($Course_offering[0]) && $Course_offering[0] != '' ) ){
				if($total_table_flag == true){
					$Total_table_html .= $table;
					$new_table .= $table;
				}
			}else{
				if($table_key != 'total_hour_table'){
					$Total_table_html .= $table;
					$new_table .= $table;
				}
				
			}  
			if($Course_offering[0] == ''){
				$new_table .= '<div style="width:100%;text-align: center; font-size:14px !important"> No Attendance Data Found.</div>';
			} 
