...
<head>
	...
	<title><?=MNU_EARNINGS_CALCULATION_TERM_BLOCK ?> | <?=$title?></title>
	<style>
            ...
            <div class="container-fluid">
                 <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h4 class="text-themecolor"><?=MNU_MONTHLY_EARNINGS_CALCULATION_TERM_BLOCK ?></h4>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
										...
										<div class="col-12 col-sm-12 ">
											<div class="row">
												...
												<div class="col-8 col-sm-8 text-right">
													<button onclick="window.location.href='manage_term_block'" type="button" class="btn waves-effect waves-light btn-info"><?=MNU_SETUP ?></button>
												</div>
												<div class="col-2 col-sm-1">
													<button onclick="window.location.href='earnings_report_term_block'" type="button" class="btn waves-effect waves-light btn-info"><?=MNU_REPORTS ?></button>
												</div>
											</div>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn waves-effect waves-light btn-dark" data-dismiss="modal" aria-label="Close" ><?=NO?></button>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		function get_earning_detail(val){
			jQuery(document).ready(function($) { 
				var data  = 'id='+val;
				var value = $.ajax({
					url: "ajax_get_earning_term_block_detail",	
					type: "POST",		 
					data: data,		
					async: false,
					cache: false,
		...
		function conf_earning_operation(val){
			jQuery(document).ready(function($) {
				...
				if(val == 1) {
					...
					var value = $.ajax({
						url: "ajax_earning_operation_term_block",
						type: "POST",		 
						data: data,		
						async: true,
						cache: false,
