...
if(!empty($_POST)){
	...
	$SCHOOL_CONTACT['SHOW_DSIS'] = $_POST['SHOW_DSIS'];
	if($_GET['id'] == ''){
		
		$SCHOOL_CONTACT['PK_ACCOUNT']  = $_SESSION['PK_ACCOUNT'];
		$SCHOOL_CONTACT['CREATED_BY']  = $_SESSION['PK_USER'];
		$SCHOOL_CONTACT['CREATED_ON']  = date("Y-m-d H:i");
		db_perform('S_SCHOOL_CONTACT', $SCHOOL_CONTACT, 'insert');
		$PK_SCHOOL_CONTACT = $db->insert_ID();
	} else {
		$SCHOOL_CONTACT['EDITED_BY']   = $_SESSION['PK_USER'];
		$SCHOOL_CONTACT['EDITED_ON']   = date("Y-m-d H:i");
		db_perform('S_SCHOOL_CONTACT', $SCHOOL_CONTACT, 'update'," PK_SCHOOL_CONTACT = '$_GET[id]' AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
	}
...
if($_GET['id'] == ''){
	...
	$res = $db->Execute("SELECT * FROM S_SCHOOL_CONTACT WHERE PK_SCHOOL_CONTACT = '$_GET[id]' AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' "); 
	...
	$SHOW_DSIS			= $res->fields['SHOW_DSIS'];
   ...
   <? require_once("pre_load.php"); ?>
								...
								<div class="p-20">
									...
									<div class="d-flex">
										<div class="col-12 col-sm-6 form-group">
											...
											<span class="bar"></span> 
											<label for="PK_CONTACT_TYPES"><?=CONTACT_TYPE?></label>
										</div>											
									</div>
									...
									<div class="d-flex">
										<div class="form-group col-12 col-sm-6">
											<div class="custom-control custom-checkbox mr-sm-2">
												<input type="checkbox" class="custom-control-input" id="SHOW_DSIS" name="SHOW_DSIS" value="1" <? if($SHOW_DSIS == 1) echo "checked"; ?> >
												<label class="custom-control-label" for="SHOW_DSIS"><?=SHOW_DSIS?></label>
											</div>
										</div>
									</div>
									...
									<div class="row">
										...
										<div class="col-9 col-sm-9">
											...
											<button type="button" onclick="window.location.href='school_profile?tab=contactTab'"  class="btn waves-effect waves-light btn-dark"><?=CANCEL?></button>
										</div>
									</div>
								</div>
							</form>
	...
	<script type="text/javascript">
		function validate_form(){
			if(document.getElementById('EMAIL').value == '' && document.getElementById('PHONE').value == '' && document.getElementById('SHOW_DSIS').checked == true) {
