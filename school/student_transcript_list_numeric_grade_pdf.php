...
class MYPDF extends TCPDF {
    ...
    public function Footer() {
		...
		$res_type = $db->Execute("SELECT FOOTER_LOC, CONTENT FROM S_PDF_FOOTER,S_PDF_FOOTER_CAMPUS WHERE S_PDF_FOOTER.ACTIVE = 1 AND S_PDF_FOOTER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PDF_FOR = 10 AND S_PDF_FOOTER.PK_PDF_FOOTER = S_PDF_FOOTER_CAMPUS.PK_PDF_FOOTER AND PK_CAMPUS = '$PK_CAMPUS'  ");
		
		//$res_type = $db->Execute("SELECT * FROM S_PDF_FOOTER WHERE ACTIVE = 1 AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PDF_FOR = 10"); //Ticket # 1234 
		/** End DIAM - 1315 **/
	 ...
	 if($res_course->RecordCount() > 0) // DIAM-2239
	 {
		...
		$txt .= '<div style="border-top: 2px #c0c0c0 solid" ></div>
				<table border="0" cellspacing="0" cellpadding="3" width="100%" >
					...
					<tr>
						<td width="20%" ><br /><br /><u><b>Term</b></u></td>
						<td width="10%" ><br /><br /><u><b>Course</b></u></td>
						<td width="30%" ><br /><br /><u><b>Course Description</b></u></td>
						<td width="15%" align="right" ><br /><br /><u><b>Numeric Grade</b></u></td>
					...
					if($_GET['exclude_tc'] != 1) {
						...
						while (!$res_tc->EOF) {
							...
							//diam-726
							if($res_grade_data->fields['NUMBER_GRADE'] > 0) {
								$tot_obt_numeric += $res_grade_data->fields['NUMBER_GRADE'];
								$tot_numeric 	 += 100;
								$numeric_count++;
							}
							...
							$txt .= '<tr>
										...
										<td width="15%" align="right" >'.$res_tc->fields['NUMBER_GRADE'].'</td>
					...
					while (!$res_course->EOF) { 
						...
						$NUMERIC_GRADE = '';
						if(trim($res_course->fields['GRADE']) != '') {
							$tot_obt_numeric += $res_course->fields['NUMERIC_GRADE'];
							$tot_numeric 	 += 100;
							$numeric_count++;
						}
			...
			$txt .= '<tr>
						<td width="10%" style="border-top:0.5px solid #c0c0c0" ></td>
						<td width="20%" style="border-top:0.5px solid #c0c0c0" ></td>
						<td width="30%" style="border-top:0.5px solid #c0c0c0" colspan="2"><i><b style="font-size:30px">Student Transcript Total</b></i></td>
						<td width="15%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker(($Numerator1/$Denominator),2).'</b></i></td>
						<td width="10%" style="border-top:0.5px solid #c0c0c0" align="right" ><i><b style="font-size:30px">'.number_format_value_checker($c_in_att_tot,2).'</b></i></td>
