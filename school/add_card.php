...
if(strtolower($_POST['FinalStatus']) == 'success'){
	if($_POST['IS_PRIMARY'] == 1) {
		$db->Execute("UPDATE S_STUDENT_CREDIT_CARD SET IS_PRIMARY = 0 WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_STUDENT_MASTER = '$_POST[s_id]' ");
	}
	...
	$res = $db->Execute("SELECT PK_STUDENT_CREDIT_CARD FROM S_STUDENT_CREDIT_CARD WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_STUDENT_MASTER = '$_POST[s_id]' ");
