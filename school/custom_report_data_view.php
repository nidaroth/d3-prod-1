				...
				<div class="row page-titles" style="padding-bottom: 10px;" >
                    <div class="col-md-8 align-self-center">
                        <h4 class="text-themecolor">
							...
							echo CUSTOM_REPORT_PAGE_TITLE.' - '.$res->fields['REPORT_NAME'] ?>
							
							<button onclick="javascript:window.location.href='custom_report_excel?id=<?=$_GET['id']?>'" type="button" class="btn waves-effect waves-light btn-info"><?=EXCEL?></button>
							
							<button onclick="javascript:window.location.href='custom_report_pdf?id=<?=$_GET['id']?>'" type="button" class="btn waves-effect waves-light btn-info"><?=PDF?></button>
						</h4>
                    </div>
				</div>
										...
										<table data-toggle="table" data-height="500" data-mobile-responsive="true" class="table-striped" >
											<thead>
												<tr>
													...
													$res_fields = $db->Execute("SELECT FIELDS,FIELD_SIZE,SORT_ORDER from S_CUSTOM_REPORT_DETAIL WHERE PK_CUSTOM_REPORT = '$_GET[id]' AND FIELD_FOR = 'INFO' AND FIELDS != '' AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ORDER BY SORT_ORDER ASC, PK_CUSTOM_REPORT_DETAIL ASC");	
													$original_order_array = [];
													while (!$res_fields->EOF){
														$field 			= $res_fields->fields['FIELDS'];
														$FIELD_SIZE[] 	= $res_fields->fields['FIELD_SIZE'];
														...
														} else if($field == "IF(S_STUDENT_ENROLLMENT.ENTRY_DATE = '0000-00-00','',DATE_FORMAT(S_STUDENT_ENROLLMENT.ENTRY_DATE,'%Y-%m-%d' )) AS ENTRY_DATE" ){
															$val = ENTRY_DATE;
														} else if($field == "IF(S_STUDENT_ENROLLMENT.ENTRY_TIME = '00:00:00','',DATE_FORMAT(S_STUDENT_ENROLLMENT.ENTRY_TIME,'%h:%i %p' )) AS ENTRY_TIME" ){ //Ticket # 1623
															$val = ENTRY_TIME;
														...
														$original_order_array[] = [$val , $res_fields->fields['SORT_ORDER']];
														?>
													...
													$res_fields = $db->Execute("SELECT FIELD_NAME, FIELDS, PK_DATA_TYPES,SORT_ORDER, TAB from S_CUSTOM_REPORT_DETAIL, S_CUSTOM_FIELDS WHERE PK_CUSTOM_REPORT = '$_GET[id]' AND FIELD_FOR = 'CUSTOM_FIELDS' AND FIELDS != '' AND S_CUSTOM_REPORT_DETAIL.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND S_CUSTOM_REPORT_DETAIL.FIELDS = S_CUSTOM_FIELDS.PK_CUSTOM_FIELDS AND S_CUSTOM_FIELDS.ACTIVE = 1 ORDER BY SORT_ORDER ASC, PK_CUSTOM_REPORT_DETAIL ASC");	
													while (!$res_fields->EOF){ 
														...
														$original_order_array[] = [$res_fields->fields['FIELD_NAME'] , $res_fields->fields['SORT_ORDER']];?>
														<!-- <th><?=$res_fields->fields['FIELD_NAME']?></th> -->
													<? $res_fields->MoveNext();
													} ?>
...
<!-- END OF SORT CODE  -->
											...
											<tbody>
												...
												LEFT JOIN M_RESIDENCY_TUITION_STATUS ON M_RESIDENCY_TUITION_STATUS.PK_RESIDENCY_TUITION_STATUS = S_STUDENT_ENROLLMENT.PK_RESIDENCY_TUITION_STATUS -- DIAM-2370
												...
												foreach ($data_rows as $key => $value) {
													...
													foreach ($original_order_array as $key1 => $value1) {
												
														$tmp_row[$key1] = $value[$value1['original_order']];
