...
foreach($PK_STUDENT_MASTER_arr as $PK_STUDENT_MASTER) { ///* Ticket # 1144  */
	...
	$txt = '<table border="0" cellspacing="0" cellpadding="2" width="100%">
			...
			$res_tc = $db->Execute("SELECT S_COURSE.COURSE_CODE FROM S_STUDENT_CREDIT_TRANSFER LEFT JOIN S_COURSE ON S_COURSE.PK_COURSE = S_STUDENT_CREDIT_TRANSFER.PK_EQUIVALENT_COURSE_MASTER , M_CREDIT_TRANSFER_STATUS WHERE S_STUDENT_CREDIT_TRANSFER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND PK_STUDENT_ENROLLMENT IN ($_GET[eid]) AND M_CREDIT_TRANSFER_STATUS.PK_CREDIT_TRANSFER_STATUS = S_STUDENT_CREDIT_TRANSFER.PK_CREDIT_TRANSFER_STATUS AND SHOW_ON_TRANSCRIPT = 1");
			while (!$res_tc->EOF) {
				$txt .= '<tr nobr="true" >
							<td width="18%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;">Transfer</td>
							<td width="12%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;">
								'.$res_tc->fields['COURSE_CODE'].'
							</td>
							<td width="13%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;"></td>
							<td width="8%" align="right" style="border-left:0.5px solid #000;border-top:0.5px solid #000;"></td>
							<td width="8%" align="right" style="border-left:0.5px solid #000;border-top:0.5px solid #000;"></td>
							<td width="9%" align="right" style="border-left:0.5px solid #000;border-top:0.5px solid #000;"></td>
							<td width="8%" align="center" style="border-left:0.5px solid #000;border-top:0.5px solid #000;"></td>
							<td width="5%" align="center" style="border-left:0.5px solid #000;border-top:0.5px solid #000;"></td>
							<td width="9%" align="right" style="border-left:0.5px solid #000;border-top:0.5px solid #000;"></td>
							<td width="10%" align="right" style="border-left:0.5px solid #000;border-top:0.5px solid #000;border-right:0.5px solid #000;" ></td>
						</tr>';
			...
			//Ticket # 1144  
			$res_course_schedule = $db->Execute("select IF(SCHEDULE_DATE != '0000-00-00', DATE_FORMAT(SCHEDULE_DATE,'%m/%d/%Y &nbsp;&nbsp;%a'),'') AS SCHEDULE_DATE1, S_STUDENT_SCHEDULE.PK_SCHEDULE_TYPE, IF(END_TIME != '00:00:00', DATE_FORMAT(END_TIME,'%h:%i %p'),'') AS END_TIME, IF(START_TIME != '00:00:00', DATE_FORMAT(START_TIME,'%h:%i %p'),'') AS START_TIME, S_STUDENT_SCHEDULE.HOURS, COURSE_CODE, TRANSCRIPT_CODE, SCHEDULE_TYPE, IF(S_STUDENT_ATTENDANCE.COMPLETED = 1,'Y','N') as COMPLETED , M_ATTENDANCE_CODE.CODE AS ATTENDANCE_CODE, SESSION, SESSION_NO,ATTENDANCE_HOURS, S_STUDENT_ATTENDANCE.PK_ATTENDANCE_CODE  from 
			...
			$res_tc = $db->Execute("SELECT SUM(HOUR) as HOUR FROM S_STUDENT_CREDIT_TRANSFER, M_CREDIT_TRANSFER_STATUS WHERE S_STUDENT_CREDIT_TRANSFER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND PK_STUDENT_ENROLLMENT IN($_REQUEST[eid]) AND M_CREDIT_TRANSFER_STATUS.PK_CREDIT_TRANSFER_STATUS = S_STUDENT_CREDIT_TRANSFER.PK_CREDIT_TRANSFER_STATUS AND SHOW_ON_TRANSCRIPT = 1");
			
			$txt .= '<tr>
						<td width="18%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;border-bottom:0.5px solid #000;" align="center" ><b>Total Scheduled: '.number_format_value_checker($total_scheduled,2).'</b></td>
						<td width="25%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;border-bottom:0.5px solid #000;;border-right:0.5px solid #000;" align="center" >
							<b>Totals for Completed Attendance</b>
						</td>
						<td width="16%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;border-bottom:0.5px solid #000;;border-right:0.5px solid #000;" align="center" >
							<b>Scheduled: '.number_format_value_checker($total_completed_scheduled,2).'</b>
						</td>
						<td width="17%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;border-bottom:0.5px solid #000;;border-right:0.5px solid #000;" align="center" >
							<b>Transferred: '.number_format_value_checker($res_tc->fields['HOUR'],2).'</b>
						</td>
						<td width="24%" style="border-left:0.5px solid #000;border-top:0.5px solid #000;border-bottom:0.5px solid #000;;border-right:0.5px solid #000;" align="center" >
							<b>Attended: '.number_format_value_checker($total_attended,2).'</b>
						</td>
					</tr>
				</tbody>
			</table>';
