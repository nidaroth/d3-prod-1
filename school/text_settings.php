...
if(!empty($_POST)){
	...
	$res = $db->Execute("select * from Z_TEXT_SETTINGS WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
	...
	if($res->RecordCount() == 0){
		...
		db_perform('Z_TEXT_SETTINGS', $TEXT_SETTINGS, 'insert');
		$PK_TEXT_SETTINGS = $db->insert_ID();
	} else {
		$TEXT_SETTINGS['EDITED_BY'] = $_SESSION['PK_USER'];
		$TEXT_SETTINGS['EDITED_ON'] = date("Y-m-d H:i:s");
		db_perform('Z_TEXT_SETTINGS', $TEXT_SETTINGS, 'update'," PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' ");
								...
								<div class="p-20">
									...
									<div class="row">
										...
										<div class="col-9 col-sm-9">
											...
											<button type="button" onclick="window.location.href='index.php'"  class="btn waves-effect waves-light btn-dark"><?=CANCEL?></button>
										</div>
									</div>
								</div>
							</form>
