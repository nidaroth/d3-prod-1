...
if(!empty($_POST)){
	...
	if (trim($_FILES['FILE_XLX']['name'])!= ""){
		...
		if(strtolower($extn[$ii]) == 'xlsx' || strtolower($extn[$ii]) == 'xls' || strtolower($extn[$ii]) == 'csv'){ 	
			$newfile1 = 'temp/student_'.date("Y-m-d-H-i-s").'.'.$extn[$ii];
			...
			foreach($sheetData as $row){
				...
				header("location:student_upload_mapping?id=".$PK_MAP_MASTER);
				exit;
			}
		}else{
...
<head>
	...
	<title><?=MNU_LEAD_UPLOAD?> | <?=$title?></title>
</head>
            ...
            <div class="container-fluid">
                 <div class="row page-titles">
                    <div class="col-md-12 align-self-center">
                        <h4 class="text-themecolor"><?=MNU_LEAD_UPLOAD?> </h4>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
