...
class MYPDF extends TCPDF {
    ...
    public function Footer() {
		...
		$res_type = $db->Execute("SELECT FOOTER_LOC, CONTENT FROM S_PDF_FOOTER,S_PDF_FOOTER_CAMPUS WHERE S_PDF_FOOTER.ACTIVE = 1 AND S_PDF_FOOTER.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PDF_FOR = 15 AND S_PDF_FOOTER.PK_PDF_FOOTER = S_PDF_FOOTER_CAMPUS.PK_PDF_FOOTER ");
...
/* Ticket # 1234 */
$res_type = $db->Execute("SELECT * FROM S_PDF_FOOTER WHERE ACTIVE = 1 AND PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND PDF_FOR = 15");
...
foreach($PK_STUDENT_MASTER_ARR as $PK_STUDENT_MASTER){
	...
	$pdf->campus 			= $res_camp->fields['PK_CAMPUS'];
	$pdf->startPageGroup();
	$pdf->AddPage();
	...
	$txt .= '<table border="0" cellspacing="0" cellpadding="3" width="100%" >
		...
		/* Ticket # 1588 */					
				...
				foreach($PK_TERM_MASTER_ARR as $PK_TERM_MASTER1){ 
					...
					$tot_UNITS 		= 0;
					$tot_FA_UNITS 	= 0;
					$res_course = $db->Execute("select S_COURSE_OFFERING.PK_COURSE, S_COURSE_OFFERING.PK_COURSE_OFFERING,TRANSCRIPT_CODE,SESSION,SESSION_NO, COURSE_DESCRIPTION, UNITS, FA_UNITS,START_DATE as START_DATE1, IF(START_DATE = '0000-00-00','',DATE_FORMAT(START_DATE, '%m/%d/%Y' )) AS START_DATE, IF(END_DATE = '0000-00-00','',DATE_FORMAT(END_DATE, '%m/%d/%Y' )) AS END_DATE, CONCAT(FIRST_NAME,' ',MIDDLE_NAME,' ',LAST_NAME) AS INSTRUCTOR from S_STUDENT_COURSE LEFT JOIN S_COURSE_OFFERING ON S_COURSE_OFFERING.PK_COURSE_OFFERING = S_STUDENT_COURSE.PK_COURSE_OFFERING LEFT JOIN M_SESSION On M_SESSION.PK_SESSION = S_COURSE_OFFERING.PK_SESSION LEFT JOIN S_COURSE ON S_COURSE.PK_COURSE = S_COURSE_OFFERING.PK_COURSE LEFT JOIN S_COURSE_OFFERING_SCHEDULE ON S_COURSE_OFFERING_SCHEDULE.PK_COURSE_OFFERING = S_COURSE_OFFERING.PK_COURSE_OFFERING LEFT JOIN S_EMPLOYEE_MASTER ON S_EMPLOYEE_MASTER.PK_EMPLOYEE_MASTER = S_COURSE_OFFERING.INSTRUCTOR WHERE PK_STUDENT_MASTER = '$PK_STUDENT_MASTER' AND S_STUDENT_COURSE.PK_TERM_MASTER = '$PK_TERM_MASTER1' ORDER BY TRANSCRIPT_CODE ASC ");	
					...
					while (!$res_course->EOF) { 
						...
						$PK_COURSE_ids .= $res_course->fields['PK_COURSE'];;
						...
						$txt .= '<tr>
									...
									<td width="5%" align="right" >'.$res_course->fields['UNITS'].'</td>
									<td width="7%" align="right" >'.$res_course->fields['FA_UNITS'].'</td>
	...
	$txt .= '<table border="0" cellspacing="0" cellpadding="0" width="100%" >
				<tr>
					<td width="100%" ><br /><b>Books Required</b></td>
				</tr>';
				if($PK_COURSE_ids != '') {
					$res_book = $db->Execute("select COURSE_CODE, S_COURSE_FEE.DESCRIPTION, FEE_AMT, ISBN_10, ISBN_13 FROM S_COURSE, S_COURSE_FEE, M_AR_LEDGER_CODE WHERE S_COURSE_FEE.PK_AR_LEDGER_CODE = M_AR_LEDGER_CODE.PK_AR_LEDGER_CODE AND S_COURSE.PK_COURSE = S_COURSE_FEE.PK_COURSE AND S_COURSE_FEE.PK_COURSE IN ($PK_COURSE_ids) AND S_COURSE_FEE.PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' AND trim(CODE) like '%books%' "); 
					while (!$res_book->EOF) {
						$txt .= '<tr>
									<td width="10%" >'.$res_book->fields['COURSE_CODE'].'</td>
									<td width="10%" >'.$res_book->fields['DESCRIPTION'].'</td>
									<td width="7%" align="right" >$ '.number_format_value_checker($res_book->fields['FEE_AMT'], 2).'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
									<td width="10%" >'.$res_book->fields['ISBN_10'].'</td>
									<td width="10%" >'.$res_book->fields['ISBN_13'].'</td>
								</tr>';
...
$file_name = 'Student Schedule with Books.pdf';
/*if($browser == 'Safari')
