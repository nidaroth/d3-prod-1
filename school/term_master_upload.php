...
if(!empty($_POST)){
	...
	if (trim($_FILES['FILE_XLX']['name'])!= ""){
		...
		if(strtolower($extn[$ii]) == 'xlsx' || strtolower($extn[$ii]) == 'xls' || strtolower($extn[$ii]) == 'csv'){ 	
			$newfile1 = 'temp/term_'.date("Y-m-d-H-i-s").'.'.$extn[$ii];
			...
			$EXCEL_MAP_MASTER['PK_ACCOUNT'] 	= $_SESSION['PK_ACCOUNT'];
			...
			foreach($sheetData as $row){
				...
				header("location:term_master_upload_mapping?id=".$PK_MAP_MASTER);
				exit;
			}
		}else{
...
<head>
	...
	<title><?=TERM_MASTER_PAGE_TITLE.' '.UPLOAD?> | <?=$title?></title>
</head>
            ...
            <div class="container-fluid">
                 <div class="row page-titles">
                    <div class="col-md-12 align-self-center">
                        <h4 class="text-themecolor"><?=TERM_MASTER_PAGE_TITLE.' '.UPLOAD?> </h4>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
