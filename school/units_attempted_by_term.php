...
if(!empty($_POST)){
	...
	if(isset($_POST['EXCLUDE_NON_PROGRAM_COURSES'])){
	 ...
	 if($_POST['FORMAT']==1){
		include('units_attempted_by_term_pdf.php');
	}else if($_POST['FORMAT']==2){
		include('units_attempted_by_term_excel.php');		
	}
}
...
<head>
	...
	<title><?=MNU_UNITS_ATTEMPTED_BY_TERM ?> | <?=$title?></title>
	<style>
...
<body class="horizontal-nav boxed skin-megna fixed-layout">
   ...
   <div id="loaders" style="display: none;">
                            ...
                            <div class="card-body">
                                <form class="floating-labels" method="post" name="form1" id="form1" >
									...
									<div class="col-md-12 m-b-30">
										...
										<div class="col-md-3" id="PK_CAMPUS_DIV"  >
											<select id="PK_CAMPUS1" name="PK_CAMPUS1[]" multiple class="form-control " onchange="get_course_term_from_campus();">
											<? $res_type = $db->Execute("select CAMPUS_CODE,PK_CAMPUS,ACTIVE from S_CAMPUS WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' order by ACTIVE DESC,CAMPUS_CODE ASC");
												while (!$res_type->EOF) { 
													...
													?>
													<option value="<?=$res_type->fields['PK_CAMPUS']?>" <? if($res_type->RecordCount() == 1) echo "selected"; ?> <? if($res_type->fields['ACTIVE'] == 0) echo "class='option_red'"; ?>><?=$res_type->fields['CAMPUS_CODE'].$option_label?></option>
												<?	$res_type->MoveNext();
												} ?>
											</select>
										</div>
										...
										<div class="col-md-3" id="PK_TERM_MASTER1_DIV" >
    ...
    max-width: 18%;">
									...
									<div class="col-md-12 m-b-30">
											<label>Student Parameters</label>
										</div>								
										<div class="col-md-2">
										</lable><?=CAMPUS?></lable>
											...
											<? $res_type = $db->Execute("select PK_TERM_MASTER, IF(BEGIN_DATE = '0000-00-00','',DATE_FORMAT(BEGIN_DATE, '%m/%d/%Y' )) AS  BEGIN_DATE_1, IF(END_DATE = '0000-00-00','',DATE_FORMAT(END_DATE, '%m/%d/%Y' )) AS  END_DATE_1, TERM_DESCRIPTION,ACTIVE from S_TERM_MASTER WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]'  order by ACTIVE DESC, BEGIN_DATE DESC");
												while (!$res_type->EOF) { 
													...
													if($res_type->fields['ACTIVE'] == 0)
														$option_label = " (Inactive)";
													?>
													<option value="<?=$res_type->fields['PK_TERM_MASTER']?>"  <? if($res_type->fields['ACTIVE'] == 0) echo "class='option_red'"; ?>><?=$res_type->fields['BEGIN_DATE_1'].' - '.$res_type->fields['END_DATE_1'].' - '.$res_type->fields['TERM_DESCRIPTION'].$option_label?></option>
												<?	$res_type->MoveNext();
												} ?>
											</select>
										</div>
											...
											<? $res_type = $db->Execute("select PK_CAMPUS_PROGRAM,CODE,DESCRIPTION,ACTIVE from M_CAMPUS_PROGRAM WHERE PK_ACCOUNT = '$_SESSION[PK_ACCOUNT]' order by ACTIVE DESC,CODE ASC");
												while (!$res_type->EOF) { 
													...
													if($res_type->fields['ACTIVE'] == 0)
														$option_label = " (Inactive)";
													?>
													<option value="<?=$res_type->fields['PK_CAMPUS_PROGRAM']?>" <? if($res_type->fields['ACTIVE'] == 0) echo "class='option_red'"; ?>><?=$res_type->fields['CODE'].' - '.$res_type->fields['DESCRIPTION'].$option_label?></option>
												<?	$res_type->MoveNext();
												} ?>
											</select>
										</div>
										...
										<div class="col-md-2" id="PK_FUNDING_DIV" >
										</lable><?=ucfirst(strtolower(FUNDING))?></lable>
											...
											<option value="0">All</option>
												...
												while (!$res_type->EOF) { 
													...
													if($res_type->fields['ACTIVE'] == 0)
														$option_label .= " (Inactive)"; ?>
													<option value="<?=$res_type->fields['PK_FUNDING']?>" <? if($res_type->fields['ACTIVE'] == 0) echo "class='option_red'"; ?> ><?=$option_label ?></option>
												<?	$res_type->MoveNext();
												} ?>
											</select>
										</div>
	...
	<script type="text/javascript">
		...
		function get_count(){
			...
			//DIAM-2195
			if(document.getElementById('TOTAL_COUNT').innerText==document.getElementById('SELECTED_COUNT').innerText){				
				if(document.getElementById('SEARCH_SELECT_ALL').checked == false)
				  get_count2();
			}else if(document.getElementById('TOTAL_COUNT').innerText > document.getElementById('SELECTED_COUNT').innerText){
				if(document.getElementById('SEARCH_SELECT_ALL').checked == true) 
					fun_select_all2()
		...
		function get_count2(){
			...
			document.getElementById('SELECTED_COUNT').innerHTML = tot
			show_btn()
			document.getElementById('SEARCH_SELECT_ALL').click();
	...
	function get_course_term_from_campus(){
			jQuery(document).ready(function($) {
				var data  = 'PK_CAMPUS='+$('#PK_CAMPUS1').val();
	...
	jQuery(document).ready(function($) {
		...
		$('#PK_FUNDING').multiselect({
			includeSelectAllOption: true,
			allSelectedText: 'All <?=ucfirst(strtolower(FUNDING))?>',
			nonSelectedText: 'Select <?=ucfirst(strtolower(FUNDING))?>',
			numberDisplayed: 2,
			nSelectedText: '<?=ucfirst(strtolower(FUNDING))?> selected'
		});
