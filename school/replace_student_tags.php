<?
function replace_mail_content($txt, $PK_STUDENT_ENROLLMENT, $PK_ACCOUNT){
	global $db;
	...
	WHERE 
	S_STUDENT_MASTER.PK_ACCOUNT = '$PK_ACCOUNT' AND 
	...
	while (!$res->EOF) {
		...
		if($field == 'SCHOOL_LOGO' || $field == 'SCHOOL_PDF_LOGO' || $field == 'STUD_IMG'){
			...
			if($res_stu->fields[$field] != '') {
				$IMG = '<img src="'.$res_stu->fields[$field].'" style="width:80px" >';
				//echo $IMG.'<br />';
				$txt = str_replace($res->fields['TAGS'],$IMG,$txt);	
			}
		} else {
			if($field == 'SSN') {
				$SSN_1 	 		= my_decrypt($PK_ACCOUNT,$res_stu->fields[$field]);
